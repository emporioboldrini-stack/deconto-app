...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};function o(){let e=a.getSettings();return`
            ...
            <div style="display: grid; grid-template-columns: 1.25fr 1fr; gap: 24px; align-items: start;">
              ...
              <div style="display: flex; flex-direction: column; gap: 16px;">
                ...
                <div style="background: rgba(16, 185, 129, 0.05); padding: 16px; border-radius: 12px; border: 1px solid rgba(16, 185, 129, 0.2); display: flex; flex-direction: column; gap: 10px; margin-bottom: 4px;">
                  ...
                  <button id="btn-modal-perform-refill" data-board-code="${t.shortCode}" class="btn btn-primary" style="padding: 10px; font-size: 0.85rem; font-weight: 800; width: 100%; border-radius: 6px; background: linear-gradient(135deg, var(--accent-green), #059669);">
  ...
  </select>`}var p=[`Ciao`,`Jessica`,`Altro`],m=Array.from({length:21},(e,t)=>String(2020+t));function h(e,t){return`<select id="${e}" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px; font-weight: 700;">
    ${p.map(e=>`<option value="${e}" ${t===e?`selected`:``}>${e}</option>`).join(``)}
  </select>`}function g(e,t){return`<select id="${e}" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px; font-weight: 700;">
    ${m.map(e=>`<option value="${e}" ${t===e?`selected`:``}>${e}</option>`).join(``)}
  </select>`}function _(e=`clients`,t=null){let n=a.getClients(),r=a.getMachines(),i=a.getBoards();a.getRefillLogs();let o=``;if(e===`clients`&&t){let e=n.find(e=>e.id===t);if(e){let t=r.find(t=>t.clientId===e.id);o=`
  ...
  `}function y(e=null){let t=e?e.clientName:`Studio Legale Brambilla`,n=e?e.boardShortCode:`3467`,r=e?e.tokenOtp:`OTP-9981-X79K2`,i=e?parseInt(e.credits,10):200,a=e?!!e.success:!1;return`
  ...
  `}function b(e=null,t=null){let n=a.getClients(),r=a.getBoards(),i=a.getMachines(),o=new Date;o.setMonth(o.getMonth()+1);let s=o.toISOString().split(`T`)[0],c=o.toLocaleDateString(`it-IT`,{day:`2-digit`,month:`2-digit`,year:`numeric`});return`
  ...
  `}function w(e=null){let t=a.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return a.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function T(e,t=null,n=null){let r=a.getUsers(),i=a.getRoleLabels(),o=a.getPermissions(),s=a.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
  ...
  `}function D(){let e=a.getSettings();return`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var O={currentUser:a.getCurrentUser(),activeTab:`dashboard`,editingId:null,editingStaffUserId:null,showProfileModal:!1,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingDecontoCode:null,viewingEmailId:null,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`,generatedOtpUrl:null,generatedOtpToken:null,diyParams:null,refillsFilter:{boardCode:``,clientName:``,date:``},extractionsFilter:{boardCode:``,clientName:``,date:``},qrParams:{clientId:``,machineId:``,boardShortCode:``,isGenerated:!1},simulatingBoardCode:null};function k(){let e=document.getElementById(`app`);if(!O.currentUser){e.innerHTML=o(),A();return}let t=``;switch(O.activeTab){case`dashboard`:t=c(O.activeTab,O.viewingDecontoCode,O.dashSearchQuery,O.dashSearchCategory,O.dashSortColumn,O.dashSortDirection,O.viewingKpiModal,O.kpiPeriod,O.kpiChartType,O.kpiCustomStart,O.kpiCustomEnd);break;case`clients`:case`machines`:case`deconto_boards`:t=_(O.activeTab,O.editingId);break;case`adr_visits`:t=v();break;case`otp_generator`:t=b(O.generatedOtpUrl,O.generatedOtpToken);break;case`client_diy`:t=y(O.diyParams);break;case`qr_generator`:t=x(O.qrParams);break;case`refills_history`:t=S(O.refillsFilter);break;case`extractions_history`:t=C(O.extractionsFilter);break;case`simulator`:t=w(O.simulatingBoardCode);break;case`user_mgmt`:case`user_management`:case`permissions_matrix`:t=T(O.activeTab,O.editingStaffUserId,O.viewingEmailId);break;case`settings`:t=D();break;default:t=c()}let n=E(O.showProfileModal,O.currentUser);e.innerHTML=`
    <div class="app-layout">
      ${s(O.currentUser,O.activeTab)}
      <main class="main-content">
        ${t}
      </main>
    </div>
    ${n}
  `,j()}function A(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value.trim();try{let e=a.login(t,n);O.currentUser=e,O.activeTab=e.role===`ADR`?`adr_visits`:`dashboard`,k()}catch(e){alert(e.message)}})}function j(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(O.activeTab=t,O.editingId=null,O.editingStaffUserId=null,O.viewingDecontoCode=null,O.viewingEmailId=null,k())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{O.showProfileModal=!0,k()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{O.showProfileModal=!1,k()}),n&&n.addEventListener(`click`,()=>{O.showProfileModal=!1,k()});let r=document.getElementById(`profile-edit-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-new-password`).value;try{O.currentUser=a.updateUserProfile(O.currentUser.id,{name:t,email:n,newPassword:r?r.trim():void 0}),O.showProfileModal=!1,alert(`✅ Credenziali del profilo aggiornate con successo!`),k()}catch(e){alert(`Errore: ${e.message}`)}});let i=document.getElementById(`btn-logout`);i&&i.addEventListener(`click`,()=>{a.logout(),O.currentUser=null,O.activeTab=`dashboard`,k()});let o=document.getElementById(`btn-toggle-add-client`),s=document.getElementById(`add-client-form-container`),c=document.getElementById(`btn-cancel-add-client`),l=document.getElementById(`btn-save-new-client`);o&&s&&o.addEventListener(`click`,()=>{let e=s.style.display===`none`||!s.style.display;s.style.display=e?`block`:`none`}),c&&s&&c.addEventListener(`click`,()=>{s.style.display=`none`}),l&&l.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-type`).value,n=document.getElementById(`new-cli-ref`).value.trim(),r=document.getElementById(`new-cli-phone`).value.trim(),i=document.getElementById(`new-cli-email`).value.trim(),o=document.getElementById(`new-cli-city`).value.trim(),s=document.getElementById(`new-cli-machine`).value;if(!e){alert(`Inserisci la Ragione Sociale / Nome Cliente!`);return}try{let c=a.addClient({name:e,clientType:t,refPerson:n,phone:r,email:i,city:o,machineId:s});alert(`✅ Cliente "${c.name}" registrato con successo!`),k()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{O.editingId=e.getAttribute(`data-id`),k()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo cliente?`))try{a.deleteClient(t),alert(`✅ Cliente eliminato dall'anagrafica!`),k()}catch(e){alert(`Errore: ${e.message}`)}})});let u=document.getElementById(`btn-cancel-edit-client`);u&&u.addEventListener(`click`,()=>{O.editingId=null,k()});let d=document.getElementById(`form-edit-client`);d&&d.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value.trim(),r=document.getElementById(`edit-cli-type`).value,i=document.getElementById(`edit-cli-ref`).value.trim(),o=document.getElementById(`edit-cli-phone`).value.trim(),s=document.getElementById(`edit-cli-city`).value.trim(),c=document.getElementById(`edit-cli-address`).value.trim(),l=document.getElementById(`edit-cli-machine`).value;try{a.updateClient(t,{name:n,clientType:r,refPerson:i,phone:o,city:s,address:c,assignedMachineId:l}),O.editingId=null,alert(`✅ Scheda Cliente salvata con successo!`),k()}catch(e){alert(`Errore: ${e.message}`)}});let f=document.getElementById(`btn-toggle-add-machine`),p=document.getElementById(`add-machine-form-container`),m=document.getElementById(`btn-cancel-add-machine`),h=document.getElementById(`btn-save-new-machine`);f&&p&&f.addEventListener(`click`,()=>{let e=p.style.display===`none`||!p.style.display;p.style.display=e?`block`:`none`}),m&&p&&m.addEventListener(`click`,()=>{p.style.display=`none`}),h&&h.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-year`).value.trim(),i=document.getElementById(`new-mc-board`).value,o=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Inserisci Seriale Macchina e Modello!`);return}try{let s=a.addMachine({serialNumber:e,brand:t,model:n,productionYear:r,boardId:i,clientId:o});alert(`✅ Macchina da caffè SN "${s.serialNumber}" registrata con successo!`),k()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{O.editingId=e.getAttribute(`data-id`),k()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa macchina da caffè?`))try{a.deleteMachine(t),alert(`✅ Macchina eliminata dal parco macchine!`),k()}catch(e){alert(`Errore: ${e.message}`)}})});let g=document.getElementById(`btn-cancel-edit-mc`);g&&g.addEventListener(`click`,()=>{O.editingId=null,k()});let _=document.getElementById(`form-edit-machine`);_&&_.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value.trim(),r=document.getElementById(`edit-mc-brand`).value.trim(),i=document.getElementById(`edit-mc-model`).value.trim(),o=document.getElementById(`edit-mc-year`).value.trim(),s=document.getElementById(`edit-mc-board`).value,c=document.getElementById(`edit-mc-client`).value;try{a.updateMachine(t,{serialNumber:n,brand:r,model:i,productionYear:o,boardId:s,clientId:c}),O.editingId=null,alert(`✅ Scheda Macchina salvata con successo!`),k()}catch(e){alert(`Errore: ${e.message}`)}});let v=document.getElementById(`btn-toggle-add-board`),y=document.getElementById(`add-board-form-container`),b=document.getElementById(`btn-cancel-add-board`),x=document.getElementById(`btn-save-new-board`);v&&y&&v.addEventListener(`click`,()=>{let e=y.style.display===`none`||!y.style.display;y.style.display=e?`block`:`none`}),b&&y&&b.addEventListener(`click`,()=>{y.style.display=`none`}),x&&x.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,i=document.getElementById(`new-board-machine`).value,o=document.getElementById(`new-board-groupcount`),s=o?parseInt(o.value,10):1;if(!e){alert(`Inserisci il Codice a 4 cifre del Deconto!`);return}try{let o=a.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,groupCount:s,machineId:i});alert(`✅ Scheda Deconto #${o.shortCode} salvata PERMANENTEMENTE nel database!`),k()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{O.editingId=e.getAttribute(`data-id`),k()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa scheda Deconto?`))try{a.deleteBoard(t),alert(`✅ Scheda Deconto eliminata!`),k()}catch(e){alert(`Errore: ${e.message}`)}})});let S=document.getElementById(`btn-cancel-edit-board`);S&&S.addEventListener(`click`,()=>{O.editingId=null,k()});let C=document.getElementById(`form-edit-board`);C&&C.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value.trim(),r=document.getElementById(`edit-board-hwserial`).value.trim(),i=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-version`).value,s=document.getElementById(`edit-board-machine`).value,c=document.getElementById(`edit-board-groupcount`),l=c?parseInt(c.value,10):1;try{a.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:i,version:o,groupCount:l,machineId:s}),O.editingId=null,alert(`✅ Scheda Deconto salvata con successo!`),k()}catch(e){alert(`Errore: ${e.message}`)}});let w=document.getElementById(`btn-close-edit-modal`);w&&w.addEventListener(`click`,()=>{O.editingId=null,k()});let T=document.getElementById(`btn-toggle-add-user`),E=document.getElementById(`add-user-form-container`),D=document.getElementById(`btn-cancel-add-user`),A=document.getElementById(`btn-save-new-user`);T&&E&&T.addEventListener(`click`,()=>{let e=E.style.display===`none`||!E.style.display;E.style.display=e?`block`:`none`}),D&&E&&D.addEventListener(`click`,()=>{E.style.display=`none`}),A&&A.addEventListener(`click`,()=>{let e=document.getElementById(`new-user-username`).value.trim(),t=document.getElementById(`new-user-password`).value.trim(),n=document.getElementById(`new-user-name`).value.trim(),r=document.getElementById(`new-user-role`).value,i=document.getElementById(`new-user-email`).value.trim(),o=document.getElementById(`new-user-phone`).value.trim();if(!e||!t||!n){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{a.addUser({username:e,password:t,name:n,role:r,email:i,phone:o}),alert(`✅ Utente dipendente "${n}" (Codice ${e}) salvato PERMANENTEMENTE nel database!`),k()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{O.editingStaffUserId=e.getAttribute(`data-id`),k()})});let j=document.getElementById(`btn-close-edit-staff-modal`),M=document.getElementById(`btn-cancel-edit-staff`);j&&j.addEventListener(`click`,()=>{O.editingStaffUserId=null,k()}),M&&M.addEventListener(`click`,()=>{O.editingStaffUserId=null,k()});let ee=document.getElementById(`edit-staff-form`);ee&&ee.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,i=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=a.updateUser(t,{username:n,name:r,role:i,email:o,phone:s,password:c?c.trim():void 0});O.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),k()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;a.updateUser(t,{status:n}),k()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{a.deleteUser(t),k()}catch(e){alert(`Errore: ${e.message}`)}})});let te=document.getElementById(`btn-open-email-logs`);te&&te.addEventListener(`click`,()=>{let e=a.getEmailLogs();e.length>0?(O.viewingEmailId=e[0].id,k()):alert(`Nessuna email spedita di recente nel registro.`)});let ne=document.getElementById(`btn-close-email-preview`),re=document.getElementById(`btn-close-email-preview-footer`);ne&&ne.addEventListener(`click`,()=>{O.viewingEmailId=null,k()}),re&&re.addEventListener(`click`,()=>{O.viewingEmailId=null,k()});let ie=document.getElementById(`rename-role-labels-form`);ie&&ie.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();a.updateRoleLabel(`UFFICIO`,t),a.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),k()});let ae=document.getElementById(`permissions-matrix-form`);ae&&ae.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),a.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),k()});let oe=document.getElementById(`setting-logo-file`);oe&&oe.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;a.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),k()},e.readAsDataURL(t)}});let se=document.getElementById(`btn-reset-logo`);se&&se.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(a.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),k())});let ce=document.getElementById(`settings-brand-form`);ce&&ce.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();a.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),k()});let le=document.getElementById(`settings-thresholds-form`);le&&le.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}a.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),k()});let ue=document.getElementById(`settings-brevo-form`);ue&&ue.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();a.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),k()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{O.viewingKpiModal=e.getAttribute(`data-kpi`),k()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{O.viewingKpiModal=null,k()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{O.kpiPeriod=e.getAttribute(`data-period`),k()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{O.kpiChartType=e.getAttribute(`data-charttype`),k()})});let de=document.getElementById(`btn-apply-kpi-custom-date`);de&&de.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(O.kpiCustomStart=e,O.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),k()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let fe=document.getElementById(`btn-dash-search`),N=document.getElementById(`dash-search-input`);fe&&N&&(fe.addEventListener(`click`,()=>{O.dashSearchQuery=N.value,O.dashSearchCategory=document.getElementById(`dash-search-category`).value,k()}),N.addEventListener(`keypress`,e=>{e.key===`Enter`&&(O.dashSearchQuery=N.value,O.dashSearchCategory=document.getElementById(`dash-search-category`).value,k())}));let pe=document.getElementById(`btn-dash-reset`);pe&&pe.addEventListener(`click`,()=>{O.dashSearchQuery=``,O.dashSearchCategory=`ALL`,k()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);O.dashSortColumn===t?O.dashSortDirection=O.dashSortDirection===`ASC`?`DESC`:`ASC`:(O.dashSortColumn=t,O.dashSortDirection=`DESC`),k()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{O.viewingDecontoCode=e.getAttribute(`data-code`),O.viewingKpiModal=null,k()})});let me=document.getElementById(`btn-close-deconto-modal`),he=document.getElementById(`btn-close-deconto-modal-footer`);me&&me.addEventListener(`click`,()=>{O.viewingDecontoCode=null,k()}),he&&he.addEventListener(`click`,()=>{O.viewingDecontoCode=null,k()});let ge=document.getElementById(`btn-export-csv`);ge&&ge.addEventListener(`click`,()=>{let e=a.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_erogazioni_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let P=document.getElementById(`btn-trigger-backup`);P&&P.addEventListener(`click`,async()=>{try{P.disabled=!0,P.innerHTML=`⏳ Backup in Corso...`;let e=a.triggerGitHubBackup();alert(`✅ Backup Cloud completato!\nID: ${e.id}\nCommit: ${e.commitHash}\nRecords: ${e.recordCount}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{P.disabled=!1,P.innerHTML=`☁️ Esegui Backup GitHub`}});let F=document.getElementById(`otp-client-select`),I=document.getElementById(`otp-machine-select`),L=document.getElementById(`otp-board-select`),R=document.getElementById(`otp-credits-input`),_e=document.getElementById(`otp-custom-credits-wrapper`);R&&_e&&R.addEventListener(`change`,()=>{R.value===`CUSTOM`?_e.style.display=`block`:_e.style.display=`none`}),F&&I&&L&&(F.addEventListener(`change`,()=>{let e=F.value;if(!e){I.value=``,L.value=``;return}let t=a.getMachines().find(t=>t.clientId===e);if(t){I.value=t.id;let e=a.getBoards().find(e=>e.machineId===t.id);e?L.value=e.shortCode:L.value=``}else I.value=``,L.value=``}),I.addEventListener(`change`,()=>{let e=I.value;if(!e){F.value=``,L.value=``;return}let t=a.getMachines().find(t=>t.id===e);if(t){F.value=t.clientId||``;let e=a.getBoards().find(e=>e.machineId===t.id);e?L.value=e.shortCode:L.value=``}}),L.addEventListener(`change`,()=>{let e=L.value;if(!e){I.value=``,F.value=``;return}let t=a.getBoards().find(t=>t.shortCode===e);if(t&&t.machineId){I.value=t.machineId;let e=a.getMachines().find(e=>e.id===t.machineId);e?F.value=e.clientId||``:F.value=``}else I.value=``,F.value=``}));let z=document.getElementById(`qr-client-select`),B=document.getElementById(`qr-machine-select`),V=document.getElementById(`qr-board-select`),ve=document.getElementById(`btn-generate-qr-label`),ye=document.getElementById(`btn-print-qr-label`);z&&B&&V&&(z.addEventListener(`change`,()=>{let e=z.value;if(O.qrParams.clientId=e,O.qrParams.isGenerated=!1,!e)O.qrParams.machineId=``,O.qrParams.boardShortCode=``;else{let t=a.getMachines().find(t=>t.clientId===e);if(t){O.qrParams.machineId=t.id;let e=a.getBoards().find(e=>e.machineId===t.id);O.qrParams.boardShortCode=e?e.shortCode:``}else O.qrParams.machineId=``,O.qrParams.boardShortCode=``}k()}),B.addEventListener(`change`,()=>{let e=B.value;if(O.qrParams.machineId=e,O.qrParams.isGenerated=!1,!e)O.qrParams.clientId=``,O.qrParams.boardShortCode=``;else{let t=a.getMachines().find(t=>t.id===e);if(t){O.qrParams.clientId=t.clientId||``;let e=a.getBoards().find(e=>e.machineId===t.id);O.qrParams.boardShortCode=e?e.shortCode:``}}k()}),V.addEventListener(`change`,()=>{let e=V.value;if(O.qrParams.boardShortCode=e,O.qrParams.isGenerated=!1,!e)O.qrParams.machineId=``,O.qrParams.clientId=``;else{let t=a.getBoards().find(t=>t.shortCode===e);if(t&&t.machineId){O.qrParams.machineId=t.machineId;let e=a.getMachines().find(e=>e.id===t.machineId);O.qrParams.clientId=e&&e.clientId||``}else O.qrParams.machineId=``,O.qrParams.clientId=``}k()})),ve&&ve.addEventListener(`click`,()=>{let e=document.getElementById(`qr-client-select`).value,t=document.getElementById(`qr-machine-select`).value,n=document.getElementById(`qr-board-select`).value;if(!e||!t||!n){alert(`Seleziona il Cliente, la Macchina e la Scheda Deconto per procedere!`);return}O.qrParams.clientId=e,O.qrParams.machineId=t,O.qrParams.boardShortCode=n,O.qrParams.isGenerated=!0,k()}),ye&&ye.addEventListener(`click`,()=>{let e=document.getElementById(`printable-qr-label`);if(!e)return;let t=document.createElement(`iframe`);t.style.position=`fixed`,t.style.right=`0`,t.style.bottom=`0`,t.style.width=`0`,t.style.height=`0`,t.style.border=`0`,document.body.appendChild(t);let n=t.contentWindow.document;n.open(),n.write(`
        <html>
          <head>
      ...
      `),n.close()});let be=document.getElementById(`btn-direct-refill-panel`);be&&be.addEventListener(`click`,()=>{let e=document.getElementById(`otp-client-select`).value,t=document.getElementById(`otp-machine-select`).value,n=document.getElementById(`otp-board-select`).value,r=document.getElementById(`otp-credits-input`).value,i=document.getElementById(`otp-custom-credits-value`).value,o=document.getElementById(`otp-expiry-input`).value;if(!e||!n){alert(`Seleziona sia il Cliente che la Scheda Deconto per procedere!`);return}let s=200;if(r===`CUSTOM`){let e=parseInt(i,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per i crediti personalizzati!`);return}s=e}else s=parseInt(r,10);try{a.performRefill({boardShortCode:n,credits:s,method:`CLOUD_DIRECT`,operatorId:O.currentUser?O.currentUser.id:`usr_001`}),alert(`✅ Ricarica diretta di ${s>=0?`+`:``}${s} caffè effettuata con successo sul Deconto #${n}!\nCredito aggiornato via Cloud.`),O.selectedClientTemp=e,O.selectedMachineTemp=t,O.selectedBoardTemp=n,O.selectedCreditsTemp=r,O.selectedCustomCreditsTemp=r===`CUSTOM`?s:``,O.selectedExpiryTemp=o,O.generatedOtpUrl=null,O.generatedOtpToken=null,k();let i=document.getElementById(`otp-client-select`),c=document.getElementById(`otp-machine-select`),l=document.getElementById(`otp-board-select`),u=document.getElementById(`otp-credits-input`),d=document.getElementById(`otp-custom-credits-wrapper`),f=document.getElementById(`otp-custom-credits-value`),p=document.getElementById(`otp-expiry-input`);i&&(i.value=O.selectedClientTemp),c&&(c.value=O.selectedMachineTemp),l&&(l.value=O.selectedBoardTemp),u&&(u.value=O.selectedCreditsTemp),u&&u.value===`CUSTOM`&&d&&(d.style.display=`block`,f&&(f.value=O.selectedCustomCreditsTemp)),p&&(p.value=O.selectedExpiryTemp)}catch(e){alert(`❌ Errore durante la ricarica diretta: ${e.message}`)}});let xe=document.getElementById(`btn-generate-otp-link`);xe&&xe.addEventListener(`click`,()=>{let e=document.getElementById(`otp-client-select`).value,t=document.getElementById(`otp-board-select`).value,n=document.getElementById(`otp-credits-input`).value,r=document.getElementById(`otp-expiry-input`).value;if(!e||!t){alert(`Seleziona sia il Cliente che la Scheda Deconto!`);return}let i=200;if(n===`CUSTOM`){let e=parseInt(document.getElementById(`otp-custom-credits-value`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per i crediti personalizzati!`);return}i=e}else i=parseInt(n,10);let o=a.getClients().find(t=>t.id===e),s=o?o.name:`Cliente`;if(!r){let e=new Date;e.setMonth(e.getMonth()+1),r=e.toISOString().split(`T`)[0]}let c=Math.random().toString(36).substring(2,7).toUpperCase(),l=`OTP-${Math.floor(1e3+Math.random()*9e3)}-${c}`;O.generatedOtpUrl=`${window.location.origin+window.location.pathname}?tab=client_diy&clientName=${encodeURIComponent(s)}&board=${t}&otp=${l}&credits=${i}`,O.generatedOtpToken=l,O.selectedClientTemp=e,O.selectedMachineTemp=document.getElementById(`otp-machine-select`).value,O.selectedBoardTemp=t,O.selectedCreditsTemp=n,O.selectedCustomCreditsTemp=n===`CUSTOM`?i:``,O.selectedExpiryTemp=r,k();let u=document.getElementById(`otp-client-select`),d=document.getElementById(`otp-machine-select`),f=document.getElementById(`otp-board-select`),p=document.getElementById(`otp-credits-input`),m=document.getElementById(`otp-custom-credits-wrapper`),h=document.getElementById(`otp-custom-credits-value`),g=document.getElementById(`otp-expiry-input`);u&&(u.value=O.selectedClientTemp),d&&(d.value=O.selectedMachineTemp),f&&(f.value=O.selectedBoardTemp),p&&(p.value=O.selectedCreditsTemp),p&&p.value===`CUSTOM`&&m&&(m.style.display=`block`,h&&(h.value=O.selectedCustomCreditsTemp)),g&&(g.value=O.selectedExpiryTemp),setTimeout(()=>{let e=document.getElementById(`otp-result-container`);e&&e.scrollIntoView({behavior:`smooth`})},50)});let Se=document.getElementById(`btn-copy-otp-link`);Se&&Se.addEventListener(`click`,()=>{let e=document.getElementById(`generated-otp-url`);e&&(e.select(),document.execCommand(`copy`),alert(`📋 Link di ricarica copiato negli appunti!`))});let Ce=document.getElementById(`btn-whatsapp-otp-send`);if(Ce&&O.generatedOtpUrl){let e=document.getElementById(`otp-board-select`)?.value||`Deconto`,t=`Ciao! Ecco il link per ricaricare +${document.getElementById(`otp-credits-input`)?.value||`200`} caffè sulla tua macchina Deconto #${e}: ${O.generatedOtpUrl}`;Ce.setAttribute(`href`,`https://wa.me/?text=${encodeURIComponent(t)}`)}let we=document.getElementById(`btn-test-otp-link`);we&&O.generatedOtpUrl&&we.addEventListener(`click`,()=>{let e=new URLSearchParams(O.generatedOtpUrl.split(`?`)[1]);O.diyParams={clientName:e.get(`clientName`),boardShortCode:e.get(`board`),tokenOtp:e.get(`otp`),credits:parseInt(e.get(`credits`),10),success:!1},O.activeTab=`client_diy`,k()});let H=document.getElementById(`btn-client-diy-refill`);H&&H.addEventListener(`click`,()=>{let e=H.getAttribute(`data-board`),t=parseInt(H.getAttribute(`data-credits`),10),n=H.getAttribute(`data-otp`),r=document.getElementById(`diy-status-msg`);r&&(r.innerHTML=`<span style="color: var(--accent-cyan); font-weight: 700;">📡 Connessione Bluetooth in corso... Sincronizzazione relè...</span>`),H.disabled=!0,setTimeout(()=>{try{a.performRefill({boardShortCode:e,credits:t,method:`WHATSAPP_OTP_BLE`,tokenOtp:n}),O.diyParams?O.diyParams.success=!0:O.diyParams={success:!0,clientName:`Studio Legale Brambilla`,boardShortCode:e,tokenOtp,credits:t},k()}catch(e){alert(`Errore durante l'accredito: ${e.message}`),r&&(r.innerHTML=`<span style="color: var(--accent-rose); font-weight: 700;">❌ Errore BLE: ${e.message}</span>`),H.disabled=!1}},1200)});let Te=document.getElementById(`btn-back-to-office-from-diy`);Te&&Te.addEventListener(`click`,()=>{O.diyParams=null,O.generatedOtpUrl=null,O.generatedOtpToken=null,O.activeTab=`otp_generator`,k()});let U=document.getElementById(`filter-refill-board`),W=document.getElementById(`filter-refill-client`),G=document.getElementById(`filter-refill-date`),Ee=document.getElementById(`btn-reset-refill-filters`);U&&U.addEventListener(`input`,()=>{O.refillsFilter.boardCode=U.value,k();let e=document.getElementById(`filter-refill-board`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),W&&W.addEventListener(`input`,()=>{O.refillsFilter.clientName=W.value,k();let e=document.getElementById(`filter-refill-client`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),G&&G.addEventListener(`change`,()=>{O.refillsFilter.date=G.value,k()}),Ee&&Ee.addEventListener(`click`,()=>{O.refillsFilter={boardCode:``,clientName:``,date:``},k()});let K=document.getElementById(`filter-extraction-board`),q=document.getElementById(`filter-extraction-client`),J=document.getElementById(`filter-extraction-date`),De=document.getElementById(`btn-reset-extraction-filters`);K&&K.addEventListener(`input`,()=>{O.extractionsFilter.boardCode=K.value,k();let e=document.getElementById(`filter-extraction-board`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),q&&q.addEventListener(`input`,()=>{O.extractionsFilter.clientName=q.value,k();let e=document.getElementById(`filter-extraction-client`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),J&&J.addEventListener(`change`,()=>{O.extractionsFilter.date=J.value,k()}),De&&De.addEventListener(`click`,()=>{O.extractionsFilter={boardCode:``,clientName:``,date:``},k()});let Y=document.getElementById(`modal-refill-credits-select`),X=document.getElementById(`modal-refill-credits-custom`),Oe=document.getElementById(`btn-modal-perform-refill`);Y&&X&&Y.addEventListener(`change`,()=>{Y.value===`CUSTOM`?X.style.display=`block`:X.style.display=`none`}),Oe&&Oe.addEventListener(`click`,()=>{let e=Oe.getAttribute(`data-board-code`),t=document.getElementById(`modal-refill-credits-select`).value,n=200;if(t===`CUSTOM`){let e=parseInt(document.getElementById(`modal-refill-credits-custom`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per la ricarica personalizzata!`);return}n=e}else n=parseInt(t,10);let r=O.currentUser,i=r&&(r.role===`ADMIN`||r.role===`UFFICIO`),o=i?`CLOUD_DIRECT`:`ADR_BLE_PHYSICAL`;try{a.performRefill({boardShortCode:e,credits:n,method:o,operatorId:r?r.id:`usr_001`}),alert(`✅ Ricarica di ${n>=0?`+`:``}${n} caffè effettuata con successo!\nCanale: ${i?`Cloud Wi-Fi (Ufficio)`:`Bluetooth BLE (ADR)`}`),k()}catch(e){alert(`❌ Errore durante la ricarica: ${e.message}`)}});let Z=document.getElementById(`sim-board-select`),Q=document.getElementById(`btn-sim-brew`),$=document.getElementById(`btn-sim-reset`);Z&&Z.addEventListener(`change`,()=>{O.simulatingBoardCode=Z.value,k()}),Q&&Q.addEventListener(`click`,()=>{let e=O.simulatingBoardCode||(a.getBoards()[0]?a.getBoards()[0].shortCode:null);if(!e)return;let t=a.getBoards().find(t=>t.shortCode===e);if(!t||t.remainingCredits<=0)return;Q.disabled=!0,Q.innerText=`☕ EROGAZIONE IN CORSO (230V SENSE IN)...`,$&&($.disabled=!0);let n=document.getElementById(`signal-sense-volts`),r=document.getElementById(`signal-sense-badge`),i=document.getElementById(`sim-console-log`);n&&(n.innerText=`230V AC`),r&&(r.innerText=`⚡ SENSE ATTIVO`,r.className=`badge badge-success`),i&&(i.innerHTML+=`<br>[INPUT]: Rilevata tensione 230V AC su Sense-In (Filo Marrone).<br>[FIRMWARE]: Conteggio tempo di erogazione avviato...`,i.scrollTop=i.scrollHeight),setTimeout(()=>{try{let n=a.registerCoffeeExtraction(e,22,1);n&&n.success===!1?alert(`Impossibile erogare: ${n.reason}`):i&&(i.innerHTML+=`<br>[FIRMWARE]: Erogazione terminata (22s simulati).<br>[MEMORY]: Credito scalato. Nuovo saldo: ${t.remainingCredits} caffè.<br>[HARDWARE]: Relè di blocco: ${t.relayStatus===`CLOSED_OK`?`CONNESSO/ABILITATO`:`APERTO/BLOCCATO`}`)}catch(e){console.error(e)}k()},1500)}),$&&$.addEventListener(`click`,()=>{let e=O.simulatingBoardCode||(a.getBoards()[0]?a.getBoards()[0].shortCode:null);if(e)try{a.performRefill({boardShortCode:e,credits:200,method:`CLOUD_DIRECT`,operatorId:O.currentUser?O.currentUser.id:`usr_001`});let t=document.getElementById(`sim-console-log`);t&&(t.innerHTML+=`<br>[CLOUDLINK]: Comando Ricarica remota +200 ricevuto via Wi-Fi.<br>[MEMORY]: Crediti aggiornati. Relè sbloccato.`),k()}catch(e){alert(`Errore: ${e.message}`)}})}function M(){let e=new URLSearchParams(window.location.search);e.get(`tab`)===`client_diy`&&(O.activeTab=`client_diy`,O.diyParams={clientName:e.get(`clientName`)||`Studio Legale Brambilla`,boardShortCode:e.get(`board`)||`3467`,tokenOtp:e.get(`otp`)||`OTP-9981-X79K2`,credits:parseInt(e.get(`credits`),10)||200,success:!1},window.history.replaceState({},document.title,window.location.pathname))}M(),k();
(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{constructor(){this.brevoApiEndpoint=`https://api.brevo.com/v3/smtp/email`}async sendEmail({to:e,recipientName:t,subject:n,htmlContent:r,plainText:a}){let o=i.getSettings(),s=o.brevoApiKey||``,c=o.brevoSenderEmail||`info@deconto.it`,l=o.brandTitle||`DECONTO`,u={id:`eml_`+Date.now(),to:e,recipientName:t,subject:n,htmlContent:r,plainText:a,timestamp:new Date().toISOString(),status:`SENT`,provider:s?`BREVO_API`:`OUTBOX_MAILTO`};if(i.data.emailLogs||(i.data.emailLogs=[]),i.data.emailLogs.unshift(u),i.saveData(),s)try{let a=await fetch(this.brevoApiEndpoint,{method:`POST`,headers:{accept:`application/json`,"api-key":s,"content-type":`application/json`},body:JSON.stringify({sender:{name:`${l} IoT System`,email:c},to:[{email:e,name:t||e}],subject:n,htmlContent:r})});if(a.ok)return console.log(`✅ [BREVO EMAIL SENT]: Email spedita con successo a ${e}`),u.status=`DELIVERED_BREVO`,i.saveData(),{success:!0,provider:`BREVO`,record:u};{let e=await a.text();console.warn(`⚠️ [BREVO API ERROR]: ${e}`),u.status=`FAILED_BREVO`,i.saveData()}}catch(e){console.error(`❌ [BREVO NETWORK ERROR]: ${e.message}`),u.status=`ERROR_NETWORK`,i.saveData()}return{success:!0,provider:`OUTBOX_INSPECTOR`,record:u}}async sendWelcomeEmail(e){let t=i.getSettings(),n=i.getRoleLabels()[e.role]||e.role,r=t.brandTitle||`DECONTO`,a=`🎉 Benvenuto nel team ${r}! Il tuo account è attivo`,o=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
    ...
    `;return this.sendEmail({to:e.email,recipientName:e.name,subject:a,htmlContent:o,plainText:`Benvenuto ${e.name}! Il tuo username è ${e.username} ed il tuo ruolo è ${n}. Accedi su https://deconto-app.web.app`})}async sendRoleUpdateEmail(e,t,n){let r=i.getSettings(),a=i.getRoleLabels(),o=a[t]||t,s=a[n]||n,c=r.brandTitle||`DECONTO`,l=`🔔 Aggiornamento Ruolo: Benvenuto nel nuovo ruolo di ${s}`,u=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};function a(){let e=i.getSettings();return`
  ...
  `}function o(e,t){let n=i.getSettings(),r=i.getRoleLabels(),a=e.role===`UFFICIO`||e.role===`ADMIN`,o=e.role===`ADR`||e.role===`ADMIN`,s=e.role===`ADMIN`;return`
    <aside class="sidebar">
  ...
  `}function s(e=`dashboard`,t=null,n=``,r=`ALL`,a=`shortCode`,o=`DESC`,s=null,c=`30DAYS`,l=`LINE`,u=`2026-07-01`,d=`2026-08-02`){let f=i.getClients(),p=i.getMachines(),m=i.getBoards(),h=i.getCoffeeLogs(),g=f.length,_=p.length,v=h.length,y=m.filter(e=>{let t=i.calculateBoardStatus(e);return t.statusKey===`WARNING_LOW`||t.statusKey===`CRITICAL_LOW`||t.statusKey===`BLOCKED_ZERO`}),b=m.filter(e=>{if(!n)return!0;let t=n.toLowerCase().trim(),a=i.getBoardFullDetails(e.id),o=a&&a.client?a.client.name.toLowerCase():``,s=a&&a.machine?a.machine.serialNumber.toLowerCase():``,c=a&&a.machine?a.machine.model.toLowerCase():``,l=e.shortCode.toLowerCase();return r===`CODE`?l.includes(t):r===`CLIENT`?o.includes(t):r===`MODEL`?c.includes(t)||s.includes(t):l.includes(t)||o.includes(t)||c.includes(t)||s.includes(t)});b.sort((e,t)=>{let n=i.getBoardFullDetails(e.id),r=i.getBoardFullDetails(t.id),s,c;return a===`shortCode`?(s=parseInt(e.shortCode,10),c=parseInt(t.shortCode,10)):a===`credits`?(s=e.remainingCredits,c=t.remainingCredits):a===`client`?(s=n&&n.client?n.client.name:``,c=r&&r.client?r.client.name:``):a===`model`?(s=n&&n.machine?n.machine.model:``,c=r&&r.machine?r.machine.model:``):a===`connection`?(s=+!!e.isOnlineWifi,c=+!!t.isOnlineWifi):a===`syncDate`?(s=new Date(e.lastSyncDate).getTime(),c=new Date(t.lastSyncDate).getTime()):(s=e.shortCode,c=t.shortCode),s<c?o===`ASC`?-1:1:s>c?o===`ASC`?1:-1:0});let x=e=>a===e?o===`ASC`?`▲`:`▼`:`<span style="opacity: 0.3;">↕</span>`,S=``;if(s===`kpi_clients`)S=`
    ...
    `;else if(s===`kpi_extractions`){let e=i.getExtractionsAnalytics(c,u,d),t=e.chartBuckets,n=Math.max(...t.map(e=>e.count),1),r=``;if(l===`LINE`){let e=t.map((e,t)=>({x:30+t*160,y:150-Math.round(e.count/n*110),count:e.count,label:e.label})),i=`M ${e[0].x},${e[0].y} Q ${e[1].x-40},${e[1].y} ${e[1].x},${e[1].y} T ${e[2].x},${e[2].y} T ${e[3].x},${e[3].y} T ${e[4].x},${e[4].y}`;r=`
    ...
    `}else if(s===`kpi_lowstock`){let e=m.length,t=0,n=0,r=0,a=0;m.forEach(e=>{let o=i.calculateBoardStatus(e);o.statusKey===`ACTIVE_OK`?t++:o.statusKey===`WARNING_LOW`?n++:o.statusKey===`CRITICAL_LOW`?r++:o.statusKey===`BLOCKED_ZERO`&&a++});let o=e>0?(t/e*100).toFixed(1):`0.0`,s=e>0?(n/e*100).toFixed(1):`0.0`,c=e>0?(r/e*100).toFixed(1):`0.0`,l=e>0?(a/e*100).toFixed(1):`0.0`,u=e>0?a/e*360:0,d=u+(e>0?r/e*360:0),f=d+(e>0?n/e*360:0);S=`
      <div class="modal-overlay" id="kpi-modal">
        <div class="modal-box" style="max-width: 1240px; width: 96%; max-height: 90vh; overflow-y: auto;">
              ...
              <table style="width: 100%; border-collapse: collapse; font-size: 0.88rem;">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style="padding: 8px 6px; text-align: center;"><strong>${a}</strong></td>
            ...
            <div style="background: rgba(0,0,0,0.25); padding: 20px; border-radius: 14px; border: 1px solid var(--border-subtle);">
              ...
              <div class="table-container" style="max-height: 480px; overflow-y: auto;">
                <table style="width: 100%;">
                  ...
                  <tbody>
                    ${y.length>0?y.map(e=>{let t=i.getBoardFullDetails(e.id),n=t&&t.client?t.client.name:`N/D`,r=i.calculateBoardStatus(e);return`
                        <tr>
    ...
    `}let C=``;if(t){let e=i.getBoardFullDetails(t);if(e&&e.board){let t=e.board,n=e.machine||{},r=e.client||{},a=e.coffees||[],o=t.avgDailyCoffees||12.4,s=o>0?Math.ceil(t.remainingCredits/o):`N/D`,c=s===`N/D`?`N/D`:new Date(Date.now()+s*864e5).toLocaleDateString(`it-IT`,{day:`2-digit`,month:`long`,year:`numeric`}),l=i.calculateBoardStatus(t);C=`
            ...
            <div style="display: grid; grid-template-columns: 1.25fr 1fr; gap: 24px; align-items: start;">
              ...
              <div style="background: rgba(0,0,0,0.25); padding: 18px; border-radius: 14px; border: 1px solid var(--border-subtle);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px;">
                  ...
                  <span class="badge badge-info" style="font-size: 0.75rem;">${a.length} Erogazioni</span>
                </div>
      ...
      <div style="margin-top: 24px;">
        ...
        <div class="table-container">
          <table>
            ...
            <tbody>
              ${b.length>0?b.map(e=>{let t=i.getBoardFullDetails(e.id),n=t&&t.client?t.client.name:`N/D`,r=t&&t.machine?t.machine.model:`N/D`,a=t&&t.machine?t.machine.serialNumber:`N/D`,o=i.calculateBoardStatus(e);return`
                  <tr>
                    <td>
    ...
    ${C}
    ${S}
  `}function c(e=`clients`,t=null){let n=i.getClients(),r=i.getMachines(),a=i.getBoards();i.getRefillLogs();let o=``;if(e===`clients`&&t){let e=n.find(e=>e.id===t);if(e){let t=r.find(t=>t.clientId===e.id);o=`
            ...
            <form id="form-edit-client">
              ...
              <div style="margin-bottom: 20px; background: rgba(56, 189, 248, 0.1); padding: 14px; border-radius: 8px; border: 1px solid var(--accent-cyan);">
                ...
                <select id="edit-cli-machine" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px; font-weight: 700;">
                  ...
                  ${r.map(n=>{let r=a.find(e=>e.machineId===n.id),i=n.clientId&&n.clientId!==e.id;return`<option value="${n.id}" ${t&&t.id===n.id?`selected`:``}>☕ ${n.serialNumber} - ${n.model} ${r?`(Deconto #${r.shortCode})`:``} ${i?`(⚠️ Già in uso)`:``}</option>`}).join(``)}
                </select>
              </div>
      ...
      `}}if(e===`machines`&&t){let e=r.find(e=>e.id===t);if(e){let t=a.find(t=>t.machineId===e.id);o=`
            ...
            <form id="form-edit-machine">
              ...
              <div style="margin-bottom: 16px; background: rgba(168, 85, 247, 0.1); padding: 14px; border-radius: 8px; border: 1px solid var(--accent-purple);">
                ...
                <select id="edit-mc-board" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px; font-weight: 700;">
                  ...
                  ${a.map(n=>`<option value="${n.id}" ${t&&t.id===n.id?`selected`:``}>📟 Deconto #${n.shortCode} (${n.remainingCredits} cr) ${n.machineId&&n.machineId!==e.id?`(⚠️ Montata altrove)`:``}</option>`).join(``)}
                </select>
              </div>
      ...
      <div>
        ...
        <div id="add-client-form-container" class="stat-card" style="display: none; margin-bottom: 32px; padding: 24px; border: 2px solid var(--accent-cyan);">
          ...
          <div style="margin-bottom: 20px; background: rgba(56, 189, 248, 0.1); padding: 14px; border-radius: 8px; border: 1px solid var(--accent-cyan);">
            ...
            <select id="new-cli-machine" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px; font-weight: 700;">
              ...
              ${r.map(e=>{let t=a.find(t=>t.machineId===e.id);return`<option value="${e.id}">☕ ${e.serialNumber} - ${e.model} ${t?`(Deconto #${t.shortCode})`:`(Senza Deconto)`} ${e.clientId?`(⚠️ Già installata altrove)`:``}</option>`}).join(``)}
            </select>
          </div>
        ...
        <div class="table-container">
          <table>
            ...
            <tbody>
              ${n.map(e=>{let t=r.filter(t=>t.clientId===e.id),n=i.calculateClientStatus(e);return`
                  <tr>
      ...
      <div>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px;">
            <div style="background: rgba(168, 85, 247, 0.1); padding: 14px; border-radius: 8px; border: 1px solid var(--accent-purple);">
              ...
              <select id="new-mc-board" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px; font-weight: 700;">
                ...
                ${a.map(e=>`<option value="${e.id}">📟 Deconto #${e.shortCode} (${e.remainingCredits} cr) ${e.machineId?`(⚠️ Montata su altra macchina)`:``}</option>`).join(``)}
              </select>
            </div>
        ...
        <div class="table-container">
          <table>
            ...
            <tbody>
              ${r.map(e=>{let t=a.find(t=>t.machineId===e.id),r=n.find(t=>t.id===e.clientId),o=t?i.calculateBoardStatus(t):{badgeHtml:`<span class="badge badge-secondary">📦 SCORTA</span>`};return`
                  <tr>
    ...
    <div>
      ...
      <div class="table-container">
        <table>
          ...
          <tbody>
            ${a.map(e=>{let t=r.find(t=>t.id===e.machineId),a=t?n.find(e=>e.id===t.clientId):null,o=i.calculateBoardStatus(e);return`
                <tr>
                  <td>
                  ...
                  <td>
                    ${a?`<strong style="color: #fff;">🏢 ${a.name}</strong><br><small style="color: var(--text-muted);">${a.city}</small>`:`<span class="badge badge-secondary">📦 LIBERA</span>`}
                  </td>
                  <td>${o.badgeHtml}</td>
                  <td>
                    <div style="display: flex; gap: 6px;">
  ...
  `}function d(e=null){let t=i.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return i.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function f(e,t=null,n=null){let r=i.getUsers(),a=i.getRoleLabels(),o=i.getPermissions(),s=i.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
  ...
  `}function m(){let e=i.getSettings();return`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var h={currentUser:i.getCurrentUser(),activeTab:`dashboard`,editingId:null,editingStaffUserId:null,showProfileModal:!1,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingDecontoCode:null,viewingEmailId:null,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`};function g(){let e=document.getElementById(`app`);if(!h.currentUser){e.innerHTML=a(),_();return}let t=``;switch(h.activeTab){case`dashboard`:t=s(h.activeTab,h.viewingDecontoCode,h.dashSearchQuery,h.dashSearchCategory,h.dashSortColumn,h.dashSortDirection,h.viewingKpiModal,h.kpiPeriod,h.kpiChartType,h.kpiCustomStart,h.kpiCustomEnd);break;case`clients`:case`machines`:case`deconto_boards`:t=c(h.activeTab,h.editingId);break;case`adr_visits`:t=l();break;case`client_diy`:t=u();break;case`simulator`:t=d();break;case`user_mgmt`:t=f(h.editingStaffUserId,h.viewingEmailId);break;case`settings`:t=m();break;default:t=s()}let n=p(h.showProfileModal,h.currentUser);e.innerHTML=`
    ...
    ${n}
  `,v()}function _(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value.trim();try{let e=i.login(t,n);h.currentUser=e,h.activeTab=e.role===`ADR`?`adr_visits`:`dashboard`,g()}catch(e){alert(e.message)}})}function v(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(h.activeTab=t,h.editingId=null,h.editingStaffUserId=null,h.viewingDecontoCode=null,h.viewingEmailId=null,g())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{h.showProfileModal=!0,g()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{h.showProfileModal=!1,g()}),n&&n.addEventListener(`click`,()=>{h.showProfileModal=!1,g()});let r=document.getElementById(`user-profile-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-phone`).value.trim(),a=document.getElementById(`profile-avatar`).value,o=document.getElementById(`profile-current-password`).value,s=document.getElementById(`profile-new-password`).value,c=document.getElementById(`profile-confirm-password`).value;if(!i.verifyPassword(h.currentUser.id,o)){alert(`Password attuale non corretta!`);return}if(s||c){if(s!==c){alert(`Le nuove password inserite non coincidono!`);return}if(s.length<4){alert(`La nuova password deve contenere almeno 4 caratteri!`);return}}try{h.currentUser=i.updateUserProfile(h.currentUser.id,{name:t,email:n,phone:r,avatar:a,newPassword:s?s.trim():void 0}),h.showProfileModal=!1,alert(`✅ Profilo utente aggiornato con successo!`),g()}catch(e){alert(`Errore: ${e.message}`)}});let a=document.getElementById(`btn-logout`);a&&a.addEventListener(`click`,()=>{i.logout(),h.currentUser=null,h.activeTab=`dashboard`,g()});let o=document.getElementById(`btn-toggle-add-client`),s=document.getElementById(`add-client-form-container`),c=document.getElementById(`btn-cancel-add-client`),l=document.getElementById(`btn-save-new-client`);o&&s&&o.addEventListener(`click`,()=>{let e=s.style.display===`none`||!s.style.display;s.style.display=e?`block`:`none`}),c&&s&&c.addEventListener(`click`,()=>{s.style.display=`none`}),l&&l.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-ref`).value.trim(),n=document.getElementById(`new-cli-phone`).value.trim(),r=document.getElementById(`new-cli-email`).value.trim(),a=document.getElementById(`new-cli-city`).value.trim(),o=document.getElementById(`new-cli-machine`).value;if(!e){alert(`Inserisci la Ragione Sociale / Nome Cliente!`);return}try{let s=i.addClient({name:e,refPerson:t,phone:n,email:r,city:a,machineId:o});alert(`✅ Cliente "${s.name}" registrato con successo!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingId=e.getAttribute(`data-id`),g()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo cliente?`))try{i.deleteClient(t),alert(`✅ Cliente eliminato dall'anagrafica!`),g()}catch(e){alert(`Errore: ${e.message}`)}})});let u=document.getElementById(`btn-cancel-edit-client`);u&&u.addEventListener(`click`,()=>{h.editingId=null,g()});let d=document.getElementById(`form-edit-client`);d&&d.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value.trim(),r=document.getElementById(`edit-cli-ref`).value.trim(),a=document.getElementById(`edit-cli-phone`).value.trim(),o=document.getElementById(`edit-cli-city`).value.trim(),s=document.getElementById(`edit-cli-address`).value.trim(),c=document.getElementById(`edit-cli-machine`).value;try{i.updateClient(t,{name:n,refPerson:r,phone:a,city:o,address:s,assignedMachineId:c}),h.editingId=null,alert(`✅ Scheda Cliente salvata con successo!`),g()}catch(e){alert(`Errore: ${e.message}`)}});let f=document.getElementById(`btn-toggle-add-machine`),p=document.getElementById(`add-machine-form-container`),m=document.getElementById(`btn-cancel-add-machine`),_=document.getElementById(`btn-save-new-machine`);f&&p&&f.addEventListener(`click`,()=>{let e=p.style.display===`none`||!p.style.display;p.style.display=e?`block`:`none`}),m&&p&&m.addEventListener(`click`,()=>{p.style.display=`none`}),_&&_.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-board`).value,a=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Inserisci Seriale Macchina e Modello!`);return}try{let o=i.addMachine({serialNumber:e,brand:t,model:n,boardId:r,clientId:a});alert(`✅ Macchina da caffè SN "${o.serialNumber}" registrata con successo!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingId=e.getAttribute(`data-id`),g()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa macchina da caffè?`))try{i.deleteMachine(t),alert(`✅ Macchina eliminata dal parco macchine!`),g()}catch(e){alert(`Errore: ${e.message}`)}})});let v=document.getElementById(`btn-cancel-edit-mc`);v&&v.addEventListener(`click`,()=>{h.editingId=null,g()});let y=document.getElementById(`form-edit-machine`);y&&y.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value.trim(),r=document.getElementById(`edit-mc-brand`).value.trim(),a=document.getElementById(`edit-mc-model`).value.trim(),o=document.getElementById(`edit-mc-board`).value,s=document.getElementById(`edit-mc-client`).value;try{i.updateMachine(t,{serialNumber:n,brand:r,model:a,boardId:o,clientId:s}),h.editingId=null,alert(`✅ Scheda Macchina salvata con successo!`),g()}catch(e){alert(`Errore: ${e.message}`)}});let b=document.getElementById(`btn-toggle-add-board`),x=document.getElementById(`add-board-form-container`),S=document.getElementById(`btn-cancel-add-board`),C=document.getElementById(`btn-save-new-board`);b&&x&&b.addEventListener(`click`,()=>{let e=x.style.display===`none`||!x.style.display;x.style.display=e?`block`:`none`}),S&&x&&S.addEventListener(`click`,()=>{x.style.display=`none`}),C&&C.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,a=document.getElementById(`new-board-machine`).value;if(!e){alert(`Inserisci il Codice a 4 cifre del Deconto!`);return}try{let o=i.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,machineId:a});alert(`✅ Scheda Deconto #${o.shortCode} salvata PERMANENTEMENTE nel database!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingId=e.getAttribute(`data-id`),g()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa scheda Deconto?`))try{i.deleteBoard(t),alert(`✅ Scheda Deconto eliminata!`),g()}catch(e){alert(`Errore: ${e.message}`)}})});let w=document.getElementById(`btn-cancel-edit-board`);w&&w.addEventListener(`click`,()=>{h.editingId=null,g()});let T=document.getElementById(`form-edit-board`);T&&T.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value.trim(),r=document.getElementById(`edit-board-hwserial`).value.trim(),a=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-version`).value,s=document.getElementById(`edit-board-machine`).value;try{i.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:a,version:o,machineId:s}),h.editingId=null,alert(`✅ Scheda Deconto salvata con successo!`),g()}catch(e){alert(`Errore: ${e.message}`)}});let E=document.getElementById(`btn-close-edit-modal`);E&&E.addEventListener(`click`,()=>{h.editingId=null,g()});let D=document.getElementById(`btn-open-add-staff-modal`),O=document.getElementById(`btn-close-add-staff-modal`),k=document.getElementById(`btn-cancel-add-staff`),A=document.getElementById(`add-staff-modal`);D&&A&&D.addEventListener(`click`,()=>{A.style.display=`flex`}),O&&A&&O.addEventListener(`click`,()=>{A.style.display=`none`}),k&&A&&k.addEventListener(`click`,()=>{A.style.display=`none`});let j=document.getElementById(`add-staff-form`);j&&j.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`new-user-username`).value.trim(),n=document.getElementById(`new-user-password`).value.trim(),r=document.getElementById(`new-user-name`).value.trim(),a=document.getElementById(`new-user-role`).value,o=document.getElementById(`new-user-email`).value.trim(),s=document.getElementById(`new-user-phone`).value.trim();if(!t||!n||!r){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{i.addUser({username:t,password:n,name:r,role:a,email:o,phone:s}),alert(`✅ Utente dipendente "${r}" (Codice ${t}) salvato PERMANENTEMENTE nel database!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingStaffUserId=e.getAttribute(`data-id`),g()})});let M=document.getElementById(`btn-close-edit-staff-modal`),N=document.getElementById(`btn-cancel-edit-staff`);M&&M.addEventListener(`click`,()=>{h.editingStaffUserId=null,g()}),N&&N.addEventListener(`click`,()=>{h.editingStaffUserId=null,g()});let P=document.getElementById(`edit-staff-form`);P&&P.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,a=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=i.updateUser(t,{username:n,name:r,role:a,email:o,phone:s,password:c?c.trim():void 0});h.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;i.updateUser(t,{status:n}),g()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{i.deleteUser(t),g()}catch(e){alert(`Errore: ${e.message}`)}})});let F=document.getElementById(`btn-open-email-logs`);F&&F.addEventListener(`click`,()=>{let e=i.getEmailLogs();e.length>0?(h.viewingEmailId=e[0].id,g()):alert(`Nessuna email spedita di recente nel registro.`)});let I=document.getElementById(`btn-close-email-preview`),L=document.getElementById(`btn-close-email-preview-footer`);I&&I.addEventListener(`click`,()=>{h.viewingEmailId=null,g()}),L&&L.addEventListener(`click`,()=>{h.viewingEmailId=null,g()});let R=document.getElementById(`rename-role-labels-form`);R&&R.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();i.updateRoleLabel(`UFFICIO`,t),i.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),g()});let z=document.getElementById(`permissions-matrix-form`);z&&z.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),i.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),g()});let B=document.getElementById(`setting-logo-file`);B&&B.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;i.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),g()},e.readAsDataURL(t)}});let V=document.getElementById(`btn-reset-logo`);V&&V.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(i.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),g())});let H=document.getElementById(`settings-brand-form`);H&&H.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();i.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),g()});let U=document.getElementById(`settings-thresholds-form`);U&&U.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}i.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),g()});let W=document.getElementById(`settings-brevo-form`);W&&W.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();i.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),g()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingKpiModal=e.getAttribute(`data-kpi`),g()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingKpiModal=null,g()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{h.kpiPeriod=e.getAttribute(`data-period`),g()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{h.kpiChartType=e.getAttribute(`data-charttype`),g()})});let G=document.getElementById(`btn-apply-kpi-custom-date`);G&&G.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(h.kpiCustomStart=e,h.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),g()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let K=document.getElementById(`btn-dash-search`),q=document.getElementById(`dash-search-input`);K&&q&&(K.addEventListener(`click`,()=>{h.dashSearchQuery=q.value,h.dashSearchCategory=document.getElementById(`dash-search-category`).value,g()}),q.addEventListener(`keypress`,e=>{e.key===`Enter`&&(h.dashSearchQuery=q.value,h.dashSearchCategory=document.getElementById(`dash-search-category`).value,g())}));let J=document.getElementById(`btn-dash-reset`);J&&J.addEventListener(`click`,()=>{h.dashSearchQuery=``,h.dashSearchCategory=`ALL`,g()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);h.dashSortColumn===t?h.dashSortDirection=h.dashSortDirection===`ASC`?`DESC`:`ASC`:(h.dashSortColumn=t,h.dashSortDirection=`DESC`),g()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingDecontoCode=e.getAttribute(`data-code`),g()})});let Y=document.getElementById(`btn-close-deconto-modal`),X=document.getElementById(`btn-close-deconto-modal-footer`);Y&&Y.addEventListener(`click`,()=>{h.viewingDecontoCode=null,g()}),X&&X.addEventListener(`click`,()=>{h.viewingDecontoCode=null,g()});let Z=document.getElementById(`btn-export-csv`);Z&&Z.addEventListener(`click`,()=>{let e=i.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_erogazioni_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let Q=document.getElementById(`btn-trigger-backup`);Q&&Q.addEventListener(`click`,async()=>{try{Q.disabled=!0,Q.innerHTML=`⏳ Backup in Corso...`;let e=i.triggerGitHubBackup();alert(`✅ Backup Cloud completato!\nID: ${e.id}\nCommit: ${e.commitHash}\nRecords: ${e.recordCount}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{Q.disabled=!1,Q.innerHTML=`☁️ Esegui Backup GitHub`}})}g();
...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};function o(){let e=a.getSettings();return`
  ...
  `}function S(e=null){let t=a.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return a.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function C(e,t=null,n=null){let r=a.getUsers(),i=a.getRoleLabels(),o=a.getPermissions(),s=a.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
  ...
  `}function T(){let e=a.getSettings();return`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var E={currentUser:a.getCurrentUser(),activeTab:`dashboard`,editingId:null,editingStaffUserId:null,showProfileModal:!1,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingDecontoCode:null,viewingEmailId:null,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`,generatedOtpUrl:null,generatedOtpToken:null,diyParams:null,refillsFilter:{boardCode:``,clientName:``,date:``},simulatingBoardCode:null};function D(){let e=document.getElementById(`app`);if(!E.currentUser){e.innerHTML=o(),O();return}let t=``;switch(E.activeTab){case`dashboard`:t=c(E.activeTab,E.viewingDecontoCode,E.dashSearchQuery,E.dashSearchCategory,E.dashSortColumn,E.dashSortDirection,E.viewingKpiModal,E.kpiPeriod,E.kpiChartType,E.kpiCustomStart,E.kpiCustomEnd);break;case`clients`:case`machines`:case`deconto_boards`:t=_(E.activeTab,E.editingId);break;case`adr_visits`:t=v();break;case`otp_generator`:t=b(E.generatedOtpUrl,E.generatedOtpToken);break;case`client_diy`:t=y(E.diyParams);break;case`qr_generator`:t=`
      ...
      `;break;case`refills_history`:t=x(E.refillsFilter);break;case`simulator`:t=S(E.simulatingBoardCode);break;case`user_mgmt`:case`user_management`:case`permissions_matrix`:t=C(E.activeTab,E.editingStaffUserId,E.viewingEmailId);break;case`settings`:t=T();break;default:t=c()}let n=w(E.showProfileModal,E.currentUser);e.innerHTML=`
    <div class="app-layout">
      ${s(E.currentUser,E.activeTab)}
      <main class="main-content">
        ${t}
      </main>
    </div>
    ${n}
  `,k()}function O(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value.trim();try{let e=a.login(t,n);E.currentUser=e,E.activeTab=e.role===`ADR`?`adr_visits`:`dashboard`,D()}catch(e){alert(e.message)}})}function k(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(E.activeTab=t,E.editingId=null,E.editingStaffUserId=null,E.viewingDecontoCode=null,E.viewingEmailId=null,D())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{E.showProfileModal=!0,D()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{E.showProfileModal=!1,D()}),n&&n.addEventListener(`click`,()=>{E.showProfileModal=!1,D()});let r=document.getElementById(`profile-edit-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-new-password`).value;try{E.currentUser=a.updateUserProfile(E.currentUser.id,{name:t,email:n,newPassword:r?r.trim():void 0}),E.showProfileModal=!1,alert(`✅ Credenziali del profilo aggiornate con successo!`),D()}catch(e){alert(`Errore: ${e.message}`)}});let i=document.getElementById(`btn-logout`);i&&i.addEventListener(`click`,()=>{a.logout(),E.currentUser=null,E.activeTab=`dashboard`,D()});let o=document.getElementById(`btn-toggle-add-client`),s=document.getElementById(`add-client-form-container`),c=document.getElementById(`btn-cancel-add-client`),l=document.getElementById(`btn-save-new-client`);o&&s&&o.addEventListener(`click`,()=>{let e=s.style.display===`none`||!s.style.display;s.style.display=e?`block`:`none`}),c&&s&&c.addEventListener(`click`,()=>{s.style.display=`none`}),l&&l.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-type`).value,n=document.getElementById(`new-cli-ref`).value.trim(),r=document.getElementById(`new-cli-phone`).value.trim(),i=document.getElementById(`new-cli-email`).value.trim(),o=document.getElementById(`new-cli-city`).value.trim(),s=document.getElementById(`new-cli-machine`).value;if(!e){alert(`Inserisci la Ragione Sociale / Nome Cliente!`);return}try{let c=a.addClient({name:e,clientType:t,refPerson:n,phone:r,email:i,city:o,machineId:s});alert(`✅ Cliente "${c.name}" registrato con successo!`),D()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{E.editingId=e.getAttribute(`data-id`),D()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo cliente?`))try{a.deleteClient(t),alert(`✅ Cliente eliminato dall'anagrafica!`),D()}catch(e){alert(`Errore: ${e.message}`)}})});let u=document.getElementById(`btn-cancel-edit-client`);u&&u.addEventListener(`click`,()=>{E.editingId=null,D()});let d=document.getElementById(`form-edit-client`);d&&d.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value.trim(),r=document.getElementById(`edit-cli-type`).value,i=document.getElementById(`edit-cli-ref`).value.trim(),o=document.getElementById(`edit-cli-phone`).value.trim(),s=document.getElementById(`edit-cli-city`).value.trim(),c=document.getElementById(`edit-cli-address`).value.trim(),l=document.getElementById(`edit-cli-machine`).value;try{a.updateClient(t,{name:n,clientType:r,refPerson:i,phone:o,city:s,address:c,assignedMachineId:l}),E.editingId=null,alert(`✅ Scheda Cliente salvata con successo!`),D()}catch(e){alert(`Errore: ${e.message}`)}});let f=document.getElementById(`btn-toggle-add-machine`),p=document.getElementById(`add-machine-form-container`),m=document.getElementById(`btn-cancel-add-machine`),h=document.getElementById(`btn-save-new-machine`);f&&p&&f.addEventListener(`click`,()=>{let e=p.style.display===`none`||!p.style.display;p.style.display=e?`block`:`none`}),m&&p&&m.addEventListener(`click`,()=>{p.style.display=`none`}),h&&h.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-year`).value.trim(),i=document.getElementById(`new-mc-board`).value,o=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Inserisci Seriale Macchina e Modello!`);return}try{let s=a.addMachine({serialNumber:e,brand:t,model:n,productionYear:r,boardId:i,clientId:o});alert(`✅ Macchina da caffè SN "${s.serialNumber}" registrata con successo!`),D()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{E.editingId=e.getAttribute(`data-id`),D()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa macchina da caffè?`))try{a.deleteMachine(t),alert(`✅ Macchina eliminata dal parco macchine!`),D()}catch(e){alert(`Errore: ${e.message}`)}})});let g=document.getElementById(`btn-cancel-edit-mc`);g&&g.addEventListener(`click`,()=>{E.editingId=null,D()});let _=document.getElementById(`form-edit-machine`);_&&_.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value.trim(),r=document.getElementById(`edit-mc-brand`).value.trim(),i=document.getElementById(`edit-mc-model`).value.trim(),o=document.getElementById(`edit-mc-year`).value.trim(),s=document.getElementById(`edit-mc-board`).value,c=document.getElementById(`edit-mc-client`).value;try{a.updateMachine(t,{serialNumber:n,brand:r,model:i,productionYear:o,boardId:s,clientId:c}),E.editingId=null,alert(`✅ Scheda Macchina salvata con successo!`),D()}catch(e){alert(`Errore: ${e.message}`)}});let v=document.getElementById(`btn-toggle-add-board`),y=document.getElementById(`add-board-form-container`),b=document.getElementById(`btn-cancel-add-board`),x=document.getElementById(`btn-save-new-board`);v&&y&&v.addEventListener(`click`,()=>{let e=y.style.display===`none`||!y.style.display;y.style.display=e?`block`:`none`}),b&&y&&b.addEventListener(`click`,()=>{y.style.display=`none`}),x&&x.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,i=document.getElementById(`new-board-machine`).value,o=document.getElementById(`new-board-groupcount`),s=o?parseInt(o.value,10):1;if(!e){alert(`Inserisci il Codice a 4 cifre del Deconto!`);return}try{let o=a.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,groupCount:s,machineId:i});alert(`✅ Scheda Deconto #${o.shortCode} salvata PERMANENTEMENTE nel database!`),D()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{E.editingId=e.getAttribute(`data-id`),D()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa scheda Deconto?`))try{a.deleteBoard(t),alert(`✅ Scheda Deconto eliminata!`),D()}catch(e){alert(`Errore: ${e.message}`)}})});let S=document.getElementById(`btn-cancel-edit-board`);S&&S.addEventListener(`click`,()=>{E.editingId=null,D()});let C=document.getElementById(`form-edit-board`);C&&C.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value.trim(),r=document.getElementById(`edit-board-hwserial`).value.trim(),i=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-version`).value,s=document.getElementById(`edit-board-machine`).value,c=document.getElementById(`edit-board-groupcount`),l=c?parseInt(c.value,10):1;try{a.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:i,version:o,groupCount:l,machineId:s}),E.editingId=null,alert(`✅ Scheda Deconto salvata con successo!`),D()}catch(e){alert(`Errore: ${e.message}`)}});let w=document.getElementById(`btn-close-edit-modal`);w&&w.addEventListener(`click`,()=>{E.editingId=null,D()});let T=document.getElementById(`btn-toggle-add-user`),O=document.getElementById(`add-user-form-container`),k=document.getElementById(`btn-cancel-add-user`),A=document.getElementById(`btn-save-new-user`);T&&O&&T.addEventListener(`click`,()=>{let e=O.style.display===`none`||!O.style.display;O.style.display=e?`block`:`none`}),k&&O&&k.addEventListener(`click`,()=>{O.style.display=`none`}),A&&A.addEventListener(`click`,()=>{let e=document.getElementById(`new-user-username`).value.trim(),t=document.getElementById(`new-user-password`).value.trim(),n=document.getElementById(`new-user-name`).value.trim(),r=document.getElementById(`new-user-role`).value,i=document.getElementById(`new-user-email`).value.trim(),o=document.getElementById(`new-user-phone`).value.trim();if(!e||!t||!n){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{a.addUser({username:e,password:t,name:n,role:r,email:i,phone:o}),alert(`✅ Utente dipendente "${n}" (Codice ${e}) salvato PERMANENTEMENTE nel database!`),D()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{E.editingStaffUserId=e.getAttribute(`data-id`),D()})});let ee=document.getElementById(`btn-close-edit-staff-modal`),te=document.getElementById(`btn-cancel-edit-staff`);ee&&ee.addEventListener(`click`,()=>{E.editingStaffUserId=null,D()}),te&&te.addEventListener(`click`,()=>{E.editingStaffUserId=null,D()});let ne=document.getElementById(`edit-staff-form`);ne&&ne.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,i=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=a.updateUser(t,{username:n,name:r,role:i,email:o,phone:s,password:c?c.trim():void 0});E.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),D()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;a.updateUser(t,{status:n}),D()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{a.deleteUser(t),D()}catch(e){alert(`Errore: ${e.message}`)}})});let re=document.getElementById(`btn-open-email-logs`);re&&re.addEventListener(`click`,()=>{let e=a.getEmailLogs();e.length>0?(E.viewingEmailId=e[0].id,D()):alert(`Nessuna email spedita di recente nel registro.`)});let ie=document.getElementById(`btn-close-email-preview`),j=document.getElementById(`btn-close-email-preview-footer`);ie&&ie.addEventListener(`click`,()=>{E.viewingEmailId=null,D()}),j&&j.addEventListener(`click`,()=>{E.viewingEmailId=null,D()});let M=document.getElementById(`rename-role-labels-form`);M&&M.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();a.updateRoleLabel(`UFFICIO`,t),a.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),D()});let ae=document.getElementById(`permissions-matrix-form`);ae&&ae.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),a.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),D()});let N=document.getElementById(`setting-logo-file`);N&&N.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;a.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),D()},e.readAsDataURL(t)}});let P=document.getElementById(`btn-reset-logo`);P&&P.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(a.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),D())});let oe=document.getElementById(`settings-brand-form`);oe&&oe.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();a.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),D()});let se=document.getElementById(`settings-thresholds-form`);se&&se.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}a.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),D()});let ce=document.getElementById(`settings-brevo-form`);ce&&ce.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();a.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),D()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{E.viewingKpiModal=e.getAttribute(`data-kpi`),D()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{E.viewingKpiModal=null,D()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{E.kpiPeriod=e.getAttribute(`data-period`),D()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{E.kpiChartType=e.getAttribute(`data-charttype`),D()})});let le=document.getElementById(`btn-apply-kpi-custom-date`);le&&le.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(E.kpiCustomStart=e,E.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),D()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let ue=document.getElementById(`btn-dash-search`),F=document.getElementById(`dash-search-input`);ue&&F&&(ue.addEventListener(`click`,()=>{E.dashSearchQuery=F.value,E.dashSearchCategory=document.getElementById(`dash-search-category`).value,D()}),F.addEventListener(`keypress`,e=>{e.key===`Enter`&&(E.dashSearchQuery=F.value,E.dashSearchCategory=document.getElementById(`dash-search-category`).value,D())}));let de=document.getElementById(`btn-dash-reset`);de&&de.addEventListener(`click`,()=>{E.dashSearchQuery=``,E.dashSearchCategory=`ALL`,D()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);E.dashSortColumn===t?E.dashSortDirection=E.dashSortDirection===`ASC`?`DESC`:`ASC`:(E.dashSortColumn=t,E.dashSortDirection=`DESC`),D()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{E.viewingDecontoCode=e.getAttribute(`data-code`),E.viewingKpiModal=null,D()})});let fe=document.getElementById(`btn-close-deconto-modal`),I=document.getElementById(`btn-close-deconto-modal-footer`);fe&&fe.addEventListener(`click`,()=>{E.viewingDecontoCode=null,D()}),I&&I.addEventListener(`click`,()=>{E.viewingDecontoCode=null,D()});let L=document.getElementById(`btn-export-csv`);L&&L.addEventListener(`click`,()=>{let e=a.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_erogazioni_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let R=document.getElementById(`btn-trigger-backup`);R&&R.addEventListener(`click`,async()=>{try{R.disabled=!0,R.innerHTML=`⏳ Backup in Corso...`;let e=a.triggerGitHubBackup();alert(`✅ Backup Cloud completato!\nID: ${e.id}\nCommit: ${e.commitHash}\nRecords: ${e.recordCount}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{R.disabled=!1,R.innerHTML=`☁️ Esegui Backup GitHub`}});let z=document.getElementById(`otp-client-select`),B=document.getElementById(`otp-machine-select`),V=document.getElementById(`otp-board-select`),H=document.getElementById(`otp-credits-input`),U=document.getElementById(`otp-custom-credits-wrapper`);H&&U&&H.addEventListener(`change`,()=>{H.value===`CUSTOM`?U.style.display=`block`:U.style.display=`none`}),z&&B&&V&&(z.addEventListener(`change`,()=>{let e=z.value;if(!e){B.value=``,V.value=``;return}let t=a.getMachines().find(t=>t.clientId===e);if(t){B.value=t.id;let e=a.getBoards().find(e=>e.machineId===t.id);e?V.value=e.shortCode:V.value=``}else B.value=``,V.value=``}),B.addEventListener(`change`,()=>{let e=B.value;if(!e){z.value=``,V.value=``;return}let t=a.getMachines().find(t=>t.id===e);if(t){z.value=t.clientId||``;let e=a.getBoards().find(e=>e.machineId===t.id);e?V.value=e.shortCode:V.value=``}}),V.addEventListener(`change`,()=>{let e=V.value;if(!e){B.value=``,z.value=``;return}let t=a.getBoards().find(t=>t.shortCode===e);if(t&&t.machineId){B.value=t.machineId;let e=a.getMachines().find(e=>e.id===t.machineId);e?z.value=e.clientId||``:z.value=``}else B.value=``,z.value=``}));let pe=document.getElementById(`btn-generate-otp-link`);pe&&pe.addEventListener(`click`,()=>{let e=document.getElementById(`otp-client-select`).value,t=document.getElementById(`otp-board-select`).value,n=document.getElementById(`otp-credits-input`).value,r=document.getElementById(`otp-expiry-input`).value;if(!e||!t){alert(`Seleziona sia il Cliente che la Scheda Deconto!`);return}let i=200;if(n===`CUSTOM`){let e=parseInt(document.getElementById(`otp-custom-credits-value`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per i crediti personalizzati!`);return}i=e}else i=parseInt(n,10);let o=a.getClients().find(t=>t.id===e),s=o?o.name:`Cliente`;if(!r){let e=new Date;e.setMonth(e.getMonth()+1),r=e.toISOString().split(`T`)[0]}let c=Math.random().toString(36).substring(2,7).toUpperCase(),l=`OTP-${Math.floor(1e3+Math.random()*9e3)}-${c}`;E.generatedOtpUrl=`${window.location.origin+window.location.pathname}?tab=client_diy&clientName=${encodeURIComponent(s)}&board=${t}&otp=${l}&credits=${i}`,E.generatedOtpToken=l,E.selectedClientTemp=e,E.selectedMachineTemp=document.getElementById(`otp-machine-select`).value,E.selectedBoardTemp=t,E.selectedCreditsTemp=n,E.selectedCustomCreditsTemp=n===`CUSTOM`?i:``,E.selectedExpiryTemp=r,D();let u=document.getElementById(`otp-client-select`),d=document.getElementById(`otp-machine-select`),f=document.getElementById(`otp-board-select`),p=document.getElementById(`otp-credits-input`),m=document.getElementById(`otp-custom-credits-wrapper`),h=document.getElementById(`otp-custom-credits-value`),g=document.getElementById(`otp-expiry-input`);u&&(u.value=E.selectedClientTemp),d&&(d.value=E.selectedMachineTemp),f&&(f.value=E.selectedBoardTemp),p&&(p.value=E.selectedCreditsTemp),p&&p.value===`CUSTOM`&&m&&(m.style.display=`block`,h&&(h.value=E.selectedCustomCreditsTemp)),g&&(g.value=E.selectedExpiryTemp),setTimeout(()=>{let e=document.getElementById(`otp-result-container`);e&&e.scrollIntoView({behavior:`smooth`})},50)});let me=document.getElementById(`btn-copy-otp-link`);me&&me.addEventListener(`click`,()=>{let e=document.getElementById(`generated-otp-url`);e&&(e.select(),document.execCommand(`copy`),alert(`📋 Link di ricarica copiato negli appunti!`))});let he=document.getElementById(`btn-whatsapp-otp-send`);if(he&&E.generatedOtpUrl){let e=document.getElementById(`otp-board-select`)?.value||`Deconto`,t=`Ciao! Ecco il link per ricaricare +${document.getElementById(`otp-credits-input`)?.value||`200`} caffè sulla tua macchina Deconto #${e}: ${E.generatedOtpUrl}`;he.setAttribute(`href`,`https://wa.me/?text=${encodeURIComponent(t)}`)}let ge=document.getElementById(`btn-test-otp-link`);ge&&E.generatedOtpUrl&&ge.addEventListener(`click`,()=>{let e=new URLSearchParams(E.generatedOtpUrl.split(`?`)[1]);E.diyParams={clientName:e.get(`clientName`),boardShortCode:e.get(`board`),tokenOtp:e.get(`otp`),credits:parseInt(e.get(`credits`),10),success:!1},E.activeTab=`client_diy`,D()});let W=document.getElementById(`btn-client-diy-refill`);W&&W.addEventListener(`click`,()=>{let e=W.getAttribute(`data-board`),t=parseInt(W.getAttribute(`data-credits`),10),n=W.getAttribute(`data-otp`),r=document.getElementById(`diy-status-msg`);r&&(r.innerHTML=`<span style="color: var(--accent-cyan); font-weight: 700;">📡 Connessione Bluetooth in corso... Sincronizzazione relè...</span>`),W.disabled=!0,setTimeout(()=>{try{a.performRefill({boardShortCode:e,credits:t,method:`WHATSAPP_OTP_BLE`,tokenOtp:n}),E.diyParams?E.diyParams.success=!0:E.diyParams={success:!0,clientName:`Studio Legale Brambilla`,boardShortCode:e,tokenOtp,credits:t},D()}catch(e){alert(`Errore durante l'accredito: ${e.message}`),r&&(r.innerHTML=`<span style="color: var(--accent-rose); font-weight: 700;">❌ Errore BLE: ${e.message}</span>`),W.disabled=!1}},1200)});let _e=document.getElementById(`btn-back-to-office-from-diy`);_e&&_e.addEventListener(`click`,()=>{E.diyParams=null,E.generatedOtpUrl=null,E.generatedOtpToken=null,E.activeTab=`otp_generator`,D()});let G=document.getElementById(`filter-refill-board`),K=document.getElementById(`filter-refill-client`),q=document.getElementById(`filter-refill-date`),ve=document.getElementById(`btn-reset-refill-filters`);G&&G.addEventListener(`input`,()=>{E.refillsFilter.boardCode=G.value,D();let e=document.getElementById(`filter-refill-board`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),K&&K.addEventListener(`input`,()=>{E.refillsFilter.clientName=K.value,D();let e=document.getElementById(`filter-refill-client`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),q&&q.addEventListener(`change`,()=>{E.refillsFilter.date=q.value,D()}),ve&&ve.addEventListener(`click`,()=>{E.refillsFilter={boardCode:``,clientName:``,date:``},D()});let J=document.getElementById(`modal-refill-credits-select`),Y=document.getElementById(`modal-refill-credits-custom`),X=document.getElementById(`btn-modal-perform-refill`);J&&Y&&J.addEventListener(`change`,()=>{J.value===`CUSTOM`?Y.style.display=`block`:Y.style.display=`none`}),X&&X.addEventListener(`click`,()=>{let e=X.getAttribute(`data-board-code`),t=document.getElementById(`modal-refill-credits-select`).value,n=200;if(t===`CUSTOM`){let e=parseInt(document.getElementById(`modal-refill-credits-custom`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per la ricarica personalizzata!`);return}n=e}else n=parseInt(t,10);let r=E.currentUser,i=r&&(r.role===`ADMIN`||r.role===`UFFICIO`),o=i?`CLOUD_DIRECT`:`ADR_BLE_PHYSICAL`;try{a.performRefill({boardShortCode:e,credits:n,method:o,operatorId:r?r.id:`usr_001`}),alert(`✅ Ricarica di ${n>=0?`+`:``}${n} caffè effettuata con successo!\nCanale: ${i?`Cloud Wi-Fi (Ufficio)`:`Bluetooth BLE (ADR)`}`),D()}catch(e){alert(`❌ Errore durante la ricarica: ${e.message}`)}});let Z=document.getElementById(`sim-board-select`),Q=document.getElementById(`btn-sim-brew`),$=document.getElementById(`btn-sim-reset`);Z&&Z.addEventListener(`change`,()=>{E.simulatingBoardCode=Z.value,D()}),Q&&Q.addEventListener(`click`,()=>{let e=E.simulatingBoardCode||(a.getBoards()[0]?a.getBoards()[0].shortCode:null);if(!e)return;let t=a.getBoards().find(t=>t.shortCode===e);if(!t||t.remainingCredits<=0)return;Q.disabled=!0,Q.innerText=`☕ EROGAZIONE IN CORSO (230V SENSE IN)...`,$&&($.disabled=!0);let n=document.getElementById(`signal-sense-volts`),r=document.getElementById(`signal-sense-badge`),i=document.getElementById(`sim-console-log`);n&&(n.innerText=`230V AC`),r&&(r.innerText=`⚡ SENSE ATTIVO`,r.className=`badge badge-success`),i&&(i.innerHTML+=`<br>[INPUT]: Rilevata tensione 230V AC su Sense-In (Filo Marrone).<br>[FIRMWARE]: Conteggio tempo di erogazione avviato...`,i.scrollTop=i.scrollHeight),setTimeout(()=>{try{let n=a.registerCoffeeExtraction(e,22,1);n&&n.success===!1?alert(`Impossibile erogare: ${n.reason}`):i&&(i.innerHTML+=`<br>[FIRMWARE]: Erogazione terminata (22s simulati).<br>[MEMORY]: Credito scalato. Nuovo saldo: ${t.remainingCredits} caffè.<br>[HARDWARE]: Relè di blocco: ${t.relayStatus===`CLOSED_OK`?`CONNESSO/ABILITATO`:`APERTO/BLOCCATO`}`)}catch(e){console.error(e)}D()},1500)}),$&&$.addEventListener(`click`,()=>{let e=E.simulatingBoardCode||(a.getBoards()[0]?a.getBoards()[0].shortCode:null);if(e)try{a.performRefill({boardShortCode:e,credits:200,method:`CLOUD_DIRECT`,operatorId:E.currentUser?E.currentUser.id:`usr_001`});let t=document.getElementById(`sim-console-log`);t&&(t.innerHTML+=`<br>[CLOUDLINK]: Comando Ricarica remota +200 ricevuto via Wi-Fi.<br>[MEMORY]: Crediti aggiornati. Relè sbloccato.`),D()}catch(e){alert(`Errore: ${e.message}`)}})}function A(){let e=new URLSearchParams(window.location.search);e.get(`tab`)===`client_diy`&&(E.activeTab=`client_diy`,E.diyParams={clientName:e.get(`clientName`)||`Studio Legale Brambilla`,boardShortCode:e.get(`board`)||`3467`,tokenOtp:e.get(`otp`)||`OTP-9981-X79K2`,credits:parseInt(e.get(`credits`),10)||200,success:!1},window.history.replaceState({},document.title,window.location.pathname))}A(),D();
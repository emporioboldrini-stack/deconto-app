...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};function o(){let e=a.getSettings();return`
  ...
  `}function s(e,t){let n=a.getSettings(),r=a.getRoleLabels(),i=e.role===`UFFICIO`||e.role===`ADMIN`;e.role===`ADR`||e.role;let o=e.role===`ADMIN`;return`
    <aside class="sidebar">
      ...
      <nav class="sidebar-nav">
        ...
        ${i?`
          ...
          <a class="nav-item ${t===`extractions_history`?`active`:``}" data-tab="extractions_history">
            <span class="nav-icon">☕</span>
        ...
        <div style="margin-top: 24px; padding: 14px 12px; border-top: 1px solid var(--border-subtle); text-align: center; background: rgba(0,0,0,0.25); border-radius: 8px;">
          ...
          <div style="font-size: 0.72rem; color: var(--text-muted); margin-top: 4px;">
            Data ultima modifica: 05/08/2026
          </div>
        </div>
  ...
  `}function c(e=`dashboard`,t=null,n=``,r=`ALL`,i=`shortCode`,o=`DESC`,s=null,c=`30DAYS`,l=`LINE`,u=`2026-07-01`,d=`2026-08-02`){let f=a.getClients(),p=a.getMachines(),m=a.getBoards(),h=a.getCoffeeLogs(),g=f.length,_=p.length,v=h.length,y=m.filter(e=>{let t=a.calculateBoardStatus(e);return t.statusKey===`WARNING_LOW`||t.statusKey===`CRITICAL_LOW`||t.statusKey===`BLOCKED_ZERO`}),b=m.filter(e=>{if(!n)return!0;let t=n.toLowerCase().trim(),i=a.getBoardFullDetails(e.id),o=i&&i.client?i.client.name.toLowerCase():``,s=i&&i.machine?i.machine.serialNumber.toLowerCase():``,c=i&&i.machine?i.machine.model.toLowerCase():``,l=e.shortCode.toLowerCase();return r===`CODE`?l.includes(t):r===`CLIENT`?o.includes(t):r===`MODEL`?c.includes(t)||s.includes(t):l.includes(t)||o.includes(t)||c.includes(t)||s.includes(t)});b.sort((e,t)=>{let n=a.getBoardFullDetails(e.id),r=a.getBoardFullDetails(t.id),s,c;return i===`shortCode`?(s=parseInt(e.shortCode,10),c=parseInt(t.shortCode,10)):i===`credits`?(s=e.remainingCredits,c=t.remainingCredits):i===`client`?(s=n&&n.client?n.client.name:``,c=r&&r.client?r.client.name:``):i===`model`?(s=n&&n.machine?n.machine.model:``,c=r&&r.machine?r.machine.model:``):i===`connection`?(s=+!!e.isOnlineWifi,c=+!!t.isOnlineWifi):i===`syncDate`?(s=new Date(e.lastSyncDate).getTime(),c=new Date(t.lastSyncDate).getTime()):(s=e.shortCode,c=t.shortCode),s<c?o===`ASC`?-1:1:s>c?o===`ASC`?1:-1:0});let x=e=>i===e?o===`ASC`?`▲`:`▼`:`<span style="opacity: 0.3;">↕</span>`,S=``;if(s===`kpi_clients`)S=`
        ...
        <div class="modal-box" style="max-width: 1240px; width: 96%; min-height: 80vh; max-height: 90vh; overflow-y: auto;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 1px solid var(--border-subtle); padding-bottom: 12px;">
            <h2 style="font-size: 1.4rem; font-weight: 800; color: var(--accent-purple); margin: 0;">
              ☕ Telemetria & Analytics Parco Macchine & Deconti (${_} macchine)
            </h2>
            <button class="btn-close-kpi-modal" style="background: none; border: none; color: var(--text-muted); font-size: 1.6rem; cursor: pointer;">&times;</button>
          </div>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
            ...
            <div style="background: rgba(0,0,0,0.3); padding: 16px; border-radius: 12px; border: 1px solid var(--border-subtle);">
              ...
              ${(()=>{let e={};p.forEach(t=>{let n=t.model||`Altro`;e[n]=(e[n]||0)+1});let t=Object.entries(e).sort((e,t)=>t[1]-e[1]),n=t.length>0?t[0][1]:1;return t.length===0?`<div style="color:var(--text-muted);font-size:0.85rem;">Nessuna macchina registrata.</div>`:t.map(([e,t])=>{let r=Math.round(t/_*100),i=Math.round(t/n*100);return`
                    <div style="margin-bottom: 12px;">
                      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; font-size: 0.85rem;">
                        <span style="font-weight: 700; color: #fff;">☕ ${e}</span>
                        <span style="color: var(--accent-cyan); font-weight: 800;">${t} macchin${t===1?`a`:`e`} &nbsp;<small style="color:var(--text-muted);">(${r}%)</small></span>
                      </div>
                      <div style="background: rgba(255,255,255,0.06); border-radius: 4px; height: 6px; width: 100%;">
                        <div style="background: var(--accent-cyan); height: 6px; border-radius: 4px; width: ${i}%; transition: width 0.4s;"></div>
                      </div>
                    </div>
                  `}).join(``)})()}
            </div>
              ...
              ${(()=>{let e={};p.forEach(t=>{let n=t.productionYear||`2026`;e[n]=(e[n]||0)+1});let t=Object.entries(e).sort((e,t)=>t[1]-e[1]),n=t.length>0?t[0][1]:1;return t.length===0?`<div style="color:var(--text-muted);font-size:0.85rem;">Nessuna macchina registrata.</div>`:t.map(([e,t])=>{let r=Math.round(t/_*100),i=Math.round(t/n*100);return`
                    <div style="margin-bottom: 12px;">
                      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; font-size: 0.85rem;">
                        ...
                        <span style="color: var(--accent-amber); font-weight: 800;">${t} macchin${t===1?`a`:`e`} &nbsp;<small style="color:var(--text-muted);">(${r}%)</small></span>
                      </div>
                      <div style="background: rgba(255,255,255,0.06); border-radius: 4px; height: 6px; width: 100%;">
                        <div style="background: var(--accent-amber); height: 6px; border-radius: 4px; width: ${i}%; transition: width 0.4s;"></div>
                      </div>
                    </div>
                  `}).join(``)})()}
            </div>
    ...
    ${S}
  `}function l(e){if(!e)return``;let t=a.getBoardFullDetails(e);if(!t||!t.board)return``;let n=t.board,r=t.machine||{},i=t.client||{},o=t.coffees||[],s=t.refills||[],c=[];s.forEach(e=>{c.push({id:e.id,timestamp:new Date(e.timestamp),type:`REFILL`,variation:e.creditsAdded,details:e.method===`CLOUD_DIRECT`?`Ricarica Cloud (Ufficio)`:e.method===`WHATSAPP_OTP_BLE`?`Ricarica OTP (Cliente)`:`Ricarica ADR (BLE)`})}),o.forEach(e=>{c.push({id:e.id,timestamp:new Date(e.timestamp),type:`EXTRACTION`,variation:-1,details:`Gruppo #${e.groupId||1} (${e.durationSeconds||22}s)`})}),c.sort((e,t)=>t.timestamp.getTime()-e.timestamp.getTime());let l=n.remainingCredits;c.forEach(e=>{e.runningBalance=l,l-=e.variation});let u=0;if(r.installDate&&n.machineExtractions>0){let e=new Date(r.installDate).getTime(),t=Math.max(1,Math.ceil((Date.now()-e)/864e5));u=Number((n.machineExtractions/t).toFixed(1))}let d=u>0?u:`N/D`,f=u>0?Math.ceil(n.remainingCredits/u):`N/D`,p=f===`N/D`?`N/D`:new Date(Date.now()+f*864e5).toLocaleDateString(`it-IT`,{day:`2-digit`,month:`long`,year:`numeric`}),m=a.calculateBoardStatus(n);return`
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; border-bottom: 1px solid var(--border-subtle); padding-bottom: 14px;">
          <div>
            <div style="display: flex; align-items: center; gap: 12px;">
              <span style="font-size: 2.2rem; font-weight: 900; color: var(--accent-cyan); font-family: monospace;">#${n.shortCode}</span>
              <span class="badge ${n.isOnlineWifi?`badge-success`:`badge-warning`}">
                ${n.isOnlineWifi?`📡 Wi-Fi Online (-62 dBm)`:`📶 Bluetooth Local Only`}
              </span>
              <span class="badge badge-info">${n.version} VERSION</span>
              ${m.badgeHtml}
            </div>
            <h2 style="font-size: 1.3rem; font-weight: 800; color: #fff; margin: 4px 0 0 0;">
              ${i.name?i.name:`Cliente Non Assegnato`}
            </h2>
            <div style="font-size: 0.85rem; color: var(--text-muted);">
              Macchina: <strong>${r.model||`N/D`}</strong> | Seriale: <code>${r.serialNumber||`N/D`}</code>
            </div>
          </div>
          ...
          <div style="background: rgba(0,0,0,0.25); padding: 18px; border-radius: 14px; border: 1px solid var(--border-subtle);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px;">
              <h3 style="font-size: 1.15rem; font-weight: 800; margin: 0; color: var(--accent-cyan); display: flex; align-items: center; gap: 8px;">
                📊 Movimenti & Saldo Deconto (#${n.shortCode})
              </h3>
              <span class="badge badge-info" style="font-size: 0.75rem;">${c.length} Movimenti Totali</span>
            </div>
                    ...
                    ${c.length>0?c.map(e=>{let t=e.type===`REFILL`,n=e.variation>=0,r=``;return r=t?`<span class="badge" style="font-size: 0.8rem; font-weight: bold; background: ${n?`rgba(34, 197, 94, 0.1)`:`rgba(239, 68, 68, 0.1)`}; color: ${n?`var(--accent-green)`:`var(--accent-rose)`}; border: 1px solid ${n?`rgba(34, 197, 94, 0.25)`:`rgba(239, 68, 68, 0.25)`};">${n?`+`:``}${e.variation} cr</span>`:`<span class="badge" style="font-size: 0.8rem; font-weight: bold; background: rgba(239, 68, 68, 0.1); color: var(--accent-rose); border: 1px solid rgba(239, 68, 68, 0.25);">${e.variation} cr</span>`,`
                      ...
                      <tr>
                        <td colspan="5" style="text-align: center; color: var(--text-muted); padding: 40px;">
                          Nessun movimento registrato (ricariche o erogazioni) per la scheda #${n.shortCode}.
                        </td>
                      </tr>
                    `}
                  </tbody>
          ...
          <div style="display: flex; flex-direction: column; gap: 16px;">
            ...
            <div style="background: linear-gradient(135deg, rgba(56, 189, 248, 0.1), rgba(168, 85, 247, 0.1)); padding: 18px; border-radius: 14px; border: 1px solid var(--accent-cyan);">
              ...
              <div style="display: flex; justify-content: space-between; align-items: baseline; margin-top: 6px;">
                <div style="font-size: 2.5rem; font-weight: 900; color: ${n.remainingCredits<=0?`var(--accent-rose)`:`var(--accent-green)`};">
                  ${n.remainingCredits} <span style="font-size: 1.1rem; font-weight: 600; color: #fff;">cialde</span>
                </div>
                <div style="text-align: right;">
                  <div style="font-size: 0.75rem; color: var(--text-muted);">Stima Esaurimento:</div>
                  <div style="font-size: 0.9rem; font-weight: 800; color: var(--accent-amber);">${p}</div>
                </div>
              </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
              <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 10px; border: 1px solid var(--border-subtle);">
                ...
                <div style="font-size: 1.2rem; font-weight: 800; color: #fff; margin-top: 2px;">${(n.machineExtractions||0).toLocaleString(`it-IT`)} ☕</div>
              </div>

              <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 10px; border: 1px solid var(--border-subtle);">
                <div style="font-size: 0.75rem; color: var(--text-muted);">Storico Hardware:</div>
                <div style="font-size: 1.2rem; font-weight: 800; color: var(--accent-cyan); margin-top: 2px;">${(n.lifetimeExtractions||0).toLocaleString(`it-IT`)} ☕</div>
              </div>

              <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 10px; border: 1px solid var(--border-subtle);">
                <div style="font-size: 0.75rem; color: var(--text-muted);">Media Consumo:</div>
                <div style="font-size: 1.2rem; font-weight: 800; color: var(--accent-green); margin-top: 2px;">${d} <small style="font-size: 0.75rem;">caffè/gg</small></div>
              </div>

              <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 10px; border: 1px solid var(--border-subtle);">
                <div style="font-size: 0.75rem; color: var(--text-muted);">Stato Relè Hardware:</div>
                <div style="font-size: 0.85rem; font-weight: 800; color: ${n.relayStatus===`CLOSED_OK`?`var(--accent-green)`:`var(--accent-rose)`}; margin-top: 4px;">
                  ${n.relayStatus===`CLOSED_OK`?`🟢 CHIUSO (ABILITATO)`:`🔴 APERTO (BLOCCATO)`}
                </div>
              </div>
            </div>
            ...
            <div style="background: rgba(16, 185, 129, 0.05); padding: 16px; border-radius: 12px; border: 1px solid rgba(16, 185, 129, 0.2); display: flex; flex-direction: column; gap: 10px; margin-bottom: 4px;">
              ...
              <button id="btn-modal-perform-refill" data-board-code="${n.shortCode}" class="btn btn-primary" style="padding: 10px; font-size: 0.85rem; font-weight: 800; width: 100%; border-radius: 6px; background: linear-gradient(135deg, var(--accent-green), #059669);">
                ${(()=>{let e=a.getCurrentUser();return e&&(e.role===`ADMIN`||e.role===`UFFICIO`)?`🚀 Esegui Ricarica Cloud (Wi-Fi)`:`📡 Esegui Ricarica Bluetooth (BLE)`})()}
              </button>
            </div>
            ...
            <div style="background: rgba(0,0,0,0.3); padding: 14px; border-radius: 10px; border: 1px solid var(--border-subtle); font-size: 0.8rem; line-height: 1.6;">
              ...
              <div>• <strong>Seriale Scheda:</strong> <code>${n.hwSerial||`N/D`}</code></div>
              <div>• <strong>Indirizzo MAC:</strong> <code>${n.macAddress||`N/D`}</code></div>
          ...
          <div class="modal-box" style="max-width: 520px; width: 95%;">
            ...
            <form id="form-edit-client">
              ...
              <div style="margin-bottom: 16px;">
                ...
                ${p(`edit-cli-type`,e.clientType||`Altro`)}
              </div>
            ...
            <tbody>
              ${r.map(e=>{let t=i.find(t=>t.machineId===e.id),r=n.find(t=>t.id===e.clientId),o=t?a.calculateBoardStatus(t):{badgeHtml:`<span class="badge badge-secondary">📦 SCORTA</span>`};return`
                  <tr>
                    ...
                    <td><span class="badge" style="background: rgba(255,255,255,0.06); color: #fff; border: 1px solid var(--border-subtle);">${e.productionYear||`2026`}</span></td>
                    <td>
  ...
  `}function b(e=null){let t=e?e.clientName:`Studio Legale Brambilla`,n=e?e.boardShortCode:`3467`,r=e?e.tokenOtp:`OTP-9981-X79K2`,i=e?parseInt(e.credits,10):200,a=e?!!e.success:!1;return`
    <div style="max-width: 500px; margin: 0 auto; text-align: center;">
      
      <!-- Card Simulata Pagina WhatsApp Link -->
      <div style="background: linear-gradient(135deg, #111827, #1f2937); border: 2px solid ${a?`var(--accent-green)`:`var(--accent-cyan)`}; border-radius: 24px; padding: 36px 24px; box-shadow: var(--shadow-card); position: relative;">
        
        <div style="width: 70px; height: 70px; background: linear-gradient(135deg, ${a?`var(--accent-green)`:`var(--accent-cyan)`}, var(--accent-purple)); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 2.2rem; margin: 0 auto 20px auto; box-shadow: var(--shadow-glow);">
          ${a?`✅`:`☕`}
        </div>

        <span class="badge ${a?`badge-success`:`badge-info`}" style="margin-bottom: 12px; padding: 6px 14px;">
          ${a?`RICARICA COMPLETATA`:`RICARICA FAI-DA-TE DECONTO`}
        </span>
        
        <h2 style="font-size: 1.6rem; font-weight: 800; color: #fff; margin-bottom: 8px;">
          ${a?`Crediti Accreditati!`:`Ricarica la tua Macchina da Caffè`}
        </h2>
        <p style="font-size: 0.9rem; color: var(--text-muted); margin-bottom: 24px;">
          ${a?`La macchina da caffè è stata ricaricata con successo. Buona pausa caffè!`:`Spedizione Cialde Ricevuta! Avvicinati alla macchina e tocca il pulsante sottostante.`}
        </p>
        ...
        <div style="background: rgba(0,0,0,0.4); border-radius: 16px; padding: 20px; border: 1px solid var(--border-subtle); margin-bottom: 24px; text-align: left;">
          ...
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
            ...
            <code style="color: var(--accent-amber);">${r}</code>
          </div>
          <div style="display: flex; justify-content: space-between; border-top: 1px solid var(--border-subtle); padding-top: 8px; margin-top: 8px;">
            ...
            <strong style="color: ${i>=0?`var(--accent-green)`:`var(--accent-rose)`}; font-size: 1.2rem;">${i>=0?`+`:``}${i} CAFFÈ</strong>
          </div>
        </div>
        ...
        `:`
          ...
          <button id="btn-client-diy-refill" 
            ...
            data-credits="${i}" 
            data-otp="${r}" 
            class="btn btn-primary" style="width: 100%; padding: 18px; font-size: 1.2rem; font-weight: 800; border-radius: 14px; background: linear-gradient(135deg, ${i>=0?`var(--accent-green)`:`var(--accent-rose)`}, ${i>=0?`#059669`:`#b91c1c`}); box-shadow: 0 8px 20px ${i>=0?`rgba(52, 211, 153, 0.4)`:`rgba(239, 68, 68, 0.4)`};">
            ✨ ${i>=0?`ACCREDITA`:`DECREMENTA`} ${Math.abs(i)} CAFFÈ ORA
          </button>
  ...
  `}function x(e=null,t=null){let n=a.getClients(),r=a.getBoards(),i=a.getMachines(),o=new Date;o.setMonth(o.getMonth()+1);let s=o.toISOString().split(`T`)[0],c=o.toLocaleDateString(`it-IT`,{day:`2-digit`,month:`2-digit`,year:`numeric`});return`
    <div style="max-width: 800px; margin: 0 auto;">
      ...
      <div class="stat-card" style="padding: 28px; border: 1px solid var(--border-subtle); margin-bottom: 32px; background: rgba(0,0,0,0.2);">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 20px;">
          ...
          <div>
            ...
            <select id="otp-machine-select" style="width: 100%; padding: 12px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 8px; font-weight: 700;">
              ...
              ${i.map(e=>{let t=n.find(t=>t.id===e.clientId);return`<option value="${e.id}">${e.serialNumber} - ${e.model} ${t?`(${t.name})`:`(Libera)`}</option>`}).join(``)}
            </select>
          </div>
          ...
          <div>
            ...
            <select id="otp-board-select" style="width: 100%; padding: 12px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 8px; font-weight: 700;">
              ...
              ${r.map(e=>{let t=i.find(t=>t.id===e.machineId),r=t?n.find(e=>e.id===t.clientId):null;return`<option value="${e.shortCode}">Deconto #${e.shortCode} (${e.remainingCredits} cr) ${r?`[${r.name}]`:``}</option>`}).join(``)}
            </select>
          </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
          ...
          <div>
            ...
            <input type="date" id="otp-expiry-input" value="${s}" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 8px; font-weight: 700; margin-bottom: 8px;">
            <div style="font-size: 0.78rem; color: var(--accent-green); font-weight: 700; display: flex; align-items: center; gap: 4px;">
              <span>📅</span> Scadenza consigliata: <strong>${c}</strong> (1 mese)
            </div>
          </div>
        </div>
      ...
      <div id="otp-result-container" class="stat-card" style="padding: 28px; border: 2px solid var(--accent-cyan); display: ${e?`block`:`none`}; background: rgba(56, 189, 248, 0.05);">
        ...
        <div style="margin-bottom: 24px;">
          ...
          <textarea id="generated-otp-url" readonly style="width: 100%; height: 80px; padding: 12px; background: rgba(0,0,0,0.4); border: 1px solid var(--border-color); border-radius: 8px; color: var(--accent-cyan); font-family: monospace; font-size: 0.85rem; resize: none; margin-bottom: 12px;">${e||``}</textarea>
  ...
  `}function S(e={}){let t=a.getClients(),n=a.getBoards(),r=a.getMachines(),i=e.clientId||``,o=e.machineId||``,s=e.boardShortCode||``,c=t.find(e=>e.id===i),l=r.find(e=>e.id===o),u=c?c.name:``,d=l?l.serialNumber:``,f=``,p=``;return e.isGenerated&&s&&(p=`${window.location.origin}/?tab=client_diy&board=${s}&clientName=${encodeURIComponent(u||`Cliente`)}`,f=`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(p)}&ecc=M&margin=1`),`
    <div style="max-width: 800px; margin: 0 auto;">
      <div style="margin-bottom: 24px;">
      ...
      <div class="stat-card" style="padding: 28px; border: 1px solid var(--border-subtle); margin-bottom: 32px; background: rgba(0,0,0,0.2);">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 24px;">
          <div>
            ...
            <select id="qr-client-select" style="width: 100%; padding: 12px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 8px; font-weight: 700;">
              <option value="">-- Scegli Cliente --</option>
              ${t.map(e=>`<option value="${e.id}" ${e.id===i?`selected`:``}>${e.name} (${e.city})</option>`).join(``)}
            </select>
          </div>
          ...
          <div>
            ...
            <select id="qr-machine-select" style="width: 100%; padding: 12px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 8px; font-weight: 700;">
              <option value="">-- Scegli Macchina --</option>
              ${r.map(e=>{let n=t.find(t=>t.id===e.clientId);return`<option value="${e.id}" ${e.id===o?`selected`:``}>${e.serialNumber} - ${e.model} ${n?`(${n.name})`:`(Libera)`}</option>`}).join(``)}
            </select>
          </div>
          ...
          <div>
            ...
            <select id="qr-board-select" style="width: 100%; padding: 12px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 8px; font-weight: 700;">
              <option value="">-- Scegli Deconto --</option>
              ${n.map(e=>{let n=r.find(t=>t.id===e.machineId),i=n?t.find(e=>e.id===n.clientId):null;return`<option value="${e.shortCode}" ${e.shortCode===s?`selected`:``}>Deconto #${e.shortCode} (${e.remainingCredits} cr) ${i?`[${i.name}]`:``}</option>`}).join(``)}
            </select>
          </div>
        </div>
        ...
        <div style="display: flex; gap: 12px; justify-content: flex-end; border-top: 1px solid var(--border-subtle); padding-top: 20px;">
          ...
          <button id="btn-print-qr-label" class="btn btn-secondary" ${e.isGenerated?``:`disabled`} style="padding: 12px 24px; font-size: 0.9rem; font-weight: 800; border-radius: 8px; color: ${e.isGenerated?`var(--accent-cyan)`:`var(--text-muted)`}; border-color: ${e.isGenerated?`var(--accent-cyan)`:`var(--border-color)`};">
            🖨️ STAMPA ETICHETTA
          </button>
        </div>
      </div>
      ...
      <div class="stat-card" style="padding: 20px; border: 1px solid var(--border-subtle); margin-bottom: 24px; background: rgba(0,0,0,0.2);">
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 16px; align-items: flex-end;">
          <div>
            ...
            <input type="text" id="filter-refill-board" value="${e.boardCode||``}" placeholder="Es. 3467" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px; font-family: monospace;">
          </div>
          ...
          <div>
            ...
            <input type="text" id="filter-refill-client" value="${e.clientName||``}" placeholder="Es. Brambilla" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px;">
          </div>

          <div>
            <label style="font-size: 0.8rem; color: var(--text-muted); display: block; margin-bottom: 6px; font-weight: 700;">📅 Filtra per Data:</label>
            <input type="date" id="filter-refill-date" value="${e.date||``}" style="width: 100%; padding: 8px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px;">
          </div>
      ...
      <div class="table-container">
        <table>
          ...
          <tbody>
            ...
            `:u.map(e=>{let t=r.find(t=>t.id===e.boardId||t.shortCode===e.shortCode),a=t&&t.machineId?i.find(e=>e.id===t.machineId):null,s=a&&a.clientId?n.find(e=>e.id===a.clientId):null,c=`N/D`;if(e.operatorType===`CLIENT_DIY`)c=`👤 Cliente (Fai-da-Te)`;else{let t=o.find(t=>t.id===e.operatorId||t.username===e.operatorId);if(t){let e=t.role===`ADMIN`?`Amministratore`:t.role===`UFFICIO`?`Ufficio`:`Agente ADR`;c=`👩‍💻 ${t.name} (${e})`}else c=`Codice: ${e.operatorId}`}let l=new Date(e.timestamp),u=l.toLocaleDateString(`it-IT`,{day:`2-digit`,month:`2-digit`,year:`numeric`})+` `+l.toLocaleTimeString(`it-IT`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`}),d=e.creditsAdded>=0,f=d?`background: rgba(34, 197, 94, 0.1); color: var(--accent-green); border: 1px solid rgba(34, 197, 94, 0.25);`:`background: rgba(239, 68, 68, 0.1); color: var(--accent-rose); border: 1px solid rgba(239, 68, 68, 0.25);`,p=e.method===`CLOUD_DIRECT`?`💻 CLOUD DIRECT (Ufficio)`:e.method===`WHATSAPP_OTP_BLE`?`💬 WHATSAPP OTP BLE`:`🚚 ADR BLE PHYSICAL`;return`
                <tr>
                  <td><strong style="color: #fff;">${u}</strong></td>
                  <td><span class="badge badge-info" style="font-family: monospace;">📟 #${e.shortCode}</span></td>
                  <td>
                    ${s?`<strong>🏢 ${s.name}</strong><br><small style="color: var(--text-muted);">${s.city}</small>`:`<span class="badge badge-secondary">📦 MAGAZZINO</span>`}
                  </td>
                  <td><span style="font-size: 0.9rem;">${c}</span></td>
                  <td>
                    <span style="font-size: 0.85rem; font-weight: 700; color: var(--text-dim);">${p}</span>
                    ${e.tokenOtp?`<br><small style="color: var(--accent-amber); font-family: monospace;">${e.tokenOtp}</small>`:``}
                  </td>
                  <td style="text-align: right;">
                    <span class="badge" style="font-size: 1.05rem; font-weight: 900; ${f}">
                      ${d?`+`:``}${e.creditsAdded} cr
                    </span>
                  </td>
                </tr>
    ...
    <div>
      ...
      <div class="stat-card" style="padding: 20px; border: 1px solid var(--border-subtle); margin-bottom: 24px; background: rgba(0,0,0,0.2);">
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 16px; align-items: flex-end;">
          <div>
            <label style="font-size: 0.8rem; color: var(--text-muted); display: block; margin-bottom: 6px; font-weight: 700;">📟 Filtra per N. Deconto:</label>
            <input type="text" id="filter-extraction-board" value="${e.boardCode||``}" placeholder="Es. 3467" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px; font-family: monospace;">
          </div>

          <div>
            <label style="font-size: 0.8rem; color: var(--text-muted); display: block; margin-bottom: 6px; font-weight: 700;">🏢 Filtra per Cliente:</label>
            <input type="text" id="filter-extraction-client" value="${e.clientName||``}" placeholder="Es. Brambilla" style="width: 100%; padding: 10px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px;">
          </div>

          <div>
            <label style="font-size: 0.8rem; color: var(--text-muted); display: block; margin-bottom: 6px; font-weight: 700;">📅 Filtra per Data:</label>
            <input type="date" id="filter-extraction-date" value="${e.date||``}" style="width: 100%; padding: 8px; background: var(--bg-primary); color: #fff; border: 1px solid var(--border-color); border-radius: 6px;">
          </div>
      ...
      <div class="table-container">
        <table>
          ...
          <tbody>
            ...
            `:l.map(e=>{let t=r.find(t=>t.id===e.boardId),a=t?t.shortCode:`N/D`,o=t&&t.machineId?i.find(e=>e.id===t.machineId):null,s=o&&o.clientId?n.find(e=>e.id===o.clientId):null,c=new Date(e.timestamp),l=c.toLocaleDateString(`it-IT`,{day:`2-digit`,month:`2-digit`,year:`numeric`})+` `+c.toLocaleTimeString(`it-IT`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`}),u=s?`👤 Cliente (${s.name})`:`👤 Utente Locale`,d=`Gruppo #${e.groupId||1} (Tempo: ${e.durationSeconds||22} s)`;return`
                <tr>
                  <td><strong style="color: #fff;">${l}</strong></td>
                  <td><span class="badge badge-info" style="font-family: monospace;">📟 #${a}</span></td>
                  <td>
                    ${s?`<strong>🏢 ${s.name}</strong><br><small style="color: var(--text-muted);">${s.city}</small>`:`<span class="badge badge-secondary">📦 MAGAZZINO</span>`}
                  </td>
                  <td><span style="font-size: 0.9rem;">${u}</span></td>
                  <td>
                    <span style="font-size: 0.85rem; font-weight: 700; color: var(--text-dim);">${d}</span>
                  </td>
                  <td style="text-align: right;">
  ...
  `}function T(e=null){let t=a.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return a.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function E(e,t=null,n=null){let r=a.getUsers(),i=a.getRoleLabels(),o=a.getPermissions(),s=a.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
  ...
  `}function O(){let e=a.getSettings();return`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var k={currentUser:a.getCurrentUser(),activeTab:`dashboard`,editingId:null,editingStaffUserId:null,showProfileModal:!1,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingDecontoCode:null,viewingEmailId:null,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`,generatedOtpUrl:null,generatedOtpToken:null,diyParams:null,refillsFilter:{boardCode:``,clientName:``,date:``},extractionsFilter:{boardCode:``,clientName:``,date:``},qrParams:{clientId:``,machineId:``,boardShortCode:``,isGenerated:!1},simulatingBoardCode:null};function A(){let e=document.getElementById(`app`);if(!k.currentUser){e.innerHTML=o(),j();return}let t=``;switch(k.activeTab){case`dashboard`:t=c(k.activeTab,k.viewingDecontoCode,k.dashSearchQuery,k.dashSearchCategory,k.dashSortColumn,k.dashSortDirection,k.viewingKpiModal,k.kpiPeriod,k.kpiChartType,k.kpiCustomStart,k.kpiCustomEnd);break;case`clients`:case`machines`:case`deconto_boards`:t=v(k.activeTab,k.editingId);break;case`adr_visits`:t=y();break;case`otp_generator`:t=x(k.generatedOtpUrl,k.generatedOtpToken);break;case`client_diy`:t=b(k.diyParams);break;case`qr_generator`:t=S(k.qrParams);break;case`refills_history`:t=C(k.refillsFilter);break;case`extractions_history`:t=w(k.extractionsFilter);break;case`simulator`:t=T(k.simulatingBoardCode);break;case`user_mgmt`:case`user_management`:case`permissions_matrix`:t=E(k.activeTab,k.editingStaffUserId,k.viewingEmailId);break;case`settings`:t=O();break;default:t=c()}let n=D(k.showProfileModal,k.currentUser),r=l(k.viewingDecontoCode);e.innerHTML=`
    <div class="app-layout">
      ${s(k.currentUser,k.activeTab)}
    ...
    ${n}
    ${r}
  `,M()}function j(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value.trim();try{let e=a.login(t,n);k.currentUser=e;let r=`dashboard`;e.role===`UFFICIO`?r=`deconto_boards`:e.role===`ADR`&&(r=`simulator`),k.activeTab=r,A()}catch(e){alert(e.message)}})}function M(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(k.activeTab=t,k.editingId=null,k.editingStaffUserId=null,k.viewingDecontoCode=null,k.viewingEmailId=null,A())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{k.showProfileModal=!0,A()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{k.showProfileModal=!1,A()}),n&&n.addEventListener(`click`,()=>{k.showProfileModal=!1,A()});let r=document.getElementById(`profile-edit-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-new-password`).value;try{k.currentUser=a.updateUserProfile(k.currentUser.id,{name:t,email:n,newPassword:r?r.trim():void 0}),k.showProfileModal=!1,alert(`✅ Credenziali del profilo aggiornate con successo!`),A()}catch(e){alert(`Errore: ${e.message}`)}});let i=document.getElementById(`btn-logout`);i&&i.addEventListener(`click`,()=>{a.logout(),k.currentUser=null,k.activeTab=`dashboard`,A()});let o=document.getElementById(`btn-toggle-add-client`),s=document.getElementById(`add-client-form-container`),c=document.getElementById(`btn-cancel-add-client`),l=document.getElementById(`btn-save-new-client`);o&&s&&o.addEventListener(`click`,()=>{let e=s.style.display===`none`||!s.style.display;s.style.display=e?`block`:`none`}),c&&s&&c.addEventListener(`click`,()=>{s.style.display=`none`}),l&&l.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-type`).value,n=document.getElementById(`new-cli-ref`).value.trim(),r=document.getElementById(`new-cli-phone`).value.trim(),i=document.getElementById(`new-cli-email`).value.trim(),o=document.getElementById(`new-cli-city`).value.trim(),s=document.getElementById(`new-cli-machine`).value;if(!e){alert(`Inserisci la Ragione Sociale / Nome Cliente!`);return}try{let c=a.addClient({name:e,clientType:t,refPerson:n,phone:r,email:i,city:o,machineId:s});alert(`✅ Cliente "${c.name}" registrato con successo!`),A()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{k.editingId=e.getAttribute(`data-id`),A()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo cliente?`))try{a.deleteClient(t),alert(`✅ Cliente eliminato dall'anagrafica!`),A()}catch(e){alert(`Errore: ${e.message}`)}})});let u=document.getElementById(`btn-cancel-edit-client`);u&&u.addEventListener(`click`,()=>{k.editingId=null,A()});let d=document.getElementById(`form-edit-client`);d&&d.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value.trim(),r=document.getElementById(`edit-cli-type`).value,i=document.getElementById(`edit-cli-ref`).value.trim(),o=document.getElementById(`edit-cli-phone`).value.trim(),s=document.getElementById(`edit-cli-city`).value.trim(),c=document.getElementById(`edit-cli-address`).value.trim(),l=document.getElementById(`edit-cli-machine`).value;try{a.updateClient(t,{name:n,clientType:r,refPerson:i,phone:o,city:s,address:c,assignedMachineId:l}),k.editingId=null,alert(`✅ Scheda Cliente salvata con successo!`),A()}catch(e){alert(`Errore: ${e.message}`)}});let f=document.getElementById(`btn-toggle-add-machine`),p=document.getElementById(`add-machine-form-container`),m=document.getElementById(`btn-cancel-add-machine`),h=document.getElementById(`btn-save-new-machine`);f&&p&&f.addEventListener(`click`,()=>{let e=p.style.display===`none`||!p.style.display;p.style.display=e?`block`:`none`}),m&&p&&m.addEventListener(`click`,()=>{p.style.display=`none`}),h&&h.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-year`).value.trim(),i=document.getElementById(`new-mc-board`).value,o=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Inserisci Seriale Macchina e Modello!`);return}try{let s=a.addMachine({serialNumber:e,brand:t,model:n,productionYear:r,boardId:i,clientId:o});alert(`✅ Macchina da caffè SN "${s.serialNumber}" registrata con successo!`),A()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{k.editingId=e.getAttribute(`data-id`),A()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa macchina da caffè?`))try{a.deleteMachine(t),alert(`✅ Macchina eliminata dal parco macchine!`),A()}catch(e){alert(`Errore: ${e.message}`)}})});let g=document.getElementById(`btn-cancel-edit-mc`);g&&g.addEventListener(`click`,()=>{k.editingId=null,A()});let _=document.getElementById(`form-edit-machine`);_&&_.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value.trim(),r=document.getElementById(`edit-mc-brand`).value.trim(),i=document.getElementById(`edit-mc-model`).value.trim(),o=document.getElementById(`edit-mc-year`).value.trim(),s=document.getElementById(`edit-mc-board`).value,c=document.getElementById(`edit-mc-client`).value;try{a.updateMachine(t,{serialNumber:n,brand:r,model:i,productionYear:o,boardId:s,clientId:c}),k.editingId=null,alert(`✅ Scheda Macchina salvata con successo!`),A()}catch(e){alert(`Errore: ${e.message}`)}});let v=document.getElementById(`btn-toggle-add-board`),y=document.getElementById(`add-board-form-container`),b=document.getElementById(`btn-cancel-add-board`),x=document.getElementById(`btn-save-new-board`);v&&y&&v.addEventListener(`click`,()=>{let e=y.style.display===`none`||!y.style.display;y.style.display=e?`block`:`none`}),b&&y&&b.addEventListener(`click`,()=>{y.style.display=`none`}),x&&x.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,i=document.getElementById(`new-board-machine`).value,o=document.getElementById(`new-board-groupcount`),s=o?parseInt(o.value,10):1;if(!e){alert(`Inserisci il Codice a 4 cifre del Deconto!`);return}try{let o=a.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,groupCount:s,machineId:i});alert(`✅ Scheda Deconto #${o.shortCode} salvata PERMANENTEMENTE nel database!`),A()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{k.editingId=e.getAttribute(`data-id`),A()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa scheda Deconto?`))try{a.deleteBoard(t),alert(`✅ Scheda Deconto eliminata!`),A()}catch(e){alert(`Errore: ${e.message}`)}})});let S=document.getElementById(`btn-cancel-edit-board`);S&&S.addEventListener(`click`,()=>{k.editingId=null,A()});let C=document.getElementById(`form-edit-board`);C&&C.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value.trim(),r=document.getElementById(`edit-board-hwserial`).value.trim(),i=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-version`).value,s=document.getElementById(`edit-board-machine`).value,c=document.getElementById(`edit-board-groupcount`),l=c?parseInt(c.value,10):1;try{a.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:i,version:o,groupCount:l,machineId:s}),k.editingId=null,alert(`✅ Scheda Deconto salvata con successo!`),A()}catch(e){alert(`Errore: ${e.message}`)}});let w=document.getElementById(`btn-close-edit-modal`);w&&w.addEventListener(`click`,()=>{k.editingId=null,A()});let T=document.getElementById(`btn-toggle-add-user`),E=document.getElementById(`add-user-form-container`),D=document.getElementById(`btn-cancel-add-user`),O=document.getElementById(`btn-save-new-user`);T&&E&&T.addEventListener(`click`,()=>{let e=E.style.display===`none`||!E.style.display;E.style.display=e?`block`:`none`}),D&&E&&D.addEventListener(`click`,()=>{E.style.display=`none`}),O&&O.addEventListener(`click`,()=>{let e=document.getElementById(`new-user-username`).value.trim(),t=document.getElementById(`new-user-password`).value.trim(),n=document.getElementById(`new-user-name`).value.trim(),r=document.getElementById(`new-user-role`).value,i=document.getElementById(`new-user-email`).value.trim(),o=document.getElementById(`new-user-phone`).value.trim();if(!e||!t||!n){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{a.addUser({username:e,password:t,name:n,role:r,email:i,phone:o}),alert(`✅ Utente dipendente "${n}" (Codice ${e}) salvato PERMANENTEMENTE nel database!`),A()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{k.editingStaffUserId=e.getAttribute(`data-id`),A()})});let j=document.getElementById(`btn-close-edit-staff-modal`),M=document.getElementById(`btn-cancel-edit-staff`);j&&j.addEventListener(`click`,()=>{k.editingStaffUserId=null,A()}),M&&M.addEventListener(`click`,()=>{k.editingStaffUserId=null,A()});let N=document.getElementById(`edit-staff-form`);N&&N.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,i=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=a.updateUser(t,{username:n,name:r,role:i,email:o,phone:s,password:c?c.trim():void 0});k.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),A()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;a.updateUser(t,{status:n}),A()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{a.deleteUser(t),A()}catch(e){alert(`Errore: ${e.message}`)}})});let ee=document.getElementById(`btn-open-email-logs`);ee&&ee.addEventListener(`click`,()=>{let e=a.getEmailLogs();e.length>0?(k.viewingEmailId=e[0].id,A()):alert(`Nessuna email spedita di recente nel registro.`)});let te=document.getElementById(`btn-close-email-preview`),ne=document.getElementById(`btn-close-email-preview-footer`);te&&te.addEventListener(`click`,()=>{k.viewingEmailId=null,A()}),ne&&ne.addEventListener(`click`,()=>{k.viewingEmailId=null,A()});let re=document.getElementById(`rename-role-labels-form`);re&&re.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();a.updateRoleLabel(`UFFICIO`,t),a.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),A()});let ie=document.getElementById(`permissions-matrix-form`);ie&&ie.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),a.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),A()});let ae=document.getElementById(`setting-logo-file`);ae&&ae.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;a.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),A()},e.readAsDataURL(t)}});let oe=document.getElementById(`btn-reset-logo`);oe&&oe.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(a.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),A())});let se=document.getElementById(`settings-brand-form`);se&&se.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();a.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),A()});let ce=document.getElementById(`settings-thresholds-form`);ce&&ce.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}a.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),A()});let le=document.getElementById(`settings-brevo-form`);le&&le.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();a.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),A()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{k.viewingKpiModal=e.getAttribute(`data-kpi`),A()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{k.viewingKpiModal=null,A()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{k.kpiPeriod=e.getAttribute(`data-period`),A()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{k.kpiChartType=e.getAttribute(`data-charttype`),A()})});let ue=document.getElementById(`btn-apply-kpi-custom-date`);ue&&ue.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(k.kpiCustomStart=e,k.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),A()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let de=document.getElementById(`btn-dash-search`),P=document.getElementById(`dash-search-input`);de&&P&&(de.addEventListener(`click`,()=>{k.dashSearchQuery=P.value,k.dashSearchCategory=document.getElementById(`dash-search-category`).value,A()}),P.addEventListener(`keypress`,e=>{e.key===`Enter`&&(k.dashSearchQuery=P.value,k.dashSearchCategory=document.getElementById(`dash-search-category`).value,A())}));let fe=document.getElementById(`btn-dash-reset`);fe&&fe.addEventListener(`click`,()=>{k.dashSearchQuery=``,k.dashSearchCategory=`ALL`,A()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);k.dashSortColumn===t?k.dashSortDirection=k.dashSortDirection===`ASC`?`DESC`:`ASC`:(k.dashSortColumn=t,k.dashSortDirection=`DESC`),A()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{k.viewingDecontoCode=e.getAttribute(`data-code`),k.viewingKpiModal=null,A()})});let pe=document.getElementById(`btn-close-deconto-modal`),me=document.getElementById(`btn-close-deconto-modal-footer`);pe&&pe.addEventListener(`click`,()=>{k.viewingDecontoCode=null,A()}),me&&me.addEventListener(`click`,()=>{k.viewingDecontoCode=null,A()});let he=document.getElementById(`btn-export-csv`);he&&he.addEventListener(`click`,()=>{let e=a.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_erogazioni_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let F=document.getElementById(`btn-trigger-backup`);F&&F.addEventListener(`click`,async()=>{try{F.disabled=!0,F.innerHTML=`⏳ Backup in Corso...`;let e=a.triggerGitHubBackup();alert(`✅ Backup Cloud completato!\nID: ${e.id}\nCommit: ${e.commitHash}\nRecords: ${e.recordCount}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{F.disabled=!1,F.innerHTML=`☁️ Esegui Backup GitHub`}});let I=document.getElementById(`otp-client-select`),L=document.getElementById(`otp-machine-select`),R=document.getElementById(`otp-board-select`),z=document.getElementById(`otp-credits-input`),B=document.getElementById(`otp-custom-credits-wrapper`);z&&B&&z.addEventListener(`change`,()=>{z.value===`CUSTOM`?B.style.display=`block`:B.style.display=`none`}),I&&L&&R&&(I.addEventListener(`change`,()=>{let e=I.value;if(!e){L.value=``,R.value=``;return}let t=a.getMachines().find(t=>t.clientId===e);if(t){L.value=t.id;let e=a.getBoards().find(e=>e.machineId===t.id);e?R.value=e.shortCode:R.value=``}else L.value=``,R.value=``}),L.addEventListener(`change`,()=>{let e=L.value;if(!e){I.value=``,R.value=``;return}let t=a.getMachines().find(t=>t.id===e);if(t){I.value=t.clientId||``;let e=a.getBoards().find(e=>e.machineId===t.id);e?R.value=e.shortCode:R.value=``}}),R.addEventListener(`change`,()=>{let e=R.value;if(!e){L.value=``,I.value=``;return}let t=a.getBoards().find(t=>t.shortCode===e);if(t&&t.machineId){L.value=t.machineId;let e=a.getMachines().find(e=>e.id===t.machineId);e?I.value=e.clientId||``:I.value=``}else L.value=``,I.value=``}));let V=document.getElementById(`qr-client-select`),H=document.getElementById(`qr-machine-select`),ge=document.getElementById(`qr-board-select`),_e=document.getElementById(`btn-generate-qr-label`),ve=document.getElementById(`btn-print-qr-label`);V&&H&&ge&&(V.addEventListener(`change`,()=>{let e=V.value;if(k.qrParams.clientId=e,k.qrParams.isGenerated=!1,!e)k.qrParams.machineId=``,k.qrParams.boardShortCode=``;else{let t=a.getMachines().find(t=>t.clientId===e);if(t){k.qrParams.machineId=t.id;let e=a.getBoards().find(e=>e.machineId===t.id);k.qrParams.boardShortCode=e?e.shortCode:``}else k.qrParams.machineId=``,k.qrParams.boardShortCode=``}A()}),H.addEventListener(`change`,()=>{let e=H.value;if(k.qrParams.machineId=e,k.qrParams.isGenerated=!1,!e)k.qrParams.clientId=``,k.qrParams.boardShortCode=``;else{let t=a.getMachines().find(t=>t.id===e);if(t){k.qrParams.clientId=t.clientId||``;let e=a.getBoards().find(e=>e.machineId===t.id);k.qrParams.boardShortCode=e?e.shortCode:``}}A()}),ge.addEventListener(`change`,()=>{let e=ge.value;if(k.qrParams.boardShortCode=e,k.qrParams.isGenerated=!1,!e)k.qrParams.machineId=``,k.qrParams.clientId=``;else{let t=a.getBoards().find(t=>t.shortCode===e);if(t&&t.machineId){k.qrParams.machineId=t.machineId;let e=a.getMachines().find(e=>e.id===t.machineId);k.qrParams.clientId=e&&e.clientId||``}else k.qrParams.machineId=``,k.qrParams.clientId=``}A()})),_e&&_e.addEventListener(`click`,()=>{let e=document.getElementById(`qr-client-select`).value,t=document.getElementById(`qr-machine-select`).value,n=document.getElementById(`qr-board-select`).value;if(!e||!t||!n){alert(`Seleziona il Cliente, la Macchina e la Scheda Deconto per procedere!`);return}k.qrParams.clientId=e,k.qrParams.machineId=t,k.qrParams.boardShortCode=n,k.qrParams.isGenerated=!0,A()}),ve&&ve.addEventListener(`click`,()=>{let e=document.getElementById(`printable-qr-label`);if(!e)return;let t=document.createElement(`iframe`);t.style.position=`fixed`,t.style.right=`0`,t.style.bottom=`0`,t.style.width=`0`,t.style.height=`0`,t.style.border=`0`,document.body.appendChild(t);let n=t.contentWindow.document;n.open(),n.write(`
        <html>
          <head>
      ...
      `),n.close()});let ye=document.getElementById(`btn-direct-refill-panel`);ye&&ye.addEventListener(`click`,()=>{let e=document.getElementById(`otp-client-select`).value,t=document.getElementById(`otp-machine-select`).value,n=document.getElementById(`otp-board-select`).value,r=document.getElementById(`otp-credits-input`).value,i=document.getElementById(`otp-custom-credits-value`).value,o=document.getElementById(`otp-expiry-input`).value;if(!e||!n){alert(`Seleziona sia il Cliente che la Scheda Deconto per procedere!`);return}let s=200;if(r===`CUSTOM`){let e=parseInt(i,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per i crediti personalizzati!`);return}s=e}else s=parseInt(r,10);try{a.performRefill({boardShortCode:n,credits:s,method:`CLOUD_DIRECT`,operatorId:k.currentUser?k.currentUser.id:`usr_001`}),alert(`✅ Ricarica diretta di ${s>=0?`+`:``}${s} caffè effettuata con successo sul Deconto #${n}!\nCredito aggiornato via Cloud.`),k.selectedClientTemp=e,k.selectedMachineTemp=t,k.selectedBoardTemp=n,k.selectedCreditsTemp=r,k.selectedCustomCreditsTemp=r===`CUSTOM`?s:``,k.selectedExpiryTemp=o,k.generatedOtpUrl=null,k.generatedOtpToken=null,A();let i=document.getElementById(`otp-client-select`),c=document.getElementById(`otp-machine-select`),l=document.getElementById(`otp-board-select`),u=document.getElementById(`otp-credits-input`),d=document.getElementById(`otp-custom-credits-wrapper`),f=document.getElementById(`otp-custom-credits-value`),p=document.getElementById(`otp-expiry-input`);i&&(i.value=k.selectedClientTemp),c&&(c.value=k.selectedMachineTemp),l&&(l.value=k.selectedBoardTemp),u&&(u.value=k.selectedCreditsTemp),u&&u.value===`CUSTOM`&&d&&(d.style.display=`block`,f&&(f.value=k.selectedCustomCreditsTemp)),p&&(p.value=k.selectedExpiryTemp)}catch(e){alert(`❌ Errore durante la ricarica diretta: ${e.message}`)}});let be=document.getElementById(`btn-generate-otp-link`);be&&be.addEventListener(`click`,()=>{let e=document.getElementById(`otp-client-select`).value,t=document.getElementById(`otp-board-select`).value,n=document.getElementById(`otp-credits-input`).value,r=document.getElementById(`otp-expiry-input`).value;if(!e||!t){alert(`Seleziona sia il Cliente che la Scheda Deconto!`);return}let i=200;if(n===`CUSTOM`){let e=parseInt(document.getElementById(`otp-custom-credits-value`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per i crediti personalizzati!`);return}i=e}else i=parseInt(n,10);let o=a.getClients().find(t=>t.id===e),s=o?o.name:`Cliente`;if(!r){let e=new Date;e.setMonth(e.getMonth()+1),r=e.toISOString().split(`T`)[0]}let c=Math.random().toString(36).substring(2,7).toUpperCase(),l=`OTP-${Math.floor(1e3+Math.random()*9e3)}-${c}`;k.generatedOtpUrl=`${window.location.origin+window.location.pathname}?tab=client_diy&clientName=${encodeURIComponent(s)}&board=${t}&otp=${l}&credits=${i}`,k.generatedOtpToken=l,k.selectedClientTemp=e,k.selectedMachineTemp=document.getElementById(`otp-machine-select`).value,k.selectedBoardTemp=t,k.selectedCreditsTemp=n,k.selectedCustomCreditsTemp=n===`CUSTOM`?i:``,k.selectedExpiryTemp=r,A();let u=document.getElementById(`otp-client-select`),d=document.getElementById(`otp-machine-select`),f=document.getElementById(`otp-board-select`),p=document.getElementById(`otp-credits-input`),m=document.getElementById(`otp-custom-credits-wrapper`),h=document.getElementById(`otp-custom-credits-value`),g=document.getElementById(`otp-expiry-input`);u&&(u.value=k.selectedClientTemp),d&&(d.value=k.selectedMachineTemp),f&&(f.value=k.selectedBoardTemp),p&&(p.value=k.selectedCreditsTemp),p&&p.value===`CUSTOM`&&m&&(m.style.display=`block`,h&&(h.value=k.selectedCustomCreditsTemp)),g&&(g.value=k.selectedExpiryTemp),setTimeout(()=>{let e=document.getElementById(`otp-result-container`);e&&e.scrollIntoView({behavior:`smooth`})},50)});let xe=document.getElementById(`btn-copy-otp-link`);xe&&xe.addEventListener(`click`,()=>{let e=document.getElementById(`generated-otp-url`);e&&(e.select(),document.execCommand(`copy`),alert(`📋 Link di ricarica copiato negli appunti!`))});let Se=document.getElementById(`btn-whatsapp-otp-send`);if(Se&&k.generatedOtpUrl){let e=document.getElementById(`otp-board-select`)?.value||`Deconto`,t=`Ciao! Ecco il link per ricaricare +${document.getElementById(`otp-credits-input`)?.value||`200`} caffè sulla tua macchina Deconto #${e}: ${k.generatedOtpUrl}`;Se.setAttribute(`href`,`https://wa.me/?text=${encodeURIComponent(t)}`)}let Ce=document.getElementById(`btn-test-otp-link`);Ce&&k.generatedOtpUrl&&Ce.addEventListener(`click`,()=>{let e=new URLSearchParams(k.generatedOtpUrl.split(`?`)[1]);k.diyParams={clientName:e.get(`clientName`),boardShortCode:e.get(`board`),tokenOtp:e.get(`otp`),credits:parseInt(e.get(`credits`),10),success:!1},k.activeTab=`client_diy`,A()});let U=document.getElementById(`btn-client-diy-refill`);U&&U.addEventListener(`click`,()=>{let e=U.getAttribute(`data-board`),t=parseInt(U.getAttribute(`data-credits`),10),n=U.getAttribute(`data-otp`),r=document.getElementById(`diy-status-msg`);r&&(r.innerHTML=`<span style="color: var(--accent-cyan); font-weight: 700;">📡 Connessione Bluetooth in corso... Sincronizzazione relè...</span>`),U.disabled=!0,setTimeout(()=>{try{a.performRefill({boardShortCode:e,credits:t,method:`WHATSAPP_OTP_BLE`,tokenOtp:n}),k.diyParams?k.diyParams.success=!0:k.diyParams={success:!0,clientName:`Studio Legale Brambilla`,boardShortCode:e,tokenOtp,credits:t},A()}catch(e){alert(`Errore durante l'accredito: ${e.message}`),r&&(r.innerHTML=`<span style="color: var(--accent-rose); font-weight: 700;">❌ Errore BLE: ${e.message}</span>`),U.disabled=!1}},1200)});let we=document.getElementById(`btn-back-to-office-from-diy`);we&&we.addEventListener(`click`,()=>{k.diyParams=null,k.generatedOtpUrl=null,k.generatedOtpToken=null,k.activeTab=`otp_generator`,A()});let W=document.getElementById(`filter-refill-board`),G=document.getElementById(`filter-refill-client`),K=document.getElementById(`filter-refill-date`),Te=document.getElementById(`btn-reset-refill-filters`);W&&W.addEventListener(`input`,()=>{k.refillsFilter.boardCode=W.value,A();let e=document.getElementById(`filter-refill-board`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),G&&G.addEventListener(`input`,()=>{k.refillsFilter.clientName=G.value,A();let e=document.getElementById(`filter-refill-client`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),K&&K.addEventListener(`change`,()=>{k.refillsFilter.date=K.value,A()}),Te&&Te.addEventListener(`click`,()=>{k.refillsFilter={boardCode:``,clientName:``,date:``},A()});let q=document.getElementById(`filter-extraction-board`),J=document.getElementById(`filter-extraction-client`),Y=document.getElementById(`filter-extraction-date`),Ee=document.getElementById(`btn-reset-extraction-filters`);q&&q.addEventListener(`input`,()=>{k.extractionsFilter.boardCode=q.value,A();let e=document.getElementById(`filter-extraction-board`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),J&&J.addEventListener(`input`,()=>{k.extractionsFilter.clientName=J.value,A();let e=document.getElementById(`filter-extraction-client`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),Y&&Y.addEventListener(`change`,()=>{k.extractionsFilter.date=Y.value,A()}),Ee&&Ee.addEventListener(`click`,()=>{k.extractionsFilter={boardCode:``,clientName:``,date:``},A()});let X=document.getElementById(`modal-refill-credits-select`),Z=document.getElementById(`modal-refill-credits-custom`),De=document.getElementById(`btn-modal-perform-refill`);X&&Z&&X.addEventListener(`change`,()=>{X.value===`CUSTOM`?Z.style.display=`block`:Z.style.display=`none`}),De&&De.addEventListener(`click`,()=>{let e=De.getAttribute(`data-board-code`),t=document.getElementById(`modal-refill-credits-select`).value,n=200;if(t===`CUSTOM`){let e=parseInt(document.getElementById(`modal-refill-credits-custom`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per la ricarica personalizzata!`);return}n=e}else n=parseInt(t,10);let r=k.currentUser,i=r&&(r.role===`ADMIN`||r.role===`UFFICIO`),o=i?`CLOUD_DIRECT`:`ADR_BLE_PHYSICAL`;try{a.performRefill({boardShortCode:e,credits:n,method:o,operatorId:r?r.id:`usr_001`}),alert(`✅ Ricarica di ${n>=0?`+`:``}${n} caffè effettuata con successo!\nCanale: ${i?`Cloud Wi-Fi (Ufficio)`:`Bluetooth BLE (ADR)`}`),A()}catch(e){alert(`❌ Errore durante la ricarica: ${e.message}`)}});let Oe=document.getElementById(`sim-board-select`),Q=document.getElementById(`btn-sim-brew`),$=document.getElementById(`btn-sim-reset`);Oe&&Oe.addEventListener(`change`,()=>{k.simulatingBoardCode=Oe.value,A()}),Q&&Q.addEventListener(`click`,()=>{let e=k.simulatingBoardCode||(a.getBoards()[0]?a.getBoards()[0].shortCode:null);if(!e)return;let t=a.getBoards().find(t=>t.shortCode===e);if(!t||t.remainingCredits<=0)return;Q.disabled=!0,Q.innerText=`☕ EROGAZIONE IN CORSO (230V SENSE IN)...`,$&&($.disabled=!0);let n=document.getElementById(`signal-sense-volts`),r=document.getElementById(`signal-sense-badge`),i=document.getElementById(`sim-console-log`);n&&(n.innerText=`230V AC`),r&&(r.innerText=`⚡ SENSE ATTIVO`,r.className=`badge badge-success`),i&&(i.innerHTML+=`<br>[INPUT]: Rilevata tensione 230V AC su Sense-In (Filo Marrone).<br>[FIRMWARE]: Conteggio tempo di erogazione avviato...`,i.scrollTop=i.scrollHeight),setTimeout(()=>{try{let n=a.registerCoffeeExtraction(e,22,1);n&&n.success===!1?alert(`Impossibile erogare: ${n.reason}`):i&&(i.innerHTML+=`<br>[FIRMWARE]: Erogazione terminata (22s simulati).<br>[MEMORY]: Credito scalato. Nuovo saldo: ${t.remainingCredits} caffè.<br>[HARDWARE]: Relè di blocco: ${t.relayStatus===`CLOSED_OK`?`CONNESSO/ABILITATO`:`APERTO/BLOCCATO`}`)}catch(e){console.error(e)}A()},1500)}),$&&$.addEventListener(`click`,()=>{let e=k.simulatingBoardCode||(a.getBoards()[0]?a.getBoards()[0].shortCode:null);if(e)try{a.performRefill({boardShortCode:e,credits:200,method:`CLOUD_DIRECT`,operatorId:k.currentUser?k.currentUser.id:`usr_001`});let t=document.getElementById(`sim-console-log`);t&&(t.innerHTML+=`<br>[CLOUDLINK]: Comando Ricarica remota +200 ricevuto via Wi-Fi.<br>[MEMORY]: Crediti aggiornati. Relè sbloccato.`),A()}catch(e){alert(`Errore: ${e.message}`)}})}function N(){let e=new URLSearchParams(window.location.search);e.get(`tab`)===`client_diy`&&(k.activeTab=`client_diy`,k.diyParams={clientName:e.get(`clientName`)||`Studio Legale Brambilla`,boardShortCode:e.get(`board`)||`3467`,tokenOtp:e.get(`otp`)||`OTP-9981-X79K2`,credits:parseInt(e.get(`credits`),10)||200,success:!1},window.history.replaceState({},document.title,window.location.pathname))}N(),A();
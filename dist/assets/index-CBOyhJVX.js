...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};new class{constructor(){this.isSupported=typeof navigator<`u`&&`bluetooth`in navigator,this.connectedDevice=null}checkSupport(){return this.isSupported}async connectToBoardByShortCode(e){if(console.log(`📡 Ricerca dispositivo Deconto con codice breve [${e}]...`),this.isSupported&&navigator.bluetooth)try{let t=await navigator.bluetooth.requestDevice({filters:[{namePrefix:`DECONTO_${e}`}],optionalServices:[`0000ffe0-0000-1000-8000-00805f9b34fb`]});return this.connectedDevice=t,{success:!0,deviceName:t.name,isRealHardware:!0}}catch(e){console.warn(`Fallback a simulazione BLE locale:`,e.message)}return await new Promise(e=>setTimeout(e,1500)),{success:!0,deviceName:`DECONTO_${e}`,shortCode:e,isRealHardware:!1,connectedAt:new Date().toISOString()}}async sendRefillOtpToken(e,t,n){if(!(await this.connectToBoardByShortCode(e)).success)throw Error(`Impossibile connettersi al dispositivo DECONTO_${e}`);return await new Promise(e=>setTimeout(e,1e3)),{success:!0,shortCode:e,creditsAccredited:t,tokenApplied:n,relayStatus:`CLOSED_OK`,timestamp:new Date().toISOString()}}};var a=new class{constructor(){this.repoUrl=`https://github.com/deconto-org/deconto-db-backups`}generateDatabaseSnapshot(){return{version:`1.0.0`,timestamp:new Date().toISOString(),data:i.data}}async executeBackupNow(){let e=this.generateDatabaseSnapshot(),t=JSON.stringify(e,null,2);return await new Promise(e=>setTimeout(e,1200)),{success:!0,backupRecord:i.triggerGitHubBackup(),sizeBytes:new Blob([t]).size,snapshotTimestamp:e.timestamp}}};function o(e,t){let n=i.getSettings(),r=i.getRoleLabels(),a=e.role===`UFFICIO`||e.role===`ADMIN`,o=e.role===`ADR`||e.role===`ADMIN`,s=e.role===`ADMIN`;return`
  ...
  `}function u(e,t=null,n=null){let r=i.getUsers(),a=i.getRoleLabels(),o=i.getPermissions(),s=i.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
    ...
    ${c}
  `}function d(e=`clients`,t=null){let n=i.getClients(),r=i.getMachines(),a=i.getBoards();i.getRefillLogs();let o=``;if(e===`clients`&&t){let e=n.find(e=>e.id===t);if(e){let t=r.find(t=>t.clientId===e.id);o=`
    ...
    ${o}
  `}function f(e){return i.getClients(),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var h={currentUser:i.getCurrentUser(),activeTab:`dashboard`,showProfileModal:!1,editingStaffUserId:null,editingId:null,viewingDecontoCode:null,viewingEmailId:null,selectedSimBoardCode:`9901`,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`};function g(){let e=document.getElementById(`app`);if(!h.currentUser){e.innerHTML=s(),_();return}let t=h.currentUser,n=``;h.activeTab===`settings`?n=p():h.activeTab===`simulator`?n=m(h.selectedSimBoardCode):h.activeTab===`user_management`||h.activeTab===`permissions_matrix`?n=u(h.activeTab,h.editingStaffUserId,h.viewingEmailId):t.role===`ADMIN`||t.role===`UFFICIO`?n=h.activeTab===`clients`||h.activeTab===`machines`||h.activeTab===`deconto_boards`||h.activeTab===`qr_generator`||h.activeTab===`otp_generator`||h.activeTab===`refills_history`?d(h.activeTab,h.editingId):h.activeTab===`adr_visits`?f(h.activeTab):l(h.activeTab,h.viewingDecontoCode,h.dashSearchQuery,h.dashSearchCategory,h.dashSortColumn,h.dashSortDirection,h.viewingKpiModal,h.kpiPeriod,h.kpiChartType,h.kpiCustomStart,h.kpiCustomEnd):t.role===`ADR`&&(n=f(h.activeTab)),e.innerHTML=`
    ...
    ${h.showProfileModal?c(t):``}
  `,v()}function _(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value;try{let e=i.login(t,n);h.currentUser=e,h.activeTab=e.role===`ADR`?`adr_visits`:`dashboard`,g()}catch(e){alert(e.message)}})}function v(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(h.activeTab=t,h.editingId=null,h.editingStaffUserId=null,h.viewingDecontoCode=null,h.viewingEmailId=null,g())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{h.showProfileModal=!0,g()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{h.showProfileModal=!1,g()}),n&&n.addEventListener(`click`,()=>{h.showProfileModal=!1,g()});let r=document.getElementById(`user-profile-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-phone`).value.trim(),a=document.getElementById(`profile-avatar`).value,o=document.getElementById(`profile-current-password`).value,s=document.getElementById(`profile-new-password`).value,c=document.getElementById(`profile-confirm-password`).value;if(!i.verifyPassword(h.currentUser.id,o)){alert(`Password attuale non corretta!`);return}if(s||c){if(s!==c){alert(`Le nuove password inserite non coincidono!`);return}if(s.length<4){alert(`La nuova password deve contenere almeno 4 caratteri!`);return}}try{h.currentUser=i.updateUserProfile(h.currentUser.id,{name:t,email:n,phone:r,avatar:a,newPassword:s?s.trim():void 0}),h.showProfileModal=!1,alert(`✅ Profilo utente aggiornato con successo!`),g()}catch(e){alert(`Errore: ${e.message}`)}});let o=document.getElementById(`btn-logout`);o&&o.addEventListener(`click`,()=>{i.logout(),h.currentUser=null,h.activeTab=`dashboard`,g()});let s=document.getElementById(`btn-open-add-staff-modal`),c=document.getElementById(`btn-close-add-staff-modal`),l=document.getElementById(`btn-cancel-add-staff`),u=document.getElementById(`add-staff-modal`);s&&u&&s.addEventListener(`click`,()=>{u.style.display=`flex`}),c&&u&&c.addEventListener(`click`,()=>{u.style.display=`none`}),l&&u&&l.addEventListener(`click`,()=>{u.style.display=`none`});let d=document.getElementById(`add-staff-form`);d&&d.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`new-user-username`).value.trim(),n=document.getElementById(`new-user-password`).value.trim(),r=document.getElementById(`new-user-name`).value.trim(),a=document.getElementById(`new-user-role`).value,o=document.getElementById(`new-user-email`).value.trim(),s=document.getElementById(`new-user-phone`).value.trim();if(!t||!n||!r){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{i.addUser({username:t,password:n,name:r,role:a,email:o,phone:s}),alert(`✅ Utente dipendente "${r}" (Codice ${t}) salvato PERMANENTEMENTE nel database!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingStaffUserId=e.getAttribute(`data-id`),g()})});let f=document.getElementById(`btn-close-edit-staff-modal`),p=document.getElementById(`btn-cancel-edit-staff`);f&&f.addEventListener(`click`,()=>{h.editingStaffUserId=null,g()}),p&&p.addEventListener(`click`,()=>{h.editingStaffUserId=null,g()});let m=document.getElementById(`edit-staff-form`);m&&m.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,a=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=i.updateUser(t,{username:n,name:r,role:a,email:o,phone:s,password:c?c.trim():void 0});h.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;i.updateUser(t,{status:n}),g()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{i.deleteUser(t),g()}catch(e){alert(`Errore: ${e.message}`)}})});let _=document.getElementById(`btn-open-email-logs`);_&&_.addEventListener(`click`,()=>{let e=i.getEmailLogs();e.length>0?(h.viewingEmailId=e[0].id,g()):alert(`Nessuna email spedita di recente nel registro.`)});let v=document.getElementById(`btn-close-email-preview`),y=document.getElementById(`btn-close-email-preview-footer`);v&&v.addEventListener(`click`,()=>{h.viewingEmailId=null,g()}),y&&y.addEventListener(`click`,()=>{h.viewingEmailId=null,g()});let b=document.getElementById(`rename-role-labels-form`);b&&b.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();i.updateRoleLabel(`UFFICIO`,t),i.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),g()});let x=document.getElementById(`permissions-matrix-form`);x&&x.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),i.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),g()});let S=document.getElementById(`setting-logo-file`);S&&S.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;i.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),g()},e.readAsDataURL(t)}});let C=document.getElementById(`btn-reset-logo`);C&&C.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(i.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),g())});let w=document.getElementById(`settings-brand-form`);w&&w.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();i.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),g()});let T=document.getElementById(`settings-thresholds-form`);T&&T.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}i.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),g()});let E=document.getElementById(`settings-brevo-form`);E&&E.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();i.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),g()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingKpiModal=e.getAttribute(`data-kpi`),g()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingKpiModal=null,g()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{h.kpiPeriod=e.getAttribute(`data-period`),g()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{h.kpiChartType=e.getAttribute(`data-charttype`),g()})});let D=document.getElementById(`btn-apply-kpi-custom-date`);D&&D.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(h.kpiCustomStart=e,h.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),g()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let O=document.getElementById(`btn-dash-search`),k=document.getElementById(`dash-search-input`);O&&k&&(O.addEventListener(`click`,()=>{h.dashSearchQuery=k.value,h.dashSearchCategory=document.getElementById(`dash-search-category`).value,g()}),k.addEventListener(`keypress`,e=>{e.key===`Enter`&&(h.dashSearchQuery=k.value,h.dashSearchCategory=document.getElementById(`dash-search-category`).value,g())}));let A=document.getElementById(`btn-dash-reset`);A&&A.addEventListener(`click`,()=>{h.dashSearchQuery=``,h.dashSearchCategory=`ALL`,g()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);h.dashSortColumn===t?h.dashSortDirection=h.dashSortDirection===`ASC`?`DESC`:`ASC`:(h.dashSortColumn=t,h.dashSortDirection=`DESC`),g()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingDecontoCode=e.getAttribute(`data-code`),g()})});let j=document.getElementById(`btn-close-deconto-modal`),M=document.getElementById(`btn-close-deconto-modal-footer`);j&&j.addEventListener(`click`,()=>{h.viewingDecontoCode=null,g()}),M&&M.addEventListener(`click`,()=>{h.viewingDecontoCode=null,g()});let N=document.getElementById(`btn-export-csv`);N&&N.addEventListener(`click`,()=>{let e=i.exportBoardsToCsv(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_parco_macchine_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let P=document.getElementById(`btn-trigger-backup`);P&&P.addEventListener(`click`,async()=>{try{P.disabled=!0,P.innerHTML=`⏳ Backup in Corso...`;let e=await a.triggerBackup();alert(`✅ Backup Cloud completato!\nEsito: ${e.message}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{P.disabled=!1,P.innerHTML=`☁️ Esegui Backup GitHub`}})}g();
...
`;return this.data.coffeeLogs.forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};new class{constructor(){this.isSupported=typeof navigator<`u`&&`bluetooth`in navigator,this.connectedDevice=null}checkSupport(){return this.isSupported}async connectToBoardByShortCode(e){if(console.log(`📡 Ricerca dispositivo Deconto con codice breve [${e}]...`),this.isSupported&&navigator.bluetooth)try{let t=await navigator.bluetooth.requestDevice({filters:[{namePrefix:`DECONTO_${e}`}],optionalServices:[`0000ffe0-0000-1000-8000-00805f9b34fb`]});return this.connectedDevice=t,{success:!0,deviceName:t.name,isRealHardware:!0}}catch(e){console.warn(`Fallback a simulazione BLE locale:`,e.message)}return await new Promise(e=>setTimeout(e,1500)),{success:!0,deviceName:`DECONTO_${e}`,shortCode:e,isRealHardware:!1,connectedAt:new Date().toISOString()}}async sendRefillOtpToken(e,t,n){if(!(await this.connectToBoardByShortCode(e)).success)throw Error(`Impossibile connettersi al dispositivo DECONTO_${e}`);return await new Promise(e=>setTimeout(e,1e3)),{success:!0,shortCode:e,creditsAccredited:t,tokenApplied:n,relayStatus:`CLOSED_OK`,timestamp:new Date().toISOString()}}},new class{constructor(){this.repoUrl=`https://github.com/deconto-org/deconto-db-backups`}generateDatabaseSnapshot(){return{version:`1.0.0`,timestamp:new Date().toISOString(),data:i.data}}async executeBackupNow(){let e=this.generateDatabaseSnapshot(),t=JSON.stringify(e,null,2);return await new Promise(e=>setTimeout(e,1200)),{success:!0,backupRecord:i.triggerGitHubBackup(),sizeBytes:new Blob([t]).size,snapshotTimestamp:e.timestamp}}};function a(e,t){let n=i.getSettings(),r=i.getRoleLabels(),a=e.role===`UFFICIO`||e.role===`ADMIN`,o=e.role===`ADR`||e.role===`ADMIN`,s=e.role===`ADMIN`;return`
    <aside class="sidebar">
      <div class="sidebar-header" style="display: flex; align-items: center; gap: 12px; padding: 20px 16px; border-bottom: 1px solid var(--border-subtle);">
        ...
        <div>
          <div style="font-weight: 800; font-size: 1.1rem; color: #fff; letter-spacing: 0.5px;">${n.brandTitle||`DECONTO`}</div>
      ...
      <div class="sidebar-footer" style="padding: 16px; border-top: 1px solid var(--border-subtle); display: flex; justify-content: space-between; align-items: center;">
        <div style="display: flex; align-items: center; gap: 10px; cursor: pointer;" id="btn-open-profile-modal">
          <div style="font-size: 1.5rem;">${e.avatar||`👤`}</div>
          <div style="line-height: 1.2;">
            <div style="font-weight: 700; font-size: 0.85rem; color: #fff;">${e.name}</div>
  ...
  `}function o(){let e=i.getSettings();return`
  ...
  `}function c(e,t=null,n=``,r=`ALL`,a=`shortCode`,o=`DESC`,s=null,c=`30DAYS`,l=`LINE`){let u=i.getClients(),d=i.getMachines(),f=i.getBoards(),p=i.getCoffeeLogs(),m=u.length,h=d.length,g=p.length,_=f.filter(e=>e.remainingCredits>20),v=f.filter(e=>e.remainingCredits>0&&e.remainingCredits<=20),y=f.filter(e=>e.remainingCredits===0),b=f.filter(e=>{if(!n.trim())return!0;let t=n.toLowerCase().trim(),a=i.getBoardFullDetails(e.id),o=(a&&a.client?a.client.name:``).toLowerCase(),s=(a&&a.machine?a.machine.model:``).toLowerCase(),c=(a&&a.machine?a.machine.serialNumber:``).toLowerCase(),l=String(e.shortCode).toLowerCase(),u=String(e.remainingCredits),d=e.isOnlineWifi?`wi-fi 6 online`:`softap offline`,f=new Date(e.lastSyncDate).toLocaleString(`it-IT`).toLowerCase();return r===`SHORT_CODE`?l.includes(t):r===`CLIENT`?o.includes(t):r===`MODEL`?s.includes(t):r===`CREDITS`?u.includes(t):r===`CONNECTION`?d.includes(t):r===`SYNC_DATE`?f.includes(t):l.includes(t)||o.includes(t)||s.includes(t)||c.includes(t)||u.includes(t)||d.includes(t)||f.includes(t)});b.sort((e,t)=>{let n=i.getBoardFullDetails(e.id),r=i.getBoardFullDetails(t.id),s=n&&n.client?n.client.name:``,c=r&&r.client?r.client.name:``,l=n&&n.machine?n.machine.model:``,u=r&&r.machine?r.machine.model:``,d,f;return a===`shortCode`?(d=parseInt(e.shortCode,10),f=parseInt(t.shortCode,10)):a===`client`?(d=s.toLowerCase(),f=c.toLowerCase()):a===`model`?(d=l.toLowerCase(),f=u.toLowerCase()):a===`credits`?(d=e.remainingCredits,f=t.remainingCredits):a===`connection`?(d=+!!e.isOnlineWifi,f=+!!t.isOnlineWifi):a===`syncDate`?(d=new Date(e.lastSyncDate).getTime(),f=new Date(t.lastSyncDate).getTime()):(d=parseInt(e.shortCode,10),f=parseInt(t.shortCode,10)),d<f?o===`ASC`?-1:1:d>f?o===`ASC`?1:-1:0});let x=e=>a===e?o===`ASC`?`<span style="color: var(--accent-cyan);"> ▲</span>`:`<span style="color: var(--accent-cyan);"> ▼</span>`:`<span style="color: var(--text-dim); opacity: 0.5;"> ⇅</span>`,S=``;if(s===`kpi_clients`){let e={};u.forEach(t=>{let n=t.city||`Milano`;e[n]=(e[n]||0)+1}),S=`
    ...
    ${C}
    ${S}
  `}function l(e,t=null,n=null){let r=i.getUsers(),a=i.getRoleLabels(),o=i.getPermissions(),s=i.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
    ...
    ${c}
  `}function u(e,t=null){let n=i.getClients(),r=i.getMachines(),a=i.getBoards();i.getRefillLogs();let o=``;if(e===`clients`&&t){let e=n.find(e=>e.id===t);e&&(o=`
      ...
      `)}if(e===`machines`&&t){let e=r.find(e=>e.id===t);e&&(o=`
            ...
            <form id="form-edit-board">
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                <div>
                  ...
                  <input type="text" id="edit-board-shortcode" value="${e.shortCode}" required style="width: 100%; padding: 10px; background: var(--bg-primary); color: var(--accent-cyan); border: 1px solid var(--border-color); border-radius: 6px; font-weight: 800; font-family: monospace;">
                </div>
                <div>
            ...
            <tbody>
              ${n.map(e=>{let t=r.filter(t=>t.clientId===e.id);return`
                  <tr>
                    ...
                    <td>
                      ${t.length>0?t.map(e=>`<span class="badge badge-info" style="margin-right: 4px;">☕ ${e.serialNumber}</span>`).join(``):`<span class="badge badge-warning">📦 Nessuna Macchina</span>`}
                    </td>
            ...
            <tbody>
              ${r.map(e=>{let t=n.find(t=>t.id===e.clientId),r=a.find(t=>t.machineId===e.id);return`
                  <tr>
                    ...
                    <td>${r?`<span class="badge badge-info">📟 Deconto #${r.shortCode}</span>`:`<span style="color: var(--text-muted);">Nessuna</span>`}</td>
          ...
          <table>
            ...
            <tbody>
              ${a.map(e=>{let t=r.find(t=>t.id===e.machineId),i=t?n.find(e=>e.id===t.clientId):null;return`
                  <tr>
                    <td>
                      <button class="btn btn-secondary btn-deconto-detail" data-code="${e.shortCode}" style="font-family: monospace; font-weight: 900; font-size: 1.1rem; color: var(--accent-cyan); padding: 4px 10px;">
                        #${e.shortCode}
                      </button>
                    </td>
                    <td><strong>${e.hwSerial||`N/D`}</strong><br><small style="color: var(--text-muted); font-family: monospace;">${e.macAddress||``}</small></td>
                    <td><strong style="font-size: 1.2rem; color: ${e.remainingCredits>20?`var(--accent-green)`:`var(--accent-rose)`};">${e.remainingCredits}</strong> cialde</td>
                    <td><span class="badge ${e.relayStatus===`CLOSED_OK`?`badge-success`:`badge-danger`}">${e.relayStatus===`CLOSED_OK`?`🔓 CHIUSO (OK)`:`🔒 APERTO (BLOCCO)`}</span></td>
                    <td>${t?`<span class="badge badge-info">☕ ${t.serialNumber}</span>`:`<span style="color: var(--text-muted);">A Banco</span>`}</td>
    ...
    `:`<div>Seleziona una voce del menu per proseguire.</div>`}function d(e){return i.getClients(),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function f(){let e=i.getSettings();return`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function p(e=null){let t=i.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return i.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var m={currentUser:i.getCurrentUser(),activeTab:`dashboard`,showProfileModal:!1,editingStaffUserId:null,editingId:null,viewingDecontoCode:null,viewingEmailId:null,selectedSimBoardCode:`9901`,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`};function h(){let e=document.getElementById(`app`);if(!m.currentUser){e.innerHTML=o(),g();return}let t=m.currentUser,n=``;m.activeTab===`settings`?n=f():m.activeTab===`simulator`?n=p(m.selectedSimBoardCode):m.activeTab===`user_management`||m.activeTab===`permissions_matrix`?n=l(m.activeTab,m.editingStaffUserId,m.viewingEmailId):t.role===`ADMIN`||t.role===`UFFICIO`?n=m.activeTab===`clients`||m.activeTab===`machines`||m.activeTab===`deconto_boards`||m.activeTab===`qr_generator`||m.activeTab===`otp_generator`||m.activeTab===`refills_history`?u(m.activeTab,m.editingId):m.activeTab===`adr_visits`?d(m.activeTab):c(m.activeTab,m.viewingDecontoCode,m.dashSearchQuery,m.dashSearchCategory,m.dashSortColumn,m.dashSortDirection,m.viewingKpiModal,m.kpiPeriod,m.kpiChartType):t.role===`ADR`&&(n=m.activeTab===`adr_visits`?d(m.activeTab):u(m.activeTab,m.editingId));let r=``;m.showProfileModal&&(r=s(t)),e.innerHTML=`
    ...
    ${r}
  `,_()}function g(){let e=document.getElementById(`login-form`),t=document.getElementById(`login-error-msg`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let n=document.getElementById(`login-username`).value,r=document.getElementById(`login-password`).value;try{let e=i.authenticate(n,r);m.currentUser=e,m.activeTab=e.role===`ADMIN`?`dashboard`:`clients`,h()}catch(e){t.innerText=e.message,t.style.display=`block`}})}function _(){let e=document.getElementById(`btn-logout`);e&&e.addEventListener(`click`,()=>{i.logout(),m.currentUser=null,h()});let t=document.getElementById(`btn-open-profile-modal`);t&&t.addEventListener(`click`,()=>{m.showProfileModal=!0,h()});let n=document.getElementById(`btn-close-profile-modal`),r=document.getElementById(`btn-cancel-profile`);n&&n.addEventListener(`click`,()=>{m.showProfileModal=!1,h()}),r&&r.addEventListener(`click`,()=>{m.showProfileModal=!1,h()}),document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(m.activeTab=t,m.editingId=null,h())})});let a=document.getElementById(`btn-toggle-add-client`),o=document.getElementById(`add-client-form-container`);a&&o&&a.addEventListener(`click`,()=>{o.style.display=o.style.display===`none`?`block`:`none`});let s=document.getElementById(`btn-cancel-add-client`);s&&o&&s.addEventListener(`click`,()=>{o.style.display=`none`});let c=document.getElementById(`btn-save-new-client`);c&&c.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-ref`).value.trim(),n=document.getElementById(`new-cli-phone`).value.trim(),r=document.getElementById(`new-cli-email`).value.trim(),a=document.getElementById(`new-cli-city`).value.trim(),o=document.getElementById(`new-cli-address`).value.trim();if(!e){alert(`Compila la Ragione Sociale del Cliente!`);return}try{i.addClient({name:e,refPerson:t,phone:n,email:r,city:a,address:o}),alert(`✅ Cliente "${e}" salvato in Anagrafica!`),h()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{m.editingId=e.getAttribute(`data-id`),h()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Eliminare questo cliente dall'anagrafica? Le macchine collegate torneranno in magazzino.`))try{i.deleteClient(t),h()}catch(e){alert(e.message)}})});let l=document.getElementById(`form-edit-client`);l&&l.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value,r=document.getElementById(`edit-cli-ref`).value,a=document.getElementById(`edit-cli-phone`).value,o=document.getElementById(`edit-cli-city`).value,s=document.getElementById(`edit-cli-address`).value;try{i.updateClient(t,{name:n,refPerson:r,phone:a,city:o,address:s}),m.editingId=null,alert(`✅ Scheda Cliente salvata!`),h()}catch(e){alert(e.message)}});let u=document.getElementById(`btn-toggle-add-machine`),d=document.getElementById(`add-machine-form-container`);u&&d&&u.addEventListener(`click`,()=>{d.style.display=d.style.display===`none`?`block`:`none`});let f=document.getElementById(`btn-cancel-add-machine`);f&&d&&f.addEventListener(`click`,()=>{d.style.display=`none`});let p=document.getElementById(`btn-save-new-machine`);p&&p.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Compila Seriale e Modello della macchina!`);return}try{i.addMachine({serialNumber:e,brand:t,model:n,clientId:r}),alert(`✅ Macchina "${e}" registrata nel parco macchine!`),h()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{m.editingId=e.getAttribute(`data-id`),h()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Eliminare questa macchina dal parco macchine?`))try{i.deleteMachine(t),h()}catch(e){alert(e.message)}})});let g=document.getElementById(`form-edit-machine`);g&&g.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value,r=document.getElementById(`edit-mc-brand`).value,a=document.getElementById(`edit-mc-model`).value,o=document.getElementById(`edit-mc-client`).value;try{i.updateMachine(t,{serialNumber:n,brand:r,model:a,clientId:o}),m.editingId=null,alert(`✅ Scheda Macchina da Caffè salvata!`),h()}catch(e){alert(e.message)}});let _=document.getElementById(`btn-toggle-add-board`),v=document.getElementById(`add-board-form-container`);_&&v&&_.addEventListener(`click`,()=>{v.style.display=v.style.display===`none`?`block`:`none`});let y=document.getElementById(`btn-cancel-add-board`);y&&v&&y.addEventListener(`click`,()=>{v.style.display=`none`});let b=document.getElementById(`btn-save-new-board`);b&&b.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,a=document.getElementById(`new-board-machine`).value;if(!e){alert(`Inserisci il codice a 4 cifre per la Scheda Deconto!`);return}try{i.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,machineId:a}),alert(`✅ Scheda Deconto #${e} registrata con successo!`),h()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{m.editingId=e.getAttribute(`data-id`),h()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Eliminare questa Scheda Hardware Deconto?`))try{i.deleteBoard(t),h()}catch(e){alert(e.message)}})});let x=document.getElementById(`form-edit-board`);x&&x.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value,r=document.getElementById(`edit-board-hwserial`).value,a=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-threshold`).value,s=document.getElementById(`edit-board-version`).value,c=document.getElementById(`edit-board-machine`).value;try{i.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:a,lowStockThreshold:o,version:s,machineId:c}),m.editingId=null,alert(`✅ Scheda Deconto aggiornata con successo!`),h()}catch(e){alert(e.message)}}),document.querySelectorAll(`#btn-close-edit-modal, #btn-cancel-edit-client, #btn-cancel-edit-mc, #btn-cancel-edit-board`).forEach(e=>{e.addEventListener(`click`,()=>{m.editingId=null,h()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{m.viewingDecontoCode=e.getAttribute(`data-code`),h()})});let S=document.getElementById(`btn-close-deconto-modal`),C=document.getElementById(`btn-close-deconto-modal-footer`);S&&S.addEventListener(`click`,()=>{m.viewingDecontoCode=null,h()}),C&&C.addEventListener(`click`,()=>{m.viewingDecontoCode=null,h()});let w=document.getElementById(`sim-board-select`);w&&w.addEventListener(`change`,e=>{m.selectedSimBoardCode=e.target.value,h()});let T=document.getElementById(`btn-sim-brew`);T&&T.addEventListener(`click`,()=>{let e=m.selectedSimBoardCode||`9901`;document.getElementById(`signal-sense-volts`).innerText=`230V AC (Impulso)`,document.getElementById(`signal-sense-badge`).className=`badge badge-warning`,document.getElementById(`signal-sense-badge`).innerText=`EROGAZIONE IN CORSO`;let t=i.registerCoffeeExtraction(e,22,1);setTimeout(()=>{if(t&&t.success){let n=document.getElementById(`sim-console-log`);n&&(n.innerHTML+=`[EXTRACTION]: Caffè erogato su #${e}! Credito rimanente: ${t.remainingCredits}.<br>`,n.scrollTop=n.scrollHeight)}h()},600)});let E=document.getElementById(`btn-sim-reset`);E&&E.addEventListener(`click`,()=>{let e=m.selectedSimBoardCode||`9901`;i.performRefill({boardShortCode:e,credits:200,method:`TEST_BENCH`,operatorId:m.currentUser?m.currentUser.id:`usr_001`}),alert(`✅ Ricaricate +200 cialde di prova sulla macchina #${e}!`),h()})}document.addEventListener(`DOMContentLoaded`,h);
...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};function o(){let e=a.getSettings();return`
  ...
  `}function x(e=null){let t=a.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return a.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function S(e,t=null,n=null){let r=a.getUsers(),i=a.getRoleLabels(),o=a.getPermissions(),s=a.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
  ...
  `}function w(){let e=a.getSettings();return`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var T={currentUser:a.getCurrentUser(),activeTab:`dashboard`,editingId:null,editingStaffUserId:null,showProfileModal:!1,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingDecontoCode:null,viewingEmailId:null,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`,generatedOtpUrl:null,generatedOtpToken:null,diyParams:null};function E(){let e=document.getElementById(`app`);if(!T.currentUser){e.innerHTML=o(),D();return}let t=``;switch(T.activeTab){case`dashboard`:t=c(T.activeTab,T.viewingDecontoCode,T.dashSearchQuery,T.dashSearchCategory,T.dashSortColumn,T.dashSortDirection,T.viewingKpiModal,T.kpiPeriod,T.kpiChartType,T.kpiCustomStart,T.kpiCustomEnd);break;case`clients`:case`machines`:case`deconto_boards`:t=_(T.activeTab,T.editingId);break;case`adr_visits`:t=v();break;case`otp_generator`:t=b(T.generatedOtpUrl,T.generatedOtpToken);break;case`client_diy`:t=y(T.diyParams);break;case`qr_generator`:t=`
      ...
      `;break;case`simulator`:t=x();break;case`user_mgmt`:case`user_management`:case`permissions_matrix`:t=S(T.activeTab,T.editingStaffUserId,T.viewingEmailId);break;case`settings`:t=w();break;default:t=c()}let n=C(T.showProfileModal,T.currentUser);e.innerHTML=`
    <div class="app-layout">
      ${s(T.currentUser,T.activeTab)}
      <main class="main-content">
        ${t}
      </main>
    </div>
    ${n}
  `,O()}function D(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value.trim();try{let e=a.login(t,n);T.currentUser=e,T.activeTab=e.role===`ADR`?`adr_visits`:`dashboard`,E()}catch(e){alert(e.message)}})}function O(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(T.activeTab=t,T.editingId=null,T.editingStaffUserId=null,T.viewingDecontoCode=null,T.viewingEmailId=null,E())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{T.showProfileModal=!0,E()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{T.showProfileModal=!1,E()}),n&&n.addEventListener(`click`,()=>{T.showProfileModal=!1,E()});let r=document.getElementById(`profile-edit-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-new-password`).value;try{T.currentUser=a.updateUserProfile(T.currentUser.id,{name:t,email:n,newPassword:r?r.trim():void 0}),T.showProfileModal=!1,alert(`✅ Credenziali del profilo aggiornate con successo!`),E()}catch(e){alert(`Errore: ${e.message}`)}});let i=document.getElementById(`btn-logout`);i&&i.addEventListener(`click`,()=>{a.logout(),T.currentUser=null,T.activeTab=`dashboard`,E()});let o=document.getElementById(`btn-toggle-add-client`),s=document.getElementById(`add-client-form-container`),c=document.getElementById(`btn-cancel-add-client`),l=document.getElementById(`btn-save-new-client`);o&&s&&o.addEventListener(`click`,()=>{let e=s.style.display===`none`||!s.style.display;s.style.display=e?`block`:`none`}),c&&s&&c.addEventListener(`click`,()=>{s.style.display=`none`}),l&&l.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-type`).value,n=document.getElementById(`new-cli-ref`).value.trim(),r=document.getElementById(`new-cli-phone`).value.trim(),i=document.getElementById(`new-cli-email`).value.trim(),o=document.getElementById(`new-cli-city`).value.trim(),s=document.getElementById(`new-cli-machine`).value;if(!e){alert(`Inserisci la Ragione Sociale / Nome Cliente!`);return}try{let c=a.addClient({name:e,clientType:t,refPerson:n,phone:r,email:i,city:o,machineId:s});alert(`✅ Cliente "${c.name}" registrato con successo!`),E()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{T.editingId=e.getAttribute(`data-id`),E()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo cliente?`))try{a.deleteClient(t),alert(`✅ Cliente eliminato dall'anagrafica!`),E()}catch(e){alert(`Errore: ${e.message}`)}})});let u=document.getElementById(`btn-cancel-edit-client`);u&&u.addEventListener(`click`,()=>{T.editingId=null,E()});let d=document.getElementById(`form-edit-client`);d&&d.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value.trim(),r=document.getElementById(`edit-cli-type`).value,i=document.getElementById(`edit-cli-ref`).value.trim(),o=document.getElementById(`edit-cli-phone`).value.trim(),s=document.getElementById(`edit-cli-city`).value.trim(),c=document.getElementById(`edit-cli-address`).value.trim(),l=document.getElementById(`edit-cli-machine`).value;try{a.updateClient(t,{name:n,clientType:r,refPerson:i,phone:o,city:s,address:c,assignedMachineId:l}),T.editingId=null,alert(`✅ Scheda Cliente salvata con successo!`),E()}catch(e){alert(`Errore: ${e.message}`)}});let f=document.getElementById(`btn-toggle-add-machine`),p=document.getElementById(`add-machine-form-container`),m=document.getElementById(`btn-cancel-add-machine`),h=document.getElementById(`btn-save-new-machine`);f&&p&&f.addEventListener(`click`,()=>{let e=p.style.display===`none`||!p.style.display;p.style.display=e?`block`:`none`}),m&&p&&m.addEventListener(`click`,()=>{p.style.display=`none`}),h&&h.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-year`).value.trim(),i=document.getElementById(`new-mc-board`).value,o=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Inserisci Seriale Macchina e Modello!`);return}try{let s=a.addMachine({serialNumber:e,brand:t,model:n,productionYear:r,boardId:i,clientId:o});alert(`✅ Macchina da caffè SN "${s.serialNumber}" registrata con successo!`),E()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{T.editingId=e.getAttribute(`data-id`),E()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa macchina da caffè?`))try{a.deleteMachine(t),alert(`✅ Macchina eliminata dal parco macchine!`),E()}catch(e){alert(`Errore: ${e.message}`)}})});let g=document.getElementById(`btn-cancel-edit-mc`);g&&g.addEventListener(`click`,()=>{T.editingId=null,E()});let _=document.getElementById(`form-edit-machine`);_&&_.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value.trim(),r=document.getElementById(`edit-mc-brand`).value.trim(),i=document.getElementById(`edit-mc-model`).value.trim(),o=document.getElementById(`edit-mc-year`).value.trim(),s=document.getElementById(`edit-mc-board`).value,c=document.getElementById(`edit-mc-client`).value;try{a.updateMachine(t,{serialNumber:n,brand:r,model:i,productionYear:o,boardId:s,clientId:c}),T.editingId=null,alert(`✅ Scheda Macchina salvata con successo!`),E()}catch(e){alert(`Errore: ${e.message}`)}});let v=document.getElementById(`btn-toggle-add-board`),y=document.getElementById(`add-board-form-container`),b=document.getElementById(`btn-cancel-add-board`),x=document.getElementById(`btn-save-new-board`);v&&y&&v.addEventListener(`click`,()=>{let e=y.style.display===`none`||!y.style.display;y.style.display=e?`block`:`none`}),b&&y&&b.addEventListener(`click`,()=>{y.style.display=`none`}),x&&x.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,i=document.getElementById(`new-board-machine`).value,o=document.getElementById(`new-board-groupcount`),s=o?parseInt(o.value,10):1;if(!e){alert(`Inserisci il Codice a 4 cifre del Deconto!`);return}try{let o=a.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,groupCount:s,machineId:i});alert(`✅ Scheda Deconto #${o.shortCode} salvata PERMANENTEMENTE nel database!`),E()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{T.editingId=e.getAttribute(`data-id`),E()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa scheda Deconto?`))try{a.deleteBoard(t),alert(`✅ Scheda Deconto eliminata!`),E()}catch(e){alert(`Errore: ${e.message}`)}})});let S=document.getElementById(`btn-cancel-edit-board`);S&&S.addEventListener(`click`,()=>{T.editingId=null,E()});let C=document.getElementById(`form-edit-board`);C&&C.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value.trim(),r=document.getElementById(`edit-board-hwserial`).value.trim(),i=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-version`).value,s=document.getElementById(`edit-board-machine`).value,c=document.getElementById(`edit-board-groupcount`),l=c?parseInt(c.value,10):1;try{a.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:i,version:o,groupCount:l,machineId:s}),T.editingId=null,alert(`✅ Scheda Deconto salvata con successo!`),E()}catch(e){alert(`Errore: ${e.message}`)}});let w=document.getElementById(`btn-close-edit-modal`);w&&w.addEventListener(`click`,()=>{T.editingId=null,E()});let D=document.getElementById(`btn-toggle-add-user`),O=document.getElementById(`add-user-form-container`),k=document.getElementById(`btn-cancel-add-user`),ee=document.getElementById(`btn-save-new-user`);D&&O&&D.addEventListener(`click`,()=>{let e=O.style.display===`none`||!O.style.display;O.style.display=e?`block`:`none`}),k&&O&&k.addEventListener(`click`,()=>{O.style.display=`none`}),ee&&ee.addEventListener(`click`,()=>{let e=document.getElementById(`new-user-username`).value.trim(),t=document.getElementById(`new-user-password`).value.trim(),n=document.getElementById(`new-user-name`).value.trim(),r=document.getElementById(`new-user-role`).value,i=document.getElementById(`new-user-email`).value.trim(),o=document.getElementById(`new-user-phone`).value.trim();if(!e||!t||!n){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{a.addUser({username:e,password:t,name:n,role:r,email:i,phone:o}),alert(`✅ Utente dipendente "${n}" (Codice ${e}) salvato PERMANENTEMENTE nel database!`),E()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{T.editingStaffUserId=e.getAttribute(`data-id`),E()})});let te=document.getElementById(`btn-close-edit-staff-modal`),ne=document.getElementById(`btn-cancel-edit-staff`);te&&te.addEventListener(`click`,()=>{T.editingStaffUserId=null,E()}),ne&&ne.addEventListener(`click`,()=>{T.editingStaffUserId=null,E()});let re=document.getElementById(`edit-staff-form`);re&&re.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,i=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=a.updateUser(t,{username:n,name:r,role:i,email:o,phone:s,password:c?c.trim():void 0});T.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),E()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;a.updateUser(t,{status:n}),E()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{a.deleteUser(t),E()}catch(e){alert(`Errore: ${e.message}`)}})});let A=document.getElementById(`btn-open-email-logs`);A&&A.addEventListener(`click`,()=>{let e=a.getEmailLogs();e.length>0?(T.viewingEmailId=e[0].id,E()):alert(`Nessuna email spedita di recente nel registro.`)});let j=document.getElementById(`btn-close-email-preview`),M=document.getElementById(`btn-close-email-preview-footer`);j&&j.addEventListener(`click`,()=>{T.viewingEmailId=null,E()}),M&&M.addEventListener(`click`,()=>{T.viewingEmailId=null,E()});let N=document.getElementById(`rename-role-labels-form`);N&&N.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();a.updateRoleLabel(`UFFICIO`,t),a.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),E()});let P=document.getElementById(`permissions-matrix-form`);P&&P.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),a.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),E()});let F=document.getElementById(`setting-logo-file`);F&&F.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;a.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),E()},e.readAsDataURL(t)}});let I=document.getElementById(`btn-reset-logo`);I&&I.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(a.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),E())});let L=document.getElementById(`settings-brand-form`);L&&L.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();a.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),E()});let R=document.getElementById(`settings-thresholds-form`);R&&R.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}a.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),E()});let z=document.getElementById(`settings-brevo-form`);z&&z.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();a.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),E()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{T.viewingKpiModal=e.getAttribute(`data-kpi`),E()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{T.viewingKpiModal=null,E()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{T.kpiPeriod=e.getAttribute(`data-period`),E()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{T.kpiChartType=e.getAttribute(`data-charttype`),E()})});let B=document.getElementById(`btn-apply-kpi-custom-date`);B&&B.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(T.kpiCustomStart=e,T.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),E()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let V=document.getElementById(`btn-dash-search`),H=document.getElementById(`dash-search-input`);V&&H&&(V.addEventListener(`click`,()=>{T.dashSearchQuery=H.value,T.dashSearchCategory=document.getElementById(`dash-search-category`).value,E()}),H.addEventListener(`keypress`,e=>{e.key===`Enter`&&(T.dashSearchQuery=H.value,T.dashSearchCategory=document.getElementById(`dash-search-category`).value,E())}));let U=document.getElementById(`btn-dash-reset`);U&&U.addEventListener(`click`,()=>{T.dashSearchQuery=``,T.dashSearchCategory=`ALL`,E()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);T.dashSortColumn===t?T.dashSortDirection=T.dashSortDirection===`ASC`?`DESC`:`ASC`:(T.dashSortColumn=t,T.dashSortDirection=`DESC`),E()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{T.viewingDecontoCode=e.getAttribute(`data-code`),E()})});let W=document.getElementById(`btn-close-deconto-modal`),G=document.getElementById(`btn-close-deconto-modal-footer`);W&&W.addEventListener(`click`,()=>{T.viewingDecontoCode=null,E()}),G&&G.addEventListener(`click`,()=>{T.viewingDecontoCode=null,E()});let K=document.getElementById(`btn-export-csv`);K&&K.addEventListener(`click`,()=>{let e=a.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_erogazioni_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let q=document.getElementById(`btn-trigger-backup`);q&&q.addEventListener(`click`,async()=>{try{q.disabled=!0,q.innerHTML=`⏳ Backup in Corso...`;let e=a.triggerGitHubBackup();alert(`✅ Backup Cloud completato!\nID: ${e.id}\nCommit: ${e.commitHash}\nRecords: ${e.recordCount}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{q.disabled=!1,q.innerHTML=`☁️ Esegui Backup GitHub`}});let J=document.getElementById(`otp-client-select`),Y=document.getElementById(`otp-machine-select`),X=document.getElementById(`otp-board-select`),Z=document.getElementById(`otp-credits-input`),Q=document.getElementById(`otp-custom-credits-wrapper`);Z&&Q&&Z.addEventListener(`change`,()=>{Z.value===`CUSTOM`?Q.style.display=`block`:Q.style.display=`none`}),J&&Y&&X&&(J.addEventListener(`change`,()=>{let e=J.value;if(!e){Y.value=``,X.value=``;return}let t=a.getMachines().find(t=>t.clientId===e);if(t){Y.value=t.id;let e=a.getBoards().find(e=>e.machineId===t.id);e?X.value=e.shortCode:X.value=``}else Y.value=``,X.value=``}),Y.addEventListener(`change`,()=>{let e=Y.value;if(!e){J.value=``,X.value=``;return}let t=a.getMachines().find(t=>t.id===e);if(t){J.value=t.clientId||``;let e=a.getBoards().find(e=>e.machineId===t.id);e?X.value=e.shortCode:X.value=``}}),X.addEventListener(`change`,()=>{let e=X.value;if(!e){Y.value=``,J.value=``;return}let t=a.getBoards().find(t=>t.shortCode===e);if(t&&t.machineId){Y.value=t.machineId;let e=a.getMachines().find(e=>e.id===t.machineId);e?J.value=e.clientId||``:J.value=``}else Y.value=``,J.value=``}));let ie=document.getElementById(`btn-generate-otp-link`);ie&&ie.addEventListener(`click`,()=>{let e=document.getElementById(`otp-client-select`).value,t=document.getElementById(`otp-board-select`).value,n=document.getElementById(`otp-credits-input`).value,r=document.getElementById(`otp-expiry-input`).value;if(!e||!t){alert(`Seleziona sia il Cliente che la Scheda Deconto!`);return}let i=200;if(n===`CUSTOM`){let e=parseInt(document.getElementById(`otp-custom-credits-value`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per i crediti personalizzati!`);return}i=e}else i=parseInt(n,10);let o=a.getClients().find(t=>t.id===e),s=o?o.name:`Cliente`;if(!r){let e=new Date;e.setMonth(e.getMonth()+1),r=e.toISOString().split(`T`)[0]}let c=Math.random().toString(36).substring(2,7).toUpperCase(),l=`OTP-${Math.floor(1e3+Math.random()*9e3)}-${c}`;T.generatedOtpUrl=`${window.location.origin+window.location.pathname}?tab=client_diy&clientName=${encodeURIComponent(s)}&board=${t}&otp=${l}&credits=${i}`,T.generatedOtpToken=l,T.selectedClientTemp=e,T.selectedMachineTemp=document.getElementById(`otp-machine-select`).value,T.selectedBoardTemp=t,T.selectedCreditsTemp=n,T.selectedCustomCreditsTemp=n===`CUSTOM`?i:``,T.selectedExpiryTemp=r,E();let u=document.getElementById(`otp-client-select`),d=document.getElementById(`otp-machine-select`),f=document.getElementById(`otp-board-select`),p=document.getElementById(`otp-credits-input`),m=document.getElementById(`otp-custom-credits-wrapper`),h=document.getElementById(`otp-custom-credits-value`),g=document.getElementById(`otp-expiry-input`);u&&(u.value=T.selectedClientTemp),d&&(d.value=T.selectedMachineTemp),f&&(f.value=T.selectedBoardTemp),p&&(p.value=T.selectedCreditsTemp),p&&p.value===`CUSTOM`&&m&&(m.style.display=`block`,h&&(h.value=T.selectedCustomCreditsTemp)),g&&(g.value=T.selectedExpiryTemp),setTimeout(()=>{let e=document.getElementById(`otp-result-container`);e&&e.scrollIntoView({behavior:`smooth`})},50)});let ae=document.getElementById(`btn-copy-otp-link`);ae&&ae.addEventListener(`click`,()=>{let e=document.getElementById(`generated-otp-url`);e&&(e.select(),document.execCommand(`copy`),alert(`📋 Link di ricarica copiato negli appunti!`))});let oe=document.getElementById(`btn-whatsapp-otp-send`);if(oe&&T.generatedOtpUrl){let e=document.getElementById(`otp-board-select`)?.value||`Deconto`,t=`Ciao! Ecco il link per ricaricare +${document.getElementById(`otp-credits-input`)?.value||`200`} caffè sulla tua macchina Deconto #${e}: ${T.generatedOtpUrl}`;oe.setAttribute(`href`,`https://wa.me/?text=${encodeURIComponent(t)}`)}let se=document.getElementById(`btn-test-otp-link`);se&&T.generatedOtpUrl&&se.addEventListener(`click`,()=>{let e=new URLSearchParams(T.generatedOtpUrl.split(`?`)[1]);T.diyParams={clientName:e.get(`clientName`),boardShortCode:e.get(`board`),tokenOtp:e.get(`otp`),credits:parseInt(e.get(`credits`),10),success:!1},T.activeTab=`client_diy`,E()});let $=document.getElementById(`btn-client-diy-refill`);$&&$.addEventListener(`click`,()=>{let e=$.getAttribute(`data-board`),t=parseInt($.getAttribute(`data-credits`),10),n=$.getAttribute(`data-otp`),r=document.getElementById(`diy-status-msg`);r&&(r.innerHTML=`<span style="color: var(--accent-cyan); font-weight: 700;">📡 Connessione Bluetooth in corso... Sincronizzazione relè...</span>`),$.disabled=!0,setTimeout(()=>{try{a.performRefill({boardShortCode:e,credits:t,method:`WHATSAPP_OTP_BLE`,tokenOtp:n}),T.diyParams?T.diyParams.success=!0:T.diyParams={success:!0,clientName:`Studio Legale Brambilla`,boardShortCode:e,tokenOtp,credits:t},E()}catch(e){alert(`Errore durante l'accredito: ${e.message}`),r&&(r.innerHTML=`<span style="color: var(--accent-rose); font-weight: 700;">❌ Errore BLE: ${e.message}</span>`),$.disabled=!1}},1200)});let ce=document.getElementById(`btn-back-to-office-from-diy`);ce&&ce.addEventListener(`click`,()=>{T.diyParams=null,T.generatedOtpUrl=null,T.generatedOtpToken=null,T.activeTab=`otp_generator`,E()})}function k(){let e=new URLSearchParams(window.location.search);e.get(`tab`)===`client_diy`&&(T.activeTab=`client_diy`,T.diyParams={clientName:e.get(`clientName`)||`Studio Legale Brambilla`,boardShortCode:e.get(`board`)||`3467`,tokenOtp:e.get(`otp`)||`OTP-9981-X79K2`,credits:parseInt(e.get(`credits`),10)||200,success:!1},window.history.replaceState({},document.title,window.location.pathname))}k(),E();
(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{async sendWelcomeEmail(e){let t=i.getRoleLabels()[e.role]||e.role,n=i.getSettings(),r=`👋 Benvenuto nel Team ${n.brandTitle||`DECONTO`} - Credenziali di Accesso`,a=`
    ...
    `,o={id:`mail_`+Date.now(),type:`WELCOME_NEW_USER`,recipientEmail:e.email||`${e.username}@deconto.it`,recipientName:e.name,subject:r,htmlBody:a,timestamp:new Date().toISOString(),status:`DELIVERED`};return i.data.emailLogs||(i.data.emailLogs=[]),i.data.emailLogs.unshift(o),i.saveData(),o}async sendRoleUpdateEmail(e,t,n){let r=i.getRoleLabels(),a=r[t]||t,o=r[n]||n,s=i.getSettings(),c=`🎉 Aggiornamento Ruolo Operativo & Nuovi Permessi - ${s.brandTitle||`DECONTO`}`,l=`
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0f172a; color: #f8fafc; padding: 24px; border-radius: 16px; border: 1px solid #334155;">
        <div style="text-align: center; margin-bottom: 24px; border-bottom: 1px solid #334155; padding-bottom: 16px;">
...
`;return this.data.coffeeLogs.forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}},a=new class{constructor(){this.isSupported=typeof navigator<`u`&&`bluetooth`in navigator,this.connectedDevice=null}checkSupport(){return this.isSupported}async connectToBoardByShortCode(e){if(console.log(`📡 Ricerca dispositivo Deconto con codice breve [${e}]...`),this.isSupported&&navigator.bluetooth)try{let t=await navigator.bluetooth.requestDevice({filters:[{namePrefix:`DECONTO_${e}`}],optionalServices:[`0000ffe0-0000-1000-8000-00805f9b34fb`]});return this.connectedDevice=t,{success:!0,deviceName:t.name,isRealHardware:!0}}catch(e){console.warn(`Fallback a simulazione BLE locale:`,e.message)}return await new Promise(e=>setTimeout(e,1500)),{success:!0,deviceName:`DECONTO_${e}`,shortCode:e,isRealHardware:!1,connectedAt:new Date().toISOString()}}async sendRefillOtpToken(e,t,n){if(!(await this.connectToBoardByShortCode(e)).success)throw Error(`Impossibile connettersi al dispositivo DECONTO_${e}`);return await new Promise(e=>setTimeout(e,1e3)),{success:!0,shortCode:e,creditsAccredited:t,tokenApplied:n,relayStatus:`CLOSED_OK`,timestamp:new Date().toISOString()}}},o=new class{constructor(){this.repoUrl=`https://github.com/deconto-org/deconto-db-backups`}generateDatabaseSnapshot(){return{version:`1.0.0`,timestamp:new Date().toISOString(),data:i.data}}async executeBackupNow(){let e=this.generateDatabaseSnapshot(),t=JSON.stringify(e,null,2);return await new Promise(e=>setTimeout(e,1200)),{success:!0,backupRecord:i.triggerGitHubBackup(),sizeBytes:new Blob([t]).size,snapshotTimestamp:e.timestamp}}};function s(e,t){let n=e||{name:`Utente Ospite`,role:`ADMIN`,username:`001`,avatar:`👨‍💼`},r=i.getPermissions(),a=i.getSettings(),o=[];if(n.role===`ADMIN`)o=[{id:`dashboard`,label:`📊 Dashboard BI`,icon:`📈`},{id:`user_management`,label:`👥 Gestione Personale`,icon:`👤`},{id:`permissions_matrix`,label:`⚙️ Matrice Permessi`,icon:`🔐`},{id:`clients`,label:`🏢 Clienti & Parco`,icon:`🏢`},{id:`qr_generator`,label:`🏷️ Generatore Etichette QR`,icon:`🖨️`},{id:`otp_generator`,label:`🔑 Genera Ricariche OTP`,icon:`💬`},{id:`refills_history`,label:`📋 Storico Ricariche`,icon:`🧾`},{id:`adr_visits`,label:`🗺️ Giro Consegne ADR`,icon:`🚚`},{id:`maintenance`,label:`🛠️ Manutenzione Predittiva`,icon:`⚠️`},{id:`backups`,label:`💾 Backup GitHub`,icon:`🐙`},{id:`simulator`,label:`☕ Simulatore Macchina HW`,icon:`⚡`},{id:`settings`,label:`⚙️ Impostazioni`,icon:`🛠️`}];else{let e=r[n.role]||{};e.canViewClients&&o.push({id:`clients`,label:`🏢 Anagrafica Clienti`,icon:`🏢`}),e.canGenerateQr&&o.push({id:`qr_generator`,label:`🏷️ Generatore Etichette QR`,icon:`🖨️`}),e.canGenerateOtp&&o.push({id:`otp_generator`,label:`🔑 Genera Ricarica OTP`,icon:`💬`}),(e.canBleRefill||n.role===`ADR`)&&o.push({id:`adr_visits`,label:`🗺️ Giro Consegne & BLE`,icon:`🚚`}),e.canViewRefillHistory&&o.push({id:`refills_history`,label:`📋 Storico Ricariche`,icon:`🧾`}),e.canUseSimulator&&o.push({id:`simulator`,label:`☕ Simulatore Macchina HW`,icon:`⚡`}),o.push({id:`settings`,label:`⚙️ Impostazioni`,icon:`🛠️`})}return`
  ...
  `}function c(){let e=i.getSettings();return`
  ...
  `}function u(e,t=null,n=``,r=`ALL`,a=`shortCode`,o=`DESC`,s=null,c=`30DAYS`,l=`LINE`){let u=i.getClients(),d=i.getMachines(),f=i.getBoards(),p=i.getCoffeeLogs(),m=u.length,h=d.length,g=p.length,_=f.filter(e=>e.remainingCredits>20),v=f.filter(e=>e.remainingCredits>0&&e.remainingCredits<=20),y=f.filter(e=>e.remainingCredits===0),b=f.filter(e=>{if(!n.trim())return!0;let t=n.toLowerCase().trim(),a=i.getBoardFullDetails(e.id),o=(a&&a.client?a.client.name:``).toLowerCase(),s=(a&&a.machine?a.machine.model:``).toLowerCase(),c=(a&&a.machine?a.machine.serialNumber:``).toLowerCase(),l=String(e.shortCode).toLowerCase(),u=String(e.remainingCredits),d=e.isOnlineWifi?`wi-fi 6 online`:`softap offline`,f=new Date(e.lastSyncDate).toLocaleString(`it-IT`).toLowerCase();return r===`SHORT_CODE`?l.includes(t):r===`CLIENT`?o.includes(t):r===`MODEL`?s.includes(t):r===`CREDITS`?u.includes(t):r===`CONNECTION`?d.includes(t):r===`SYNC_DATE`?f.includes(t):l.includes(t)||o.includes(t)||s.includes(t)||c.includes(t)||u.includes(t)||d.includes(t)||f.includes(t)});b.sort((e,t)=>{let n=i.getBoardFullDetails(e.id),r=i.getBoardFullDetails(t.id),s=n&&n.client?n.client.name:``,c=r&&r.client?r.client.name:``,l=n&&n.machine?n.machine.model:``,u=r&&r.machine?r.machine.model:``,d,f;return a===`shortCode`?(d=parseInt(e.shortCode,10),f=parseInt(t.shortCode,10)):a===`client`?(d=s.toLowerCase(),f=c.toLowerCase()):a===`model`?(d=l.toLowerCase(),f=u.toLowerCase()):a===`credits`?(d=e.remainingCredits,f=t.remainingCredits):a===`connection`?(d=+!!e.isOnlineWifi,f=+!!t.isOnlineWifi):a===`syncDate`?(d=new Date(e.lastSyncDate).getTime(),f=new Date(t.lastSyncDate).getTime()):(d=parseInt(e.shortCode,10),f=parseInt(t.shortCode,10)),d<f?o===`ASC`?-1:1:d>f?o===`ASC`?1:-1:0});let x=e=>a===e?o===`ASC`?`<span style="color: var(--accent-cyan);"> ▲</span>`:`<span style="color: var(--accent-cyan);"> ▼</span>`:`<span style="color: var(--text-dim); opacity: 0.5;"> ⇅</span>`,S=``;if(s===`kpi_clients`){let e={};u.forEach(t=>{let n=t.city||`Milano`;e[n]=(e[n]||0)+1}),S=`
    ...
    ${C}
    ${S}
  `}function d(e,t=null,n=null){let r=i.getUsers(),a=i.getRoleLabels(),o=i.getPermissions(),s=i.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
        ...
        <div class="modal-overlay" id="email-preview-modal">
          <div class="modal-box" style="max-width: 680px; width: 95%;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid var(--border-subtle); padding-bottom: 10px;">
              <div>
                <h3 style="margin: 0; color: var(--accent-cyan);">✉️ Email Spedita: ${e.subject}</h3>
                <div style="font-size: 0.8rem; color: var(--text-muted);">
                  Destinatario: <strong>${e.recipientName} (${e.recipientEmail})</strong> | Data: ${new Date(e.timestamp).toLocaleString(`it-IT`)}
                </div>
              </div>
    ...
    ${c}
  `}function f(e,t=null){let n=i.getClients(),r=i.getBoards(),a=i.getRefillLogs(),o=i.hasPermission(`canCreateClients`),s=i.hasPermission(`canEditClients`),c=i.hasPermission(`canDeleteClients`),l=``;if(t){let e=n.find(e=>e.id===t),r=e?i.getMachines().find(t=>t.clientId===e.id):null,a=r?i.getBoards().find(e=>e.machineId===r.id):null;e&&(l=`
          ...
          <div class="stat-card" style="padding: 24px;">
            ...
            <div style="margin-bottom: 16px;">
              ...
              <select id="qr-board-select" style="width: 100%; padding: 10px; background: var(--bg-primary); color: var(--text-main); border: 1px solid var(--border-color); border-radius: 6px;">
                ${r.map(e=>{let t=i.getBoardFullDetails(e.id);return`<option value="${e.shortCode}">${e.shortCode} - ${t.client?t.client.name:`N/D`} (${t.machine?t.machine.serialNumber:``})</option>`}).join(``)}
              </select>
            </div>
          ...
          <div class="stat-card" style="padding: 24px;">
            ...
            <div style="margin-bottom: 16px;">
              ...
              <select id="otp-board-select" style="width: 100%; padding: 10px; background: var(--bg-primary); color: var(--text-main); border: 1px solid var(--border-color); border-radius: 6px;">
                ${r.map(e=>{let t=i.getBoardFullDetails(e.id);return`<option value="${e.shortCode}">${t.client?t.client.name:`N/D`} (Deconto #${e.shortCode})</option>`}).join(``)}
              </select>
            </div>
    ...
    <div>
      ...
      <div class="table-container">
        <table>
          ...
          <tbody>
            ${n.map(e=>{let t=i.getMachines().find(t=>t.clientId===e.id),n=t?i.getBoards().find(e=>e.machineId===t.id):null;return`
                <tr>
                  <td><strong>${e.name}</strong></td>
    ...
    ${l}
  `}function p(e){return i.getClients(),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function h(){let e=i.getBoards(),t=e[0];return i.getBoardFullDetails(t.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var g={currentUser:i.getCurrentUser(),activeTab:`dashboard`,showProfileModal:!1,editingStaffUserId:null,editingClientId:null,viewingDecontoCode:null,viewingEmailId:null,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`};function _(){let e=document.getElementById(`app`);if(!g.currentUser){e.innerHTML=c(),v();return}let t=g.currentUser,n=``;g.activeTab===`settings`?n=m():g.activeTab===`simulator`?n=h():g.activeTab===`user_management`||g.activeTab===`permissions_matrix`?n=d(g.activeTab,g.editingStaffUserId,g.viewingEmailId):t.role===`ADMIN`?n=g.activeTab===`clients`||g.activeTab===`qr_generator`||g.activeTab===`otp_generator`||g.activeTab===`refills_history`?f(g.activeTab,g.editingClientId):g.activeTab===`adr_visits`?p(g.activeTab):u(g.activeTab,g.viewingDecontoCode,g.dashSearchQuery,g.dashSearchCategory,g.dashSortColumn,g.dashSortDirection,g.viewingKpiModal,g.kpiPeriod,g.kpiChartType):(t.role===`UFFICIO`||t.role===`ADR`)&&(n=g.activeTab===`adr_visits`?p(g.activeTab):f(g.activeTab,g.editingClientId));let r=``;g.showProfileModal&&(r=l(t)),e.innerHTML=`
    ...
    ${r}
  `,y()}function v(){let e=document.getElementById(`login-form`),t=document.getElementById(`login-error-msg`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let n=document.getElementById(`login-username`).value,r=document.getElementById(`login-password`).value;try{let e=i.authenticate(n,r);g.currentUser=e,g.activeTab=e.role===`ADMIN`?`dashboard`:`clients`,_()}catch(e){t.innerText=e.message,t.style.display=`block`}})}function y(){let e=document.getElementById(`btn-logout`);e&&e.addEventListener(`click`,()=>{i.logout(),g.currentUser=null,_()});let t=document.getElementById(`btn-open-profile-modal`);t&&t.addEventListener(`click`,()=>{g.showProfileModal=!0,_()});let n=document.getElementById(`btn-close-profile-modal`),r=document.getElementById(`btn-cancel-profile`);n&&n.addEventListener(`click`,()=>{g.showProfileModal=!1,_()}),r&&r.addEventListener(`click`,()=>{g.showProfileModal=!1,_()});let s=document.getElementById(`profile-edit-form`);s&&s.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-user-name`).value.trim(),n=document.getElementById(`edit-user-username`).value.trim(),r=document.getElementById(`edit-user-email`).value.trim(),a=document.getElementById(`edit-user-password`).value.trim();try{g.currentUser=i.updateUserProfile(g.currentUser.id,{name:t,username:n,email:r,newPassword:a||void 0}),g.showProfileModal=!1,alert(`✅ Credenziali e Profilo aggiornati con successo!`),_()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(g.activeTab=t,_())})});let c=document.getElementById(`btn-open-email-logs`);c&&c.addEventListener(`click`,()=>{let e=i.getEmailLogs();e.length>0?(g.viewingEmailId=e[0].id,_()):alert(`Nessuna email spedita di recente nel registro.`)});let l=document.getElementById(`btn-close-email-preview`),u=document.getElementById(`btn-close-email-preview-footer`);l&&l.addEventListener(`click`,()=>{g.viewingEmailId=null,_()}),u&&u.addEventListener(`click`,()=>{g.viewingEmailId=null,_()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{g.viewingKpiModal=e.getAttribute(`data-kpi`),_()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{g.viewingKpiModal=null,_()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{g.kpiPeriod=e.getAttribute(`data-period`),_()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{g.kpiChartType=e.getAttribute(`data-charttype`),_()})});let d=document.getElementById(`btn-dash-search`),f=document.getElementById(`dash-search-input`);d&&f&&(d.addEventListener(`click`,()=>{g.dashSearchQuery=f.value,g.dashSearchCategory=document.getElementById(`dash-search-category`).value,_()}),f.addEventListener(`keypress`,e=>{e.key===`Enter`&&(g.dashSearchQuery=f.value,g.dashSearchCategory=document.getElementById(`dash-search-category`).value,_())}));let p=document.getElementById(`btn-dash-reset`);p&&p.addEventListener(`click`,()=>{g.dashSearchQuery=``,g.dashSearchCategory=`ALL`,_()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);g.dashSortColumn===t?g.dashSortDirection=g.dashSortDirection===`ASC`?`DESC`:`ASC`:(g.dashSortColumn=t,g.dashSortDirection=`ASC`),_()})});let m=document.getElementById(`setting-logo-file`);m&&m.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;i.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),_()},e.readAsDataURL(t)}});let h=document.getElementById(`btn-reset-logo`);h&&h.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(i.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),_())});let v=document.getElementById(`settings-brand-form`);v&&v.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();i.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),_()}),document.querySelectorAll(`.btn-edit-client`).forEach(e=>{e.addEventListener(`click`,()=>{g.editingClientId=e.getAttribute(`data-id`),_()})});let y=document.getElementById(`btn-close-edit-client-modal`),b=document.getElementById(`btn-cancel-edit-client`);y&&y.addEventListener(`click`,()=>{g.editingClientId=null,_()}),b&&b.addEventListener(`click`,()=>{g.editingClientId=null,_()});let x=document.getElementById(`edit-client-form`);x&&x.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value,r=document.getElementById(`edit-cli-ref`).value,a=document.getElementById(`edit-cli-phone`).value,o=document.getElementById(`edit-cli-city`).value,s=document.getElementById(`edit-cli-address`).value,c=document.getElementById(`edit-cli-mc-model`).value,l=document.getElementById(`edit-cli-mc-serial`).value,u=document.getElementById(`edit-cli-shortcode`).value,d=document.getElementById(`edit-cli-credits`).value,f=document.getElementById(`edit-cli-threshold`).value,p=document.getElementById(`edit-cli-board-version`).value;try{i.updateClientAndMachine(t,{name:n,refPerson:r,phone:a,city:o,address:s,machineModel:c,machineSerial:l,shortCode:u,remainingCredits:d,lowStockThreshold:f,boardVersion:p}),g.editingClientId=null,alert(`✅ Scheda Cliente, Macchina e Deconto aggiornata con successo!`),_()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{g.viewingDecontoCode=e.getAttribute(`data-code`),_()})});let S=document.getElementById(`btn-close-deconto-modal`),C=document.getElementById(`btn-close-deconto-modal-footer`);S&&S.addEventListener(`click`,()=>{g.viewingDecontoCode=null,_()}),C&&C.addEventListener(`click`,()=>{g.viewingDecontoCode=null,_()});let w=document.getElementById(`rename-role-labels-form`);w&&w.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();i.updateRoleLabel(`UFFICIO`,t),i.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),_()});let T=document.getElementById(`btn-toggle-add-user`),E=document.getElementById(`add-user-form-container`);T&&E&&T.addEventListener(`click`,()=>{E.style.display=E.style.display===`none`?`block`:`none`});let D=document.getElementById(`btn-cancel-add-user`);D&&E&&D.addEventListener(`click`,()=>{E.style.display=`none`});let O=document.getElementById(`btn-save-new-user`);O&&O.addEventListener(`click`,()=>{let e=document.getElementById(`new-user-username`).value.trim(),t=document.getElementById(`new-user-password`).value.trim(),n=document.getElementById(`new-user-name`).value.trim(),r=document.getElementById(`new-user-role`).value,a=document.getElementById(`new-user-email`).value.trim(),o=document.getElementById(`new-user-phone`).value.trim();if(!e||!t||!n){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{let s=i.addUser({username:e,password:t,name:n,role:r,email:a,phone:o});alert(`✅ Utente dipendente "${n}" (Codice ${e}) creato con successo!\n\n✉️ Email di Benvenuto generata e spedita a ${s.email}!`),_()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{g.editingStaffUserId=e.getAttribute(`data-id`),_()})});let k=document.getElementById(`btn-close-edit-staff-modal`),A=document.getElementById(`btn-cancel-edit-staff`);k&&k.addEventListener(`click`,()=>{g.editingStaffUserId=null,_()}),A&&A.addEventListener(`click`,()=>{g.editingStaffUserId=null,_()});let j=document.getElementById(`edit-staff-form`);j&&j.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,a=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=i.updateUser(t,{username:n,name:r,role:a,email:o,phone:s,password:c?c.trim():void 0});g.editingStaffUserId=null,alert(`✅ Scheda Utente aggiornata con successo!\n\n✉️ Notifica Email di aggiornamento ruolo spedita a ${e.email}!`),_()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;i.updateUser(t,{status:n}),_()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{i.deleteUser(t),_()}catch(e){alert(`Errore: ${e.message}`)}})});let M=document.getElementById(`permissions-matrix-form`);M&&M.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),i.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),_()});let N=document.getElementById(`btn-export-csv`);N&&N.addEventListener(`click`,()=>{let e=i.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=`DECONTO_Report_Consumi_${new Date().toISOString().split(`T`)[0]}.csv`,r.click(),alert(`📥 Report Consumi CSV Scaricato con successo!`)});let P=document.getElementById(`btn-trigger-backup`);P&&P.addEventListener(`click`,async()=>{P.disabled=!0,P.innerText=`⏳ Backup in corso su GitHub...`;let e=await o.executeBackupNow();alert(`✅ Backup GitHub Eseguito con Successo!\n\nRepository: https://github.com/emporioboldrini-stack/deconto-app.git\nCommit Hash: ${e.backupRecord.commitHash}\nEntità salvate: ${e.backupRecord.recordCount}`),_()});let F=document.getElementById(`btn-toggle-add-client`),I=document.getElementById(`add-client-form-container`);F&&I&&F.addEventListener(`click`,()=>{I.style.display=I.style.display===`none`?`block`:`none`});let L=document.getElementById(`btn-cancel-add-client`);L&&I&&L.addEventListener(`click`,()=>{I.style.display=`none`});let R=document.getElementById(`btn-save-new-client`);R&&R.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-ref`).value.trim(),n=document.getElementById(`new-cli-phone`).value.trim(),r=document.getElementById(`new-cli-city`).value.trim(),a=document.getElementById(`new-cli-mc-model`).value.trim(),o=document.getElementById(`new-cli-code`).value.trim(),s=document.getElementById(`new-cli-credits`).value;if(!e||!t||!n){alert(`Compila i campi obbligatori: Nome Cliente, Referente e Telefono!`);return}try{i.addClient({name:e,refPerson:t,phone:n,city:r,address:r,machineModel:a||`Didiesse Frog Revolution`,shortCode:o||`${Math.floor(1e3+Math.random()*9e3)}`,initialCredits:s}),alert(`✅ Cliente "${e}" registrato con successo ed associato alla scheda Deconto!`),_()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-del-client`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler rimuovere questo cliente dal sistema?`))try{i.deleteClient(t),_()}catch(e){alert(`Errore: ${e.message}`)}})});let z=document.getElementById(`btn-generate-otp`);z&&z.addEventListener(`click`,()=>{let e=document.getElementById(`otp-board-select`).value,t=parseInt(document.getElementById(`otp-credits-select`).value,10),n=`OTP-${Math.floor(1e3+Math.random()*9e3)}-${Math.random().toString(36).substring(2,7).toUpperCase()}`,r=`https://deconto-vending-app.web.app/?short=${e}&otp=${n}&c=${t}`;document.getElementById(`otp-code-val`).innerText=n,document.getElementById(`otp-link-val`).innerText=r,alert(`✅ Token OTP Generato per Deconto #${e} (+${t} Caffè)!`)});let B=document.getElementById(`btn-send-whatsapp`);B&&B.addEventListener(`click`,()=>{let e=`Gentile cliente, ecco il link per ricaricare la tua macchina da caffè Deconto: ${document.getElementById(`otp-link-val`).innerText}`;window.open(`https://wa.me/?text=${encodeURIComponent(e)}`,`_blank`)});let V=document.getElementById(`btn-copy-otp-link`);V&&V.addEventListener(`click`,()=>{let e=document.getElementById(`otp-link-val`).innerText;navigator.clipboard.writeText(e),alert(`📋 Link Ricarica Copiato negli appunti!`)});let H=document.getElementById(`btn-print-qr`);H&&H.addEventListener(`click`,()=>{window.print()});let U=document.getElementById(`qr-header-input`);U&&U.addEventListener(`input`,e=>{document.getElementById(`lbl-header-title`).innerText=`☕ ${e.target.value.toUpperCase()} ☕`});let W=document.getElementById(`qr-board-select`);W&&W.addEventListener(`change`,e=>{let t=i.getBoardFullDetails(e.target.value);t&&(document.getElementById(`lbl-short-code-display`).innerText=t.board.shortCode,document.getElementById(`lbl-mc-sn`).innerText=t.machine?t.machine.serialNumber:`N/D`,document.getElementById(`lbl-hw-sn`).innerText=t.board.hwSerial)});let G=document.getElementById(`btn-adr-ble-connect`);G&&G.addEventListener(`click`,async()=>{let e=document.getElementById(`adr-code-input`).value.trim(),t=parseInt(document.getElementById(`adr-credits-select`).value,10),n=document.getElementById(`adr-status-box`);if(!e){alert(`Inserisci il codice a 4 cifre!`);return}n.style.display=`block`,n.innerHTML=`📡 Scansione Bluetooth BLE per <strong>DECONTO_${e}</strong> in corso...`;try{await a.sendRefillOtpToken(e,t,`ADR_BLE_MANUAL`),i.performRefill({boardShortCode:e,credits:t,method:`BLE_PWA`,operatorId:g.currentUser?g.currentUser.id:`usr_003`}),n.innerHTML=`<span style="color: var(--accent-green);">✅ Ricarica Completata! Accreditate <strong>+${t} cialde</strong> sulla macchina #${e}. Relè Ripristinato.</span>`,setTimeout(()=>_(),2e3)}catch(e){n.innerHTML=`<span style="color: var(--accent-rose);">❌ Errore connessione: ${e.message}</span>`}}),document.querySelectorAll(`.btn-adr-quick-fill`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.getAttribute(`data-code`);await a.sendRefillOtpToken(t,200,`ADR_QUICK_BLE`),i.performRefill({boardShortCode:t,credits:200,method:`BLE_PWA`,operatorId:g.currentUser?g.currentUser.id:`usr_003`}),alert(`✅ Ricaricate +200 cialde via Bluetooth sulla macchina #${t}!`),_()})});let K=document.getElementById(`sim-board-select`);K&&K.addEventListener(`change`,e=>{let t=e.target.value,n=i.getBoardFullDetails(t);n&&(document.getElementById(`sim-badge-code`).innerText=`DECONTO ${t}`,document.getElementById(`sim-credits-display`).innerText=n.board.remainingCredits)});let q=document.getElementById(`btn-sim-brew`);q&&q.addEventListener(`click`,()=>{let e=document.getElementById(`sim-board-select`),t=e?e.value:`3467`;document.getElementById(`signal-sense-volts`).innerText=`230V AC (Impulso)`,document.getElementById(`signal-sense-badge`).className=`badge badge-warning`,document.getElementById(`signal-sense-badge`).innerText=`EROGAZIONE IN CORSO`;let n=i.registerCoffeeExtraction(t,22,1);setTimeout(()=>{if(document.getElementById(`signal-sense-volts`).innerText=`0V AC`,document.getElementById(`signal-sense-badge`).className=`badge badge-info`,document.getElementById(`signal-sense-badge`).innerText=`INATTIVO`,n&&n.success){let e=document.getElementById(`sim-console-log`);e.innerHTML+=`[EXTRACTION]: Caffè erogato! Credito rimanente: ${n.remainingCredits}.<br>`,e.scrollTop=e.scrollHeight,n.isLowStock&&(e.innerHTML+=`<span style="color: var(--accent-amber);">[BUZZER 60s]: CREDITO &lt; 20! SEGNALE ACUSTICO ATTIVATO (BIP... BIP...).</span><br>`)}else if(n&&!n.success){let e=document.getElementById(`sim-console-log`);e.innerHTML+=`<span style="color: var(--accent-rose);">[HARDWARE LOCK]: CREDITO 0! RELÈ APERTO. POMPA DISATTIVATA.</span><br>`}_()},800)});let J=document.getElementById(`btn-sim-reset`);J&&J.addEventListener(`click`,()=>{let e=document.getElementById(`sim-board-select`),t=e?e.value:`3467`;i.performRefill({boardShortCode:t,credits:200,method:`TEST_BENCH`,operatorId:g.currentUser?g.currentUser.id:`usr_001`}),alert(`✅ Ricaricate +200 cialde di prova sulla macchina #${t}!`),_()})}document.addEventListener(`DOMContentLoaded`,_);
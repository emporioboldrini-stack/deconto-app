...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};function o(){let e=a.getSettings();return`
  ...
  `}function c(e=`dashboard`,t=null,n=``,r=`ALL`,i=`shortCode`,o=`DESC`,s=null,c=`30DAYS`,l=`LINE`,u=`2026-07-01`,d=`2026-08-02`){let f=a.getClients(),p=a.getMachines(),m=a.getBoards(),h=a.getCoffeeLogs(),g=f.length,_=p.length,v=h.length,y=m.filter(e=>{let t=a.calculateBoardStatus(e);return t.statusKey===`WARNING_LOW`||t.statusKey===`CRITICAL_LOW`||t.statusKey===`BLOCKED_ZERO`}),b=m.filter(e=>{if(!n)return!0;let t=n.toLowerCase().trim(),i=a.getBoardFullDetails(e.id),o=i&&i.client?i.client.name.toLowerCase():``,s=i&&i.machine?i.machine.serialNumber.toLowerCase():``,c=i&&i.machine?i.machine.model.toLowerCase():``,l=e.shortCode.toLowerCase();return r===`CODE`?l.includes(t):r===`CLIENT`?o.includes(t):r===`MODEL`?c.includes(t)||s.includes(t):l.includes(t)||o.includes(t)||c.includes(t)||s.includes(t)});b.sort((e,t)=>{let n=a.getBoardFullDetails(e.id),r=a.getBoardFullDetails(t.id),s,c;return i===`shortCode`?(s=parseInt(e.shortCode,10),c=parseInt(t.shortCode,10)):i===`credits`?(s=e.remainingCredits,c=t.remainingCredits):i===`client`?(s=n&&n.client?n.client.name:``,c=r&&r.client?r.client.name:``):i===`model`?(s=n&&n.machine?n.machine.model:``,c=r&&r.machine?r.machine.model:``):i===`connection`?(s=+!!e.isOnlineWifi,c=+!!t.isOnlineWifi):i===`syncDate`?(s=new Date(e.lastSyncDate).getTime(),c=new Date(t.lastSyncDate).getTime()):(s=e.shortCode,c=t.shortCode),s<c?o===`ASC`?-1:1:s>c?o===`ASC`?1:-1:0});let x=e=>i===e?o===`ASC`?`▲`:`▼`:`<span style="opacity: 0.3;">↕</span>`,S=``;if(s===`kpi_clients`)S=`
    ...
    ${C}
    ${S}
  `}var l=`Abano Terme.Abbiategrasso.Acerra.Acireale.Acqui Terme.Adrano.Afragola.Agrigento.Agropoli.Alba.Albano Laziale.Albenga.Alghero.Altamura.Amalfi.Ancona.Andria.Angri.Anzio.Aosta.Aprilia.Ariano Irpino.Ariccia.Arezzo.Arzignano.Assisi.Asti.Augusta.Aversa.Avezzano.Avola.Avellino.Bagnoli.Bagheria.Barcellona Pozzo di Gotto.Bari.Barletta.Battipaglia.Benevento.Bergamo.Biella.Bitonto.Bologna.Bolzano.Borgomanero.Brindisi.Brescia.Busto Arsizio.Cagliari.Caltanissetta.Campi Bisenzio.Campobasso.Canosa di Puglia.Carpi.Carrara.Caserta.Cassino.Castel Volturno.Castellammare di Stabia.Catania.Catanzaro.Cava de' Tirreni.Cerignola.Cesena.Chieti.Cinisello Balsamo.Cisterna di Latina.Civitavecchia.Collegno.Como.Cosenza.Cremona.Crotone.Cuneo.Desio.Empoli.Ercolano.Fabriano.Faenza.Fano.Ferrara.Firenze.Foggia.Foligno.Forlì.Formia.Frosinone.Gallarate.Gela.Genova.Giugliano in Campania.Gorizia.Grosseto.Guidonia Montecelio.Isernia.L'Aquila.La Spezia.Lamezia Terme.Lanciano.Latina.Lecce.Lecco.Legnano.Livorno.Lodi.Lucca.Macerata.Manduria.Manfredonia.Mantova.Marsala.Massa.Matera.Mazara del Vallo.Mestre.Messina.Milano.Misterbianco.Modena.Mola di Bari.Molfetta.Mondragone.Monreale.Monza.Napoli.Nettuno.Nocera Inferiore.Nola.Novara.Nuoro.Olbia.Ottaviano.Pachino.Padova.Palermo.Parma.Pavia.Perugia.Pesaro.Pescara.Piacenza.Pinerolo.Pistoia.Poggibonsi.Pompei.Portici.Pozzuoli.Prato.Ragusa.Ravenna.Reggio Calabria.Reggio Emilia.Rieti.Rimini.Roma.Rovigo.Salerno.San Benedetto del Tronto.San Giorgio a Cremano.San Giovanni Rotondo.San Severo.Sassari.Savona.Scafati.Sciacca.Senigallia.Seregno.Seriate.Sesto San Giovanni.Settimo Torinese.Siena.Siracusa.Sondrio.Sora.Taranto.Teramo.Terni.Torre Annunziata.Torre del Greco.Tortona.Torino.Trapani.Trani.Trento.Treviglio.Treviso.Trieste.Udine.Varese.Venezia.Verbania.Vercelli.Verona.Vibo Valentia.Vicenza.Vigevano.Vittoria.Viterbo.Vittorio Veneto.Voghera.Abbadia San Salvatore.Agliana.Agropoli.Albano Laziale.Alghero.Altamura.Ariano Irpino.Arzano.Avelino.Bagheria.Bareggio.Beinasco.Bergamo.Bisceglie.Bollate.Brugherio.Brunico.Busto Garolfo.Caivano.Caltagirone.Calvizzano.Canegrate.Capannori.Carate Brianza.Caronno Pertusella.Casamassima.Casarano.Casavatore.Casoria.Casteldaccia.Castellana Grotte.Castelli Calepio.Castel Maggiore.Castelvetrano.Castrovillari.Catanzaro.Ceglie Messapica.Cerignola.Cesano Maderno.Chioggia.Chivasso.Ciampino.Cinisi.Ciriè.Civitanova Marche.Comiso.Concorezzo.Cornaredo.Corsico.Cosentino.Crema.Cutro.Desenzano del Garda.Dronero.Erba.Fasano.Floridia.Foiano della Chiana.Formigine.Frosinone.Gallarate.Gallipoli.Garbagnate Milanese.Gragnano.Gravina in Puglia.Grosseto.Imola.Ivrea.Jesolo.Lainate.Lavagna.Lentini.Lissone.Locate di Triulzi.Loreto.Lucera.Lugo.Magenta.Marghera.Marigliano.Martina Franca.Melito di Napoli.Melzo.Menfi.Merano.Mesagne.Mezzago.Mira.Mirandola.Missaglia.Modica.Moglia.Moncalieri.Montebelluna.Montecorvino Rovella.Montemurlo.Montesarchio.Montevarchi.Monticello Brianza.Mottola.Nardò.Nichelino.Noicattaro.Orbassano.Oristano.Orta Nova.Ostuni.Palagiano.Palmi.Parabiago.Paternò.Patù.Paullo.Peschiera del Garda.Pescina.Piazza Armerina.Pioltello.Polistena.Qualiano.Quarto.Rapallo.Riccione.Rivoli.Rozzano.Rutigliano.San Donà di Piave.San Giorgio Ionico.San Giuliano Milanese.San Lazzaro di Savena.San Nicola la Strada.Sanremo.Sant'Anastasia.Sant'Antimo.Sarno.Saronno.Scandicci.Selvazzano Dentro.Senago.Sestu.Signa.Solaro.Somi.Soresina.Spinea.Spoleto.Stezzano.Strongoli.Sulmona.Taranto.Terracina.Thiene.Tivoli.Tolentino.Torri di Quartesolo.Tradate.Triggiano.Troia.Turi.Uboldo.Uzzano.Valdagno.Valenza.Vallefoglia.Valsamoggia.Venaria Reale.Ventimiglia.Vimodrone.Volla.Zagarolo.Zola Predosa`.split(`.`).filter((e,t,n)=>n.indexOf(e)===t).sort((e,t)=>e.localeCompare(t,`it`));function u(e=`clients`,t=null){let n=a.getClients(),r=a.getMachines(),i=a.getBoards();a.getRefillLogs();let o=``;if(e===`clients`&&t){let e=n.find(e=>e.id===t);if(e){let t=r.find(t=>t.clientId===e.id);o=`
  ...
  `}function p(e=null){let t=a.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return a.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function m(e,t=null,n=null){let r=a.getUsers(),i=a.getRoleLabels(),o=a.getPermissions(),s=a.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
  ...
  `}function g(){let e=a.getSettings();return`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var _={currentUser:a.getCurrentUser(),activeTab:`dashboard`,editingId:null,editingStaffUserId:null,showProfileModal:!1,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingDecontoCode:null,viewingEmailId:null,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`};function v(){let e=document.getElementById(`app`);if(!_.currentUser){e.innerHTML=o(),y();return}let t=``;switch(_.activeTab){case`dashboard`:t=c(_.activeTab,_.viewingDecontoCode,_.dashSearchQuery,_.dashSearchCategory,_.dashSortColumn,_.dashSortDirection,_.viewingKpiModal,_.kpiPeriod,_.kpiChartType,_.kpiCustomStart,_.kpiCustomEnd);break;case`clients`:case`machines`:case`deconto_boards`:t=u(_.activeTab,_.editingId);break;case`adr_visits`:t=d();break;case`client_diy`:case`otp_generator`:case`qr_generator`:case`refills_history`:t=f();break;case`simulator`:t=p();break;case`user_mgmt`:case`user_management`:case`permissions_matrix`:t=m(_.activeTab,_.editingStaffUserId,_.viewingEmailId);break;case`settings`:t=g();break;default:t=c()}let n=h(_.showProfileModal,_.currentUser);e.innerHTML=`
    <div class="app-layout">
      ${s(_.currentUser,_.activeTab)}
      <main class="main-content">
        ${t}
      </main>
    </div>
    ${n}
  `,b()}function y(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value.trim();try{let e=a.login(t,n);_.currentUser=e,_.activeTab=e.role===`ADR`?`adr_visits`:`dashboard`,v()}catch(e){alert(e.message)}})}function b(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(_.activeTab=t,_.editingId=null,_.editingStaffUserId=null,_.viewingDecontoCode=null,_.viewingEmailId=null,v())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{_.showProfileModal=!0,v()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{_.showProfileModal=!1,v()}),n&&n.addEventListener(`click`,()=>{_.showProfileModal=!1,v()});let r=document.getElementById(`profile-edit-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-new-password`).value;try{_.currentUser=a.updateUserProfile(_.currentUser.id,{name:t,email:n,newPassword:r?r.trim():void 0}),_.showProfileModal=!1,alert(`✅ Credenziali del profilo aggiornate con successo!`),v()}catch(e){alert(`Errore: ${e.message}`)}});let i=document.getElementById(`btn-logout`);i&&i.addEventListener(`click`,()=>{a.logout(),_.currentUser=null,_.activeTab=`dashboard`,v()});let o=document.getElementById(`btn-toggle-add-client`),s=document.getElementById(`add-client-form-container`),c=document.getElementById(`btn-cancel-add-client`),l=document.getElementById(`btn-save-new-client`);o&&s&&o.addEventListener(`click`,()=>{let e=s.style.display===`none`||!s.style.display;s.style.display=e?`block`:`none`}),c&&s&&c.addEventListener(`click`,()=>{s.style.display=`none`}),l&&l.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-ref`).value.trim(),n=document.getElementById(`new-cli-phone`).value.trim(),r=document.getElementById(`new-cli-email`).value.trim(),i=document.getElementById(`new-cli-city`).value.trim(),o=document.getElementById(`new-cli-machine`).value;if(!e){alert(`Inserisci la Ragione Sociale / Nome Cliente!`);return}try{let s=a.addClient({name:e,refPerson:t,phone:n,email:r,city:i,machineId:o});alert(`✅ Cliente "${s.name}" registrato con successo!`),v()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{_.editingId=e.getAttribute(`data-id`),v()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo cliente?`))try{a.deleteClient(t),alert(`✅ Cliente eliminato dall'anagrafica!`),v()}catch(e){alert(`Errore: ${e.message}`)}})});let u=document.getElementById(`btn-cancel-edit-client`);u&&u.addEventListener(`click`,()=>{_.editingId=null,v()});let d=document.getElementById(`form-edit-client`);d&&d.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value.trim(),r=document.getElementById(`edit-cli-ref`).value.trim(),i=document.getElementById(`edit-cli-phone`).value.trim(),o=document.getElementById(`edit-cli-city`).value.trim(),s=document.getElementById(`edit-cli-address`).value.trim(),c=document.getElementById(`edit-cli-machine`).value;try{a.updateClient(t,{name:n,refPerson:r,phone:i,city:o,address:s,assignedMachineId:c}),_.editingId=null,alert(`✅ Scheda Cliente salvata con successo!`),v()}catch(e){alert(`Errore: ${e.message}`)}});let f=document.getElementById(`btn-toggle-add-machine`),p=document.getElementById(`add-machine-form-container`),m=document.getElementById(`btn-cancel-add-machine`),h=document.getElementById(`btn-save-new-machine`);f&&p&&f.addEventListener(`click`,()=>{let e=p.style.display===`none`||!p.style.display;p.style.display=e?`block`:`none`}),m&&p&&m.addEventListener(`click`,()=>{p.style.display=`none`}),h&&h.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-board`).value,i=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Inserisci Seriale Macchina e Modello!`);return}try{let o=a.addMachine({serialNumber:e,brand:t,model:n,boardId:r,clientId:i});alert(`✅ Macchina da caffè SN "${o.serialNumber}" registrata con successo!`),v()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{_.editingId=e.getAttribute(`data-id`),v()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa macchina da caffè?`))try{a.deleteMachine(t),alert(`✅ Macchina eliminata dal parco macchine!`),v()}catch(e){alert(`Errore: ${e.message}`)}})});let g=document.getElementById(`btn-cancel-edit-mc`);g&&g.addEventListener(`click`,()=>{_.editingId=null,v()});let y=document.getElementById(`form-edit-machine`);y&&y.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value.trim(),r=document.getElementById(`edit-mc-brand`).value.trim(),i=document.getElementById(`edit-mc-model`).value.trim(),o=document.getElementById(`edit-mc-board`).value,s=document.getElementById(`edit-mc-client`).value;try{a.updateMachine(t,{serialNumber:n,brand:r,model:i,boardId:o,clientId:s}),_.editingId=null,alert(`✅ Scheda Macchina salvata con successo!`),v()}catch(e){alert(`Errore: ${e.message}`)}});let b=document.getElementById(`btn-toggle-add-board`),x=document.getElementById(`add-board-form-container`),S=document.getElementById(`btn-cancel-add-board`),C=document.getElementById(`btn-save-new-board`);b&&x&&b.addEventListener(`click`,()=>{let e=x.style.display===`none`||!x.style.display;x.style.display=e?`block`:`none`}),S&&x&&S.addEventListener(`click`,()=>{x.style.display=`none`}),C&&C.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,i=document.getElementById(`new-board-machine`).value,o=document.getElementById(`new-board-groupcount`),s=o?parseInt(o.value,10):1;if(!e){alert(`Inserisci il Codice a 4 cifre del Deconto!`);return}try{let o=a.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,groupCount:s,machineId:i});alert(`✅ Scheda Deconto #${o.shortCode} salvata PERMANENTEMENTE nel database!`),v()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{_.editingId=e.getAttribute(`data-id`),v()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa scheda Deconto?`))try{a.deleteBoard(t),alert(`✅ Scheda Deconto eliminata!`),v()}catch(e){alert(`Errore: ${e.message}`)}})});let w=document.getElementById(`btn-cancel-edit-board`);w&&w.addEventListener(`click`,()=>{_.editingId=null,v()});let T=document.getElementById(`form-edit-board`);T&&T.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value.trim(),r=document.getElementById(`edit-board-hwserial`).value.trim(),i=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-version`).value,s=document.getElementById(`edit-board-machine`).value,c=document.getElementById(`edit-board-groupcount`),l=c?parseInt(c.value,10):1;try{a.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:i,version:o,groupCount:l,machineId:s}),_.editingId=null,alert(`✅ Scheda Deconto salvata con successo!`),v()}catch(e){alert(`Errore: ${e.message}`)}});let E=document.getElementById(`btn-close-edit-modal`);E&&E.addEventListener(`click`,()=>{_.editingId=null,v()});let D=document.getElementById(`btn-toggle-add-user`),O=document.getElementById(`add-user-form-container`),k=document.getElementById(`btn-cancel-add-user`),A=document.getElementById(`btn-save-new-user`);D&&O&&D.addEventListener(`click`,()=>{let e=O.style.display===`none`||!O.style.display;O.style.display=e?`block`:`none`}),k&&O&&k.addEventListener(`click`,()=>{O.style.display=`none`}),A&&A.addEventListener(`click`,()=>{let e=document.getElementById(`new-user-username`).value.trim(),t=document.getElementById(`new-user-password`).value.trim(),n=document.getElementById(`new-user-name`).value.trim(),r=document.getElementById(`new-user-role`).value,i=document.getElementById(`new-user-email`).value.trim(),o=document.getElementById(`new-user-phone`).value.trim();if(!e||!t||!n){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{a.addUser({username:e,password:t,name:n,role:r,email:i,phone:o}),alert(`✅ Utente dipendente "${n}" (Codice ${e}) salvato PERMANENTEMENTE nel database!`),v()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{_.editingStaffUserId=e.getAttribute(`data-id`),v()})});let j=document.getElementById(`btn-close-edit-staff-modal`),M=document.getElementById(`btn-cancel-edit-staff`);j&&j.addEventListener(`click`,()=>{_.editingStaffUserId=null,v()}),M&&M.addEventListener(`click`,()=>{_.editingStaffUserId=null,v()});let N=document.getElementById(`edit-staff-form`);N&&N.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,i=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=a.updateUser(t,{username:n,name:r,role:i,email:o,phone:s,password:c?c.trim():void 0});_.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),v()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;a.updateUser(t,{status:n}),v()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{a.deleteUser(t),v()}catch(e){alert(`Errore: ${e.message}`)}})});let P=document.getElementById(`btn-open-email-logs`);P&&P.addEventListener(`click`,()=>{let e=a.getEmailLogs();e.length>0?(_.viewingEmailId=e[0].id,v()):alert(`Nessuna email spedita di recente nel registro.`)});let F=document.getElementById(`btn-close-email-preview`),I=document.getElementById(`btn-close-email-preview-footer`);F&&F.addEventListener(`click`,()=>{_.viewingEmailId=null,v()}),I&&I.addEventListener(`click`,()=>{_.viewingEmailId=null,v()});let L=document.getElementById(`rename-role-labels-form`);L&&L.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();a.updateRoleLabel(`UFFICIO`,t),a.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),v()});let R=document.getElementById(`permissions-matrix-form`);R&&R.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),a.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),v()});let z=document.getElementById(`setting-logo-file`);z&&z.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;a.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),v()},e.readAsDataURL(t)}});let B=document.getElementById(`btn-reset-logo`);B&&B.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(a.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),v())});let V=document.getElementById(`settings-brand-form`);V&&V.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();a.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),v()});let H=document.getElementById(`settings-thresholds-form`);H&&H.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}a.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),v()});let U=document.getElementById(`settings-brevo-form`);U&&U.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();a.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),v()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{_.viewingKpiModal=e.getAttribute(`data-kpi`),v()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{_.viewingKpiModal=null,v()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{_.kpiPeriod=e.getAttribute(`data-period`),v()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{_.kpiChartType=e.getAttribute(`data-charttype`),v()})});let W=document.getElementById(`btn-apply-kpi-custom-date`);W&&W.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(_.kpiCustomStart=e,_.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),v()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let G=document.getElementById(`btn-dash-search`),K=document.getElementById(`dash-search-input`);G&&K&&(G.addEventListener(`click`,()=>{_.dashSearchQuery=K.value,_.dashSearchCategory=document.getElementById(`dash-search-category`).value,v()}),K.addEventListener(`keypress`,e=>{e.key===`Enter`&&(_.dashSearchQuery=K.value,_.dashSearchCategory=document.getElementById(`dash-search-category`).value,v())}));let q=document.getElementById(`btn-dash-reset`);q&&q.addEventListener(`click`,()=>{_.dashSearchQuery=``,_.dashSearchCategory=`ALL`,v()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);_.dashSortColumn===t?_.dashSortDirection=_.dashSortDirection===`ASC`?`DESC`:`ASC`:(_.dashSortColumn=t,_.dashSortDirection=`DESC`),v()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{_.viewingDecontoCode=e.getAttribute(`data-code`),v()})});let J=document.getElementById(`btn-close-deconto-modal`),Y=document.getElementById(`btn-close-deconto-modal-footer`);J&&J.addEventListener(`click`,()=>{_.viewingDecontoCode=null,v()}),Y&&Y.addEventListener(`click`,()=>{_.viewingDecontoCode=null,v()});let X=document.getElementById(`btn-export-csv`);X&&X.addEventListener(`click`,()=>{let e=a.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_erogazioni_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let Z=document.getElementById(`btn-trigger-backup`);Z&&Z.addEventListener(`click`,async()=>{try{Z.disabled=!0,Z.innerHTML=`⏳ Backup in Corso...`;let e=a.triggerGitHubBackup();alert(`✅ Backup Cloud completato!\nID: ${e.id}\nCommit: ${e.commitHash}\nRecords: ${e.recordCount}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{Z.disabled=!1,Z.innerHTML=`☁️ Esegui Backup GitHub`}})}v();
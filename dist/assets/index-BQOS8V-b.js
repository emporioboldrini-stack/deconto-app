...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};function o(){let e=a.getSettings();return`
  ...
  `}function C(e=null){let t=a.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return a.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function w(e,t=null,n=null){let r=a.getUsers(),i=a.getRoleLabels(),o=a.getPermissions(),s=a.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
  ...
  `}function E(){let e=a.getSettings();return`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}var D={currentUser:a.getCurrentUser(),activeTab:`dashboard`,editingId:null,editingStaffUserId:null,showProfileModal:!1,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingDecontoCode:null,viewingEmailId:null,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`,generatedOtpUrl:null,generatedOtpToken:null,diyParams:null,refillsFilter:{boardCode:``,clientName:``,date:``},extractionsFilter:{boardCode:``,clientName:``,date:``},simulatingBoardCode:null};function O(){let e=document.getElementById(`app`);if(!D.currentUser){e.innerHTML=o(),k();return}let t=``;switch(D.activeTab){case`dashboard`:t=c(D.activeTab,D.viewingDecontoCode,D.dashSearchQuery,D.dashSearchCategory,D.dashSortColumn,D.dashSortDirection,D.viewingKpiModal,D.kpiPeriod,D.kpiChartType,D.kpiCustomStart,D.kpiCustomEnd);break;case`clients`:case`machines`:case`deconto_boards`:t=_(D.activeTab,D.editingId);break;case`adr_visits`:t=v();break;case`otp_generator`:t=b(D.generatedOtpUrl,D.generatedOtpToken);break;case`client_diy`:t=y(D.diyParams);break;case`qr_generator`:t=`
      ...
      `;break;case`refills_history`:t=x(D.refillsFilter);break;case`extractions_history`:t=S(D.extractionsFilter);break;case`simulator`:t=C(D.simulatingBoardCode);break;case`user_mgmt`:case`user_management`:case`permissions_matrix`:t=w(D.activeTab,D.editingStaffUserId,D.viewingEmailId);break;case`settings`:t=E();break;default:t=c()}let n=T(D.showProfileModal,D.currentUser);e.innerHTML=`
    <div class="app-layout">
      ${s(D.currentUser,D.activeTab)}
      <main class="main-content">
        ${t}
      </main>
    </div>
    ${n}
  `,A()}function k(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value.trim();try{let e=a.login(t,n);D.currentUser=e,D.activeTab=e.role===`ADR`?`adr_visits`:`dashboard`,O()}catch(e){alert(e.message)}})}function A(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(D.activeTab=t,D.editingId=null,D.editingStaffUserId=null,D.viewingDecontoCode=null,D.viewingEmailId=null,O())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{D.showProfileModal=!0,O()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{D.showProfileModal=!1,O()}),n&&n.addEventListener(`click`,()=>{D.showProfileModal=!1,O()});let r=document.getElementById(`profile-edit-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-new-password`).value;try{D.currentUser=a.updateUserProfile(D.currentUser.id,{name:t,email:n,newPassword:r?r.trim():void 0}),D.showProfileModal=!1,alert(`✅ Credenziali del profilo aggiornate con successo!`),O()}catch(e){alert(`Errore: ${e.message}`)}});let i=document.getElementById(`btn-logout`);i&&i.addEventListener(`click`,()=>{a.logout(),D.currentUser=null,D.activeTab=`dashboard`,O()});let o=document.getElementById(`btn-toggle-add-client`),s=document.getElementById(`add-client-form-container`),c=document.getElementById(`btn-cancel-add-client`),l=document.getElementById(`btn-save-new-client`);o&&s&&o.addEventListener(`click`,()=>{let e=s.style.display===`none`||!s.style.display;s.style.display=e?`block`:`none`}),c&&s&&c.addEventListener(`click`,()=>{s.style.display=`none`}),l&&l.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-type`).value,n=document.getElementById(`new-cli-ref`).value.trim(),r=document.getElementById(`new-cli-phone`).value.trim(),i=document.getElementById(`new-cli-email`).value.trim(),o=document.getElementById(`new-cli-city`).value.trim(),s=document.getElementById(`new-cli-machine`).value;if(!e){alert(`Inserisci la Ragione Sociale / Nome Cliente!`);return}try{let c=a.addClient({name:e,clientType:t,refPerson:n,phone:r,email:i,city:o,machineId:s});alert(`✅ Cliente "${c.name}" registrato con successo!`),O()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{D.editingId=e.getAttribute(`data-id`),O()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo cliente?`))try{a.deleteClient(t),alert(`✅ Cliente eliminato dall'anagrafica!`),O()}catch(e){alert(`Errore: ${e.message}`)}})});let u=document.getElementById(`btn-cancel-edit-client`);u&&u.addEventListener(`click`,()=>{D.editingId=null,O()});let d=document.getElementById(`form-edit-client`);d&&d.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value.trim(),r=document.getElementById(`edit-cli-type`).value,i=document.getElementById(`edit-cli-ref`).value.trim(),o=document.getElementById(`edit-cli-phone`).value.trim(),s=document.getElementById(`edit-cli-city`).value.trim(),c=document.getElementById(`edit-cli-address`).value.trim(),l=document.getElementById(`edit-cli-machine`).value;try{a.updateClient(t,{name:n,clientType:r,refPerson:i,phone:o,city:s,address:c,assignedMachineId:l}),D.editingId=null,alert(`✅ Scheda Cliente salvata con successo!`),O()}catch(e){alert(`Errore: ${e.message}`)}});let f=document.getElementById(`btn-toggle-add-machine`),p=document.getElementById(`add-machine-form-container`),m=document.getElementById(`btn-cancel-add-machine`),h=document.getElementById(`btn-save-new-machine`);f&&p&&f.addEventListener(`click`,()=>{let e=p.style.display===`none`||!p.style.display;p.style.display=e?`block`:`none`}),m&&p&&m.addEventListener(`click`,()=>{p.style.display=`none`}),h&&h.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-year`).value.trim(),i=document.getElementById(`new-mc-board`).value,o=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Inserisci Seriale Macchina e Modello!`);return}try{let s=a.addMachine({serialNumber:e,brand:t,model:n,productionYear:r,boardId:i,clientId:o});alert(`✅ Macchina da caffè SN "${s.serialNumber}" registrata con successo!`),O()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{D.editingId=e.getAttribute(`data-id`),O()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa macchina da caffè?`))try{a.deleteMachine(t),alert(`✅ Macchina eliminata dal parco macchine!`),O()}catch(e){alert(`Errore: ${e.message}`)}})});let g=document.getElementById(`btn-cancel-edit-mc`);g&&g.addEventListener(`click`,()=>{D.editingId=null,O()});let _=document.getElementById(`form-edit-machine`);_&&_.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value.trim(),r=document.getElementById(`edit-mc-brand`).value.trim(),i=document.getElementById(`edit-mc-model`).value.trim(),o=document.getElementById(`edit-mc-year`).value.trim(),s=document.getElementById(`edit-mc-board`).value,c=document.getElementById(`edit-mc-client`).value;try{a.updateMachine(t,{serialNumber:n,brand:r,model:i,productionYear:o,boardId:s,clientId:c}),D.editingId=null,alert(`✅ Scheda Macchina salvata con successo!`),O()}catch(e){alert(`Errore: ${e.message}`)}});let v=document.getElementById(`btn-toggle-add-board`),y=document.getElementById(`add-board-form-container`),b=document.getElementById(`btn-cancel-add-board`),x=document.getElementById(`btn-save-new-board`);v&&y&&v.addEventListener(`click`,()=>{let e=y.style.display===`none`||!y.style.display;y.style.display=e?`block`:`none`}),b&&y&&b.addEventListener(`click`,()=>{y.style.display=`none`}),x&&x.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,i=document.getElementById(`new-board-machine`).value,o=document.getElementById(`new-board-groupcount`),s=o?parseInt(o.value,10):1;if(!e){alert(`Inserisci il Codice a 4 cifre del Deconto!`);return}try{let o=a.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,groupCount:s,machineId:i});alert(`✅ Scheda Deconto #${o.shortCode} salvata PERMANENTEMENTE nel database!`),O()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{D.editingId=e.getAttribute(`data-id`),O()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa scheda Deconto?`))try{a.deleteBoard(t),alert(`✅ Scheda Deconto eliminata!`),O()}catch(e){alert(`Errore: ${e.message}`)}})});let S=document.getElementById(`btn-cancel-edit-board`);S&&S.addEventListener(`click`,()=>{D.editingId=null,O()});let C=document.getElementById(`form-edit-board`);C&&C.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value.trim(),r=document.getElementById(`edit-board-hwserial`).value.trim(),i=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-version`).value,s=document.getElementById(`edit-board-machine`).value,c=document.getElementById(`edit-board-groupcount`),l=c?parseInt(c.value,10):1;try{a.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:i,version:o,groupCount:l,machineId:s}),D.editingId=null,alert(`✅ Scheda Deconto salvata con successo!`),O()}catch(e){alert(`Errore: ${e.message}`)}});let w=document.getElementById(`btn-close-edit-modal`);w&&w.addEventListener(`click`,()=>{D.editingId=null,O()});let T=document.getElementById(`btn-toggle-add-user`),E=document.getElementById(`add-user-form-container`),k=document.getElementById(`btn-cancel-add-user`),A=document.getElementById(`btn-save-new-user`);T&&E&&T.addEventListener(`click`,()=>{let e=E.style.display===`none`||!E.style.display;E.style.display=e?`block`:`none`}),k&&E&&k.addEventListener(`click`,()=>{E.style.display=`none`}),A&&A.addEventListener(`click`,()=>{let e=document.getElementById(`new-user-username`).value.trim(),t=document.getElementById(`new-user-password`).value.trim(),n=document.getElementById(`new-user-name`).value.trim(),r=document.getElementById(`new-user-role`).value,i=document.getElementById(`new-user-email`).value.trim(),o=document.getElementById(`new-user-phone`).value.trim();if(!e||!t||!n){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{a.addUser({username:e,password:t,name:n,role:r,email:i,phone:o}),alert(`✅ Utente dipendente "${n}" (Codice ${e}) salvato PERMANENTEMENTE nel database!`),O()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{D.editingStaffUserId=e.getAttribute(`data-id`),O()})});let j=document.getElementById(`btn-close-edit-staff-modal`),ee=document.getElementById(`btn-cancel-edit-staff`);j&&j.addEventListener(`click`,()=>{D.editingStaffUserId=null,O()}),ee&&ee.addEventListener(`click`,()=>{D.editingStaffUserId=null,O()});let te=document.getElementById(`edit-staff-form`);te&&te.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,i=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=a.updateUser(t,{username:n,name:r,role:i,email:o,phone:s,password:c?c.trim():void 0});D.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),O()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;a.updateUser(t,{status:n}),O()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{a.deleteUser(t),O()}catch(e){alert(`Errore: ${e.message}`)}})});let ne=document.getElementById(`btn-open-email-logs`);ne&&ne.addEventListener(`click`,()=>{let e=a.getEmailLogs();e.length>0?(D.viewingEmailId=e[0].id,O()):alert(`Nessuna email spedita di recente nel registro.`)});let re=document.getElementById(`btn-close-email-preview`),ie=document.getElementById(`btn-close-email-preview-footer`);re&&re.addEventListener(`click`,()=>{D.viewingEmailId=null,O()}),ie&&ie.addEventListener(`click`,()=>{D.viewingEmailId=null,O()});let M=document.getElementById(`rename-role-labels-form`);M&&M.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();a.updateRoleLabel(`UFFICIO`,t),a.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),O()});let ae=document.getElementById(`permissions-matrix-form`);ae&&ae.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),a.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),O()});let oe=document.getElementById(`setting-logo-file`);oe&&oe.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;a.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),O()},e.readAsDataURL(t)}});let se=document.getElementById(`btn-reset-logo`);se&&se.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(a.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),O())});let N=document.getElementById(`settings-brand-form`);N&&N.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();a.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),O()});let ce=document.getElementById(`settings-thresholds-form`);ce&&ce.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}a.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),O()});let le=document.getElementById(`settings-brevo-form`);le&&le.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();a.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),O()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{D.viewingKpiModal=e.getAttribute(`data-kpi`),O()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{D.viewingKpiModal=null,O()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{D.kpiPeriod=e.getAttribute(`data-period`),O()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{D.kpiChartType=e.getAttribute(`data-charttype`),O()})});let ue=document.getElementById(`btn-apply-kpi-custom-date`);ue&&ue.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(D.kpiCustomStart=e,D.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),O()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let de=document.getElementById(`btn-dash-search`),P=document.getElementById(`dash-search-input`);de&&P&&(de.addEventListener(`click`,()=>{D.dashSearchQuery=P.value,D.dashSearchCategory=document.getElementById(`dash-search-category`).value,O()}),P.addEventListener(`keypress`,e=>{e.key===`Enter`&&(D.dashSearchQuery=P.value,D.dashSearchCategory=document.getElementById(`dash-search-category`).value,O())}));let fe=document.getElementById(`btn-dash-reset`);fe&&fe.addEventListener(`click`,()=>{D.dashSearchQuery=``,D.dashSearchCategory=`ALL`,O()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);D.dashSortColumn===t?D.dashSortDirection=D.dashSortDirection===`ASC`?`DESC`:`ASC`:(D.dashSortColumn=t,D.dashSortDirection=`DESC`),O()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{D.viewingDecontoCode=e.getAttribute(`data-code`),D.viewingKpiModal=null,O()})});let pe=document.getElementById(`btn-close-deconto-modal`),me=document.getElementById(`btn-close-deconto-modal-footer`);pe&&pe.addEventListener(`click`,()=>{D.viewingDecontoCode=null,O()}),me&&me.addEventListener(`click`,()=>{D.viewingDecontoCode=null,O()});let he=document.getElementById(`btn-export-csv`);he&&he.addEventListener(`click`,()=>{let e=a.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_erogazioni_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let F=document.getElementById(`btn-trigger-backup`);F&&F.addEventListener(`click`,async()=>{try{F.disabled=!0,F.innerHTML=`⏳ Backup in Corso...`;let e=a.triggerGitHubBackup();alert(`✅ Backup Cloud completato!\nID: ${e.id}\nCommit: ${e.commitHash}\nRecords: ${e.recordCount}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{F.disabled=!1,F.innerHTML=`☁️ Esegui Backup GitHub`}});let I=document.getElementById(`otp-client-select`),L=document.getElementById(`otp-machine-select`),R=document.getElementById(`otp-board-select`),z=document.getElementById(`otp-credits-input`),B=document.getElementById(`otp-custom-credits-wrapper`);z&&B&&z.addEventListener(`change`,()=>{z.value===`CUSTOM`?B.style.display=`block`:B.style.display=`none`}),I&&L&&R&&(I.addEventListener(`change`,()=>{let e=I.value;if(!e){L.value=``,R.value=``;return}let t=a.getMachines().find(t=>t.clientId===e);if(t){L.value=t.id;let e=a.getBoards().find(e=>e.machineId===t.id);e?R.value=e.shortCode:R.value=``}else L.value=``,R.value=``}),L.addEventListener(`change`,()=>{let e=L.value;if(!e){I.value=``,R.value=``;return}let t=a.getMachines().find(t=>t.id===e);if(t){I.value=t.clientId||``;let e=a.getBoards().find(e=>e.machineId===t.id);e?R.value=e.shortCode:R.value=``}}),R.addEventListener(`change`,()=>{let e=R.value;if(!e){L.value=``,I.value=``;return}let t=a.getBoards().find(t=>t.shortCode===e);if(t&&t.machineId){L.value=t.machineId;let e=a.getMachines().find(e=>e.id===t.machineId);e?I.value=e.clientId||``:I.value=``}else L.value=``,I.value=``}));let ge=document.getElementById(`btn-generate-otp-link`);ge&&ge.addEventListener(`click`,()=>{let e=document.getElementById(`otp-client-select`).value,t=document.getElementById(`otp-board-select`).value,n=document.getElementById(`otp-credits-input`).value,r=document.getElementById(`otp-expiry-input`).value;if(!e||!t){alert(`Seleziona sia il Cliente che la Scheda Deconto!`);return}let i=200;if(n===`CUSTOM`){let e=parseInt(document.getElementById(`otp-custom-credits-value`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per i crediti personalizzati!`);return}i=e}else i=parseInt(n,10);let o=a.getClients().find(t=>t.id===e),s=o?o.name:`Cliente`;if(!r){let e=new Date;e.setMonth(e.getMonth()+1),r=e.toISOString().split(`T`)[0]}let c=Math.random().toString(36).substring(2,7).toUpperCase(),l=`OTP-${Math.floor(1e3+Math.random()*9e3)}-${c}`;D.generatedOtpUrl=`${window.location.origin+window.location.pathname}?tab=client_diy&clientName=${encodeURIComponent(s)}&board=${t}&otp=${l}&credits=${i}`,D.generatedOtpToken=l,D.selectedClientTemp=e,D.selectedMachineTemp=document.getElementById(`otp-machine-select`).value,D.selectedBoardTemp=t,D.selectedCreditsTemp=n,D.selectedCustomCreditsTemp=n===`CUSTOM`?i:``,D.selectedExpiryTemp=r,O();let u=document.getElementById(`otp-client-select`),d=document.getElementById(`otp-machine-select`),f=document.getElementById(`otp-board-select`),p=document.getElementById(`otp-credits-input`),m=document.getElementById(`otp-custom-credits-wrapper`),h=document.getElementById(`otp-custom-credits-value`),g=document.getElementById(`otp-expiry-input`);u&&(u.value=D.selectedClientTemp),d&&(d.value=D.selectedMachineTemp),f&&(f.value=D.selectedBoardTemp),p&&(p.value=D.selectedCreditsTemp),p&&p.value===`CUSTOM`&&m&&(m.style.display=`block`,h&&(h.value=D.selectedCustomCreditsTemp)),g&&(g.value=D.selectedExpiryTemp),setTimeout(()=>{let e=document.getElementById(`otp-result-container`);e&&e.scrollIntoView({behavior:`smooth`})},50)});let _e=document.getElementById(`btn-copy-otp-link`);_e&&_e.addEventListener(`click`,()=>{let e=document.getElementById(`generated-otp-url`);e&&(e.select(),document.execCommand(`copy`),alert(`📋 Link di ricarica copiato negli appunti!`))});let ve=document.getElementById(`btn-whatsapp-otp-send`);if(ve&&D.generatedOtpUrl){let e=document.getElementById(`otp-board-select`)?.value||`Deconto`,t=`Ciao! Ecco il link per ricaricare +${document.getElementById(`otp-credits-input`)?.value||`200`} caffè sulla tua macchina Deconto #${e}: ${D.generatedOtpUrl}`;ve.setAttribute(`href`,`https://wa.me/?text=${encodeURIComponent(t)}`)}let ye=document.getElementById(`btn-test-otp-link`);ye&&D.generatedOtpUrl&&ye.addEventListener(`click`,()=>{let e=new URLSearchParams(D.generatedOtpUrl.split(`?`)[1]);D.diyParams={clientName:e.get(`clientName`),boardShortCode:e.get(`board`),tokenOtp:e.get(`otp`),credits:parseInt(e.get(`credits`),10),success:!1},D.activeTab=`client_diy`,O()});let V=document.getElementById(`btn-client-diy-refill`);V&&V.addEventListener(`click`,()=>{let e=V.getAttribute(`data-board`),t=parseInt(V.getAttribute(`data-credits`),10),n=V.getAttribute(`data-otp`),r=document.getElementById(`diy-status-msg`);r&&(r.innerHTML=`<span style="color: var(--accent-cyan); font-weight: 700;">📡 Connessione Bluetooth in corso... Sincronizzazione relè...</span>`),V.disabled=!0,setTimeout(()=>{try{a.performRefill({boardShortCode:e,credits:t,method:`WHATSAPP_OTP_BLE`,tokenOtp:n}),D.diyParams?D.diyParams.success=!0:D.diyParams={success:!0,clientName:`Studio Legale Brambilla`,boardShortCode:e,tokenOtp,credits:t},O()}catch(e){alert(`Errore durante l'accredito: ${e.message}`),r&&(r.innerHTML=`<span style="color: var(--accent-rose); font-weight: 700;">❌ Errore BLE: ${e.message}</span>`),V.disabled=!1}},1200)});let be=document.getElementById(`btn-back-to-office-from-diy`);be&&be.addEventListener(`click`,()=>{D.diyParams=null,D.generatedOtpUrl=null,D.generatedOtpToken=null,D.activeTab=`otp_generator`,O()});let H=document.getElementById(`filter-refill-board`),U=document.getElementById(`filter-refill-client`),W=document.getElementById(`filter-refill-date`),xe=document.getElementById(`btn-reset-refill-filters`);H&&H.addEventListener(`input`,()=>{D.refillsFilter.boardCode=H.value,O();let e=document.getElementById(`filter-refill-board`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),U&&U.addEventListener(`input`,()=>{D.refillsFilter.clientName=U.value,O();let e=document.getElementById(`filter-refill-client`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),W&&W.addEventListener(`change`,()=>{D.refillsFilter.date=W.value,O()}),xe&&xe.addEventListener(`click`,()=>{D.refillsFilter={boardCode:``,clientName:``,date:``},O()});let G=document.getElementById(`filter-extraction-board`),K=document.getElementById(`filter-extraction-client`),q=document.getElementById(`filter-extraction-date`),Se=document.getElementById(`btn-reset-extraction-filters`);G&&G.addEventListener(`input`,()=>{D.extractionsFilter.boardCode=G.value,O();let e=document.getElementById(`filter-extraction-board`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),K&&K.addEventListener(`input`,()=>{D.extractionsFilter.clientName=K.value,O();let e=document.getElementById(`filter-extraction-client`);e&&(e.focus(),e.setSelectionRange(e.value.length,e.value.length))}),q&&q.addEventListener(`change`,()=>{D.extractionsFilter.date=q.value,O()}),Se&&Se.addEventListener(`click`,()=>{D.extractionsFilter={boardCode:``,clientName:``,date:``},O()});let J=document.getElementById(`modal-refill-credits-select`),Y=document.getElementById(`modal-refill-credits-custom`),X=document.getElementById(`btn-modal-perform-refill`);J&&Y&&J.addEventListener(`change`,()=>{J.value===`CUSTOM`?Y.style.display=`block`:Y.style.display=`none`}),X&&X.addEventListener(`click`,()=>{let e=X.getAttribute(`data-board-code`),t=document.getElementById(`modal-refill-credits-select`).value,n=200;if(t===`CUSTOM`){let e=parseInt(document.getElementById(`modal-refill-credits-custom`).value,10);if(isNaN(e)){alert(`Inserisci un valore numerico valido (positivo o negativo) per la ricarica personalizzata!`);return}n=e}else n=parseInt(t,10);let r=D.currentUser,i=r&&(r.role===`ADMIN`||r.role===`UFFICIO`),o=i?`CLOUD_DIRECT`:`ADR_BLE_PHYSICAL`;try{a.performRefill({boardShortCode:e,credits:n,method:o,operatorId:r?r.id:`usr_001`}),alert(`✅ Ricarica di ${n>=0?`+`:``}${n} caffè effettuata con successo!\nCanale: ${i?`Cloud Wi-Fi (Ufficio)`:`Bluetooth BLE (ADR)`}`),O()}catch(e){alert(`❌ Errore durante la ricarica: ${e.message}`)}});let Z=document.getElementById(`sim-board-select`),Q=document.getElementById(`btn-sim-brew`),$=document.getElementById(`btn-sim-reset`);Z&&Z.addEventListener(`change`,()=>{D.simulatingBoardCode=Z.value,O()}),Q&&Q.addEventListener(`click`,()=>{let e=D.simulatingBoardCode||(a.getBoards()[0]?a.getBoards()[0].shortCode:null);if(!e)return;let t=a.getBoards().find(t=>t.shortCode===e);if(!t||t.remainingCredits<=0)return;Q.disabled=!0,Q.innerText=`☕ EROGAZIONE IN CORSO (230V SENSE IN)...`,$&&($.disabled=!0);let n=document.getElementById(`signal-sense-volts`),r=document.getElementById(`signal-sense-badge`),i=document.getElementById(`sim-console-log`);n&&(n.innerText=`230V AC`),r&&(r.innerText=`⚡ SENSE ATTIVO`,r.className=`badge badge-success`),i&&(i.innerHTML+=`<br>[INPUT]: Rilevata tensione 230V AC su Sense-In (Filo Marrone).<br>[FIRMWARE]: Conteggio tempo di erogazione avviato...`,i.scrollTop=i.scrollHeight),setTimeout(()=>{try{let n=a.registerCoffeeExtraction(e,22,1);n&&n.success===!1?alert(`Impossibile erogare: ${n.reason}`):i&&(i.innerHTML+=`<br>[FIRMWARE]: Erogazione terminata (22s simulati).<br>[MEMORY]: Credito scalato. Nuovo saldo: ${t.remainingCredits} caffè.<br>[HARDWARE]: Relè di blocco: ${t.relayStatus===`CLOSED_OK`?`CONNESSO/ABILITATO`:`APERTO/BLOCCATO`}`)}catch(e){console.error(e)}O()},1500)}),$&&$.addEventListener(`click`,()=>{let e=D.simulatingBoardCode||(a.getBoards()[0]?a.getBoards()[0].shortCode:null);if(e)try{a.performRefill({boardShortCode:e,credits:200,method:`CLOUD_DIRECT`,operatorId:D.currentUser?D.currentUser.id:`usr_001`});let t=document.getElementById(`sim-console-log`);t&&(t.innerHTML+=`<br>[CLOUDLINK]: Comando Ricarica remota +200 ricevuto via Wi-Fi.<br>[MEMORY]: Crediti aggiornati. Relè sbloccato.`),O()}catch(e){alert(`Errore: ${e.message}`)}})}function j(){let e=new URLSearchParams(window.location.search);e.get(`tab`)===`client_diy`&&(D.activeTab=`client_diy`,D.diyParams={clientName:e.get(`clientName`)||`Studio Legale Brambilla`,boardShortCode:e.get(`board`)||`3467`,tokenOtp:e.get(`otp`)||`OTP-9981-X79K2`,credits:parseInt(e.get(`credits`),10)||200,success:!1},window.history.replaceState({},document.title,window.location.pathname))}j(),O();
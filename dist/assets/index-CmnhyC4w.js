...
`;return this.data.coffeeLogs.forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};new class{constructor(){this.isSupported=typeof navigator<`u`&&`bluetooth`in navigator,this.connectedDevice=null}checkSupport(){return this.isSupported}async connectToBoardByShortCode(e){if(console.log(`📡 Ricerca dispositivo Deconto con codice breve [${e}]...`),this.isSupported&&navigator.bluetooth)try{let t=await navigator.bluetooth.requestDevice({filters:[{namePrefix:`DECONTO_${e}`}],optionalServices:[`0000ffe0-0000-1000-8000-00805f9b34fb`]});return this.connectedDevice=t,{success:!0,deviceName:t.name,isRealHardware:!0}}catch(e){console.warn(`Fallback a simulazione BLE locale:`,e.message)}return await new Promise(e=>setTimeout(e,1500)),{success:!0,deviceName:`DECONTO_${e}`,shortCode:e,isRealHardware:!1,connectedAt:new Date().toISOString()}}async sendRefillOtpToken(e,t,n){if(!(await this.connectToBoardByShortCode(e)).success)throw Error(`Impossibile connettersi al dispositivo DECONTO_${e}`);return await new Promise(e=>setTimeout(e,1e3)),{success:!0,shortCode:e,creditsAccredited:t,tokenApplied:n,relayStatus:`CLOSED_OK`,timestamp:new Date().toISOString()}}};var a=new class{constructor(){this.repoUrl=`https://github.com/deconto-org/deconto-db-backups`}generateDatabaseSnapshot(){return{version:`1.0.0`,timestamp:new Date().toISOString(),data:i.data}}async executeBackupNow(){let e=this.generateDatabaseSnapshot(),t=JSON.stringify(e,null,2);return await new Promise(e=>setTimeout(e,1200)),{success:!0,backupRecord:i.triggerGitHubBackup(),sizeBytes:new Blob([t]).size,snapshotTimestamp:e.timestamp}}};function o(e,t){let n=i.getSettings(),r=i.getRoleLabels(),a=e.role===`UFFICIO`||e.role===`ADMIN`,o=e.role===`ADR`||e.role===`ADMIN`,s=e.role===`ADMIN`;return`
  ...
  `}var h={currentUser:i.getCurrentUser(),activeTab:`dashboard`,showProfileModal:!1,editingStaffUserId:null,editingId:null,viewingDecontoCode:null,viewingEmailId:null,selectedSimBoardCode:`9901`,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`};function g(){let e=document.getElementById(`app`);if(!h.currentUser){e.innerHTML=s(),_();return}let t=h.currentUser,n=``;h.activeTab===`settings`?n=p():h.activeTab===`simulator`?n=m(h.selectedSimBoardCode):h.activeTab===`user_management`||h.activeTab===`permissions_matrix`?n=u(h.activeTab,h.editingStaffUserId,h.viewingEmailId):t.role===`ADMIN`||t.role===`UFFICIO`?n=h.activeTab===`clients`||h.activeTab===`machines`||h.activeTab===`deconto_boards`||h.activeTab===`qr_generator`||h.activeTab===`otp_generator`||h.activeTab===`refills_history`?d(h.activeTab,h.editingId):h.activeTab===`adr_visits`?f(h.activeTab):l(h.activeTab,h.viewingDecontoCode,h.dashSearchQuery,h.dashSearchCategory,h.dashSortColumn,h.dashSortDirection,h.viewingKpiModal,h.kpiPeriod,h.kpiChartType):t.role===`ADR`&&(n=h.activeTab===`adr_visits`?f(h.activeTab):d(h.activeTab,h.editingId));let r=``;h.showProfileModal&&(r=c(t)),e.innerHTML=`
    ...
    ${r}
  `,v()}function _(){let e=document.getElementById(`login-form`),t=document.getElementById(`login-error-msg`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let n=document.getElementById(`login-username`).value,r=document.getElementById(`login-password`).value;try{let e=i.authenticate(n,r);h.currentUser=e,h.activeTab=e.role===`ADMIN`?`dashboard`:`clients`,g()}catch(e){t.innerText=e.message,t.style.display=`block`}})}function v(){let e=document.getElementById(`btn-logout`);e&&e.addEventListener(`click`,()=>{i.logout(),h.currentUser=null,g()});let t=document.getElementById(`btn-open-profile-modal`);t&&t.addEventListener(`click`,()=>{h.showProfileModal=!0,g()});let n=document.getElementById(`btn-close-profile-modal`),r=document.getElementById(`btn-cancel-profile`);n&&n.addEventListener(`click`,()=>{h.showProfileModal=!1,g()}),r&&r.addEventListener(`click`,()=>{h.showProfileModal=!1,g()}),document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(h.activeTab=t,h.editingId=null,g())})});let o=document.getElementById(`setting-logo-file`);o&&o.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;i.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),g()},e.readAsDataURL(t)}});let s=document.getElementById(`btn-reset-logo`);s&&s.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(i.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),g())});let c=document.getElementById(`settings-brand-form`);c&&c.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();i.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),g()});let l=document.getElementById(`settings-brevo-form`);l&&l.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();i.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),g()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingKpiModal=e.getAttribute(`data-kpi`),g()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingKpiModal=null,g()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{h.kpiPeriod=e.getAttribute(`data-period`),g()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{h.kpiChartType=e.getAttribute(`data-charttype`),g()})});let u=document.getElementById(`btn-dash-search`),d=document.getElementById(`dash-search-input`);u&&d&&(u.addEventListener(`click`,()=>{h.dashSearchQuery=d.value,h.dashSearchCategory=document.getElementById(`dash-search-category`).value,g()}),d.addEventListener(`keypress`,e=>{e.key===`Enter`&&(h.dashSearchQuery=d.value,h.dashSearchCategory=document.getElementById(`dash-search-category`).value,g())}));let f=document.getElementById(`btn-dash-reset`);f&&f.addEventListener(`click`,()=>{h.dashSearchQuery=``,h.dashSearchCategory=`ALL`,g()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);h.dashSortColumn===t?h.dashSortDirection=h.dashSortDirection===`ASC`?`DESC`:`ASC`:(h.dashSortColumn=t,h.dashSortDirection=`ASC`),g()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{h.viewingDecontoCode=e.getAttribute(`data-code`),g()})});let p=document.getElementById(`btn-close-deconto-modal`),m=document.getElementById(`btn-close-deconto-modal-footer`);p&&p.addEventListener(`click`,()=>{h.viewingDecontoCode=null,g()}),m&&m.addEventListener(`click`,()=>{h.viewingDecontoCode=null,g()});let _=document.getElementById(`btn-export-csv`);_&&_.addEventListener(`click`,()=>{let e=i.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=`DECONTO_Report_Consumi_${new Date().toISOString().split(`T`)[0]}.csv`,r.click(),alert(`📥 Report Consumi CSV Scaricato con successo!`)});let v=document.getElementById(`btn-trigger-backup`);v&&v.addEventListener(`click`,async()=>{v.disabled=!0,v.innerText=`⏳ Backup in corso su GitHub...`;let e=await a.executeBackupNow();alert(`✅ Backup GitHub Eseguito con Successo!\n\nRepository: https://github.com/emporioboldrini-stack/deconto-app.git\nCommit Hash: ${e.backupRecord.commitHash}\nEntità salvate: ${e.backupRecord.recordCount}`),g()});let y=document.getElementById(`btn-toggle-add-client`),b=document.getElementById(`add-client-form-container`);y&&b&&y.addEventListener(`click`,()=>{b.style.display=b.style.display===`none`?`block`:`none`});let x=document.getElementById(`btn-cancel-add-client`);x&&b&&x.addEventListener(`click`,()=>{b.style.display=`none`});let S=document.getElementById(`btn-save-new-client`);S&&S.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-ref`).value.trim(),n=document.getElementById(`new-cli-phone`).value.trim(),r=document.getElementById(`new-cli-email`).value.trim(),a=document.getElementById(`new-cli-city`).value.trim(),o=document.getElementById(`new-cli-address`).value.trim(),s=document.getElementById(`new-cli-machine`)?document.getElementById(`new-cli-machine`).value:null;if(!e){alert(`Compila la Ragione Sociale del Cliente!`);return}try{i.addClient({name:e,refPerson:t,phone:n,email:r,city:a,address:o,machineId:s}),alert(`✅ Cliente "${e}" salvato ed installato con successo!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingId=e.getAttribute(`data-id`),g()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Eliminare questo cliente dall'anagrafica? Le macchine collegate torneranno in magazzino.`))try{i.deleteClient(t),g()}catch(e){alert(e.message)}})});let C=document.getElementById(`form-edit-client`);C&&C.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value,r=document.getElementById(`edit-cli-ref`).value,a=document.getElementById(`edit-cli-phone`).value,o=document.getElementById(`edit-cli-city`).value,s=document.getElementById(`edit-cli-address`).value,c=document.getElementById(`edit-cli-machine`)?document.getElementById(`edit-cli-machine`).value:void 0;try{i.updateClient(t,{name:n,refPerson:r,phone:a,city:o,address:s,assignedMachineId:c}),h.editingId=null,alert(`✅ Scheda Cliente e Macchina installata aggiornata!`),g()}catch(e){alert(e.message)}});let w=document.getElementById(`btn-toggle-add-machine`),T=document.getElementById(`add-machine-form-container`);w&&T&&w.addEventListener(`click`,()=>{T.style.display=T.style.display===`none`?`block`:`none`});let E=document.getElementById(`btn-cancel-add-machine`);E&&T&&E.addEventListener(`click`,()=>{T.style.display=`none`});let D=document.getElementById(`btn-save-new-machine`);D&&D.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-board`)?document.getElementById(`new-mc-board`).value:null,a=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Compila Seriale e Modello della macchina!`);return}try{i.addMachine({serialNumber:e,brand:t,model:n,boardId:r,clientId:a}),alert(`✅ Macchina "${e}" registrata ed associata nel parco macchine!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingId=e.getAttribute(`data-id`),g()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Eliminare questa macchina dal parco macchine?`))try{i.deleteMachine(t),g()}catch(e){alert(e.message)}})});let O=document.getElementById(`form-edit-machine`);O&&O.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value,r=document.getElementById(`edit-mc-brand`).value,a=document.getElementById(`edit-mc-model`).value,o=document.getElementById(`edit-mc-board`)?document.getElementById(`edit-mc-board`).value:void 0,s=document.getElementById(`edit-mc-client`).value;try{i.updateMachine(t,{serialNumber:n,brand:r,model:a,boardId:o,clientId:s}),h.editingId=null,alert(`✅ Macchina da Caffè e Scheda Deconto collegate con successo!`),g()}catch(e){alert(e.message)}});let k=document.getElementById(`btn-toggle-add-board`),A=document.getElementById(`add-board-form-container`);k&&A&&k.addEventListener(`click`,()=>{A.style.display=A.style.display===`none`?`block`:`none`});let j=document.getElementById(`btn-cancel-add-board`);j&&A&&j.addEventListener(`click`,()=>{A.style.display=`none`});let M=document.getElementById(`btn-save-new-board`);M&&M.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,a=document.getElementById(`new-board-machine`).value;if(!e){alert(`Inserisci il codice a 4 cifre per la Scheda Deconto (es. 9902)!`);return}try{i.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,machineId:a}),alert(`✅ NUOVA SCHEDA DECONTO #${e} CREATA CON SUCCESSO!`),g()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingId=e.getAttribute(`data-id`),g()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Eliminare questa Scheda Hardware Deconto?`))try{i.deleteBoard(t),g()}catch(e){alert(e.message)}})});let N=document.getElementById(`form-edit-board`);N&&N.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value,r=document.getElementById(`edit-board-hwserial`).value,a=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-threshold`).value,s=document.getElementById(`edit-board-version`).value,c=document.getElementById(`edit-board-machine`).value;try{i.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:a,lowStockThreshold:o,version:s,machineId:c}),h.editingId=null,alert(`✅ Scheda Deconto aggiornata con successo!`),g()}catch(e){alert(e.message)}}),document.querySelectorAll(`#btn-close-edit-modal, #btn-cancel-edit-client, #btn-cancel-edit-mc, #btn-cancel-edit-board`).forEach(e=>{e.addEventListener(`click`,()=>{h.editingId=null,g()})});let P=document.getElementById(`sim-board-select`);P&&P.addEventListener(`change`,e=>{h.selectedSimBoardCode=e.target.value,g()});let F=document.getElementById(`btn-sim-brew`);F&&F.addEventListener(`click`,()=>{let e=h.selectedSimBoardCode||`9901`;document.getElementById(`signal-sense-volts`).innerText=`230V AC (Impulso)`,document.getElementById(`signal-sense-badge`).className=`badge badge-warning`,document.getElementById(`signal-sense-badge`).innerText=`EROGAZIONE IN CORSO`;let t=i.registerCoffeeExtraction(e,22,1);setTimeout(()=>{if(t&&t.success){let n=document.getElementById(`sim-console-log`);n&&(n.innerHTML+=`[EXTRACTION]: Caffè erogato su #${e}! Credito rimanente: ${t.remainingCredits}.<br>`,n.scrollTop=n.scrollHeight)}g()},600)});let I=document.getElementById(`btn-sim-reset`);I&&I.addEventListener(`click`,()=>{let e=h.selectedSimBoardCode||`9901`;i.performRefill({boardShortCode:e,credits:200,method:`TEST_BENCH`,operatorId:h.currentUser?h.currentUser.id:`usr_001`}),alert(`✅ Ricaricate +200 cialde di prova sulla macchina #${e}!`),g()})}document.addEventListener(`DOMContentLoaded`,g);
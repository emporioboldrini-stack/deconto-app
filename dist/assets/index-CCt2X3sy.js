...
`;return(this.data.coffeeLogs||[]).forEach(t=>{let n=this.getBoardFullDetails(t.boardId),r=n&&n.client?n.client.name.replace(/,/g,` `):`N/D`,i=n&&n.machine?n.machine.serialNumber:`N/D`,a=n&&n.machine?n.machine.model.replace(/,/g,` `):`N/D`,o=n&&n.board?n.board.shortCode:`N/D`;e+=`${t.id},${o},"${r}",${i},"${a}",${t.timestamp},${t.durationSeconds},${t.groupId}\n`}),e}triggerGitHubBackup(){let e={id:`bak_`+Date.now(),timestamp:new Date().toISOString(),repo:`emporioboldrini-stack/deconto-app`,commitHash:`git-`+Math.random().toString(36).substring(2,10),status:`SUCCESS`,recordCount:this.data.clients.length+this.data.machines.length+this.data.decontoBoards.length+this.data.refillLogs.length};return this.data.backupLogs.unshift(e),this.saveData(),e}};function o(){let e=a.getSettings();return`
  ...
  `}function b(e=null){let t=a.getBoards(),n=e&&t.find(t=>t.shortCode===e)||t[0];return a.getBoardFullDetails(n.shortCode),`
    <div>
      <div style="margin-bottom: 24px;">
  ...
  `}function x(e,t=null,n=null){let r=a.getUsers(),i=a.getRoleLabels(),o=a.getPermissions(),s=a.getEmailLogs(),c=``;if(t){let e=r.find(e=>e.id===t);e&&(c=`
  ...
  `}var w={currentUser:a.getCurrentUser(),activeTab:`dashboard`,editingId:null,editingStaffUserId:null,showProfileModal:!1,dashSearchQuery:``,dashSearchCategory:`ALL`,dashSortColumn:`shortCode`,dashSortDirection:`DESC`,viewingDecontoCode:null,viewingEmailId:null,viewingKpiModal:null,kpiPeriod:`30DAYS`,kpiChartType:`LINE`,kpiCustomStart:`2026-07-01`,kpiCustomEnd:`2026-08-02`};function T(){let e=document.getElementById(`app`);if(!w.currentUser){e.innerHTML=o(),E();return}let t=``;switch(w.activeTab){case`dashboard`:t=c(w.activeTab,w.viewingDecontoCode,w.dashSearchQuery,w.dashSearchCategory,w.dashSortColumn,w.dashSortDirection,w.viewingKpiModal,w.kpiPeriod,w.kpiChartType,w.kpiCustomStart,w.kpiCustomEnd);break;case`clients`:case`machines`:case`deconto_boards`:t=_(w.activeTab,w.editingId);break;case`adr_visits`:t=v();break;case`client_diy`:case`otp_generator`:case`qr_generator`:case`refills_history`:t=y();break;case`simulator`:t=b();break;case`user_mgmt`:case`user_management`:case`permissions_matrix`:t=x(w.activeTab,w.editingStaffUserId,w.viewingEmailId);break;case`settings`:t=C();break;default:t=c()}let n=S(w.showProfileModal,w.currentUser);e.innerHTML=`
    <div class="app-layout">
      ${s(w.currentUser,w.activeTab)}
      <main class="main-content">
        ${t}
      </main>
    </div>
    ${n}
  `,D()}function E(){let e=document.getElementById(`login-form`);e&&e.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`login-username`).value.trim(),n=document.getElementById(`login-password`).value.trim();try{let e=a.login(t,n);w.currentUser=e,w.activeTab=e.role===`ADR`?`adr_visits`:`dashboard`,T()}catch(e){alert(e.message)}})}function D(){document.querySelectorAll(`.nav-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&(w.activeTab=t,w.editingId=null,w.editingStaffUserId=null,w.viewingDecontoCode=null,w.viewingEmailId=null,T())})});let e=document.getElementById(`btn-open-profile-modal`);e&&e.addEventListener(`click`,()=>{w.showProfileModal=!0,T()});let t=document.getElementById(`btn-close-profile-modal`),n=document.getElementById(`btn-cancel-profile-modal`);t&&t.addEventListener(`click`,()=>{w.showProfileModal=!1,T()}),n&&n.addEventListener(`click`,()=>{w.showProfileModal=!1,T()});let r=document.getElementById(`profile-edit-form`);r&&r.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`profile-name`).value.trim(),n=document.getElementById(`profile-email`).value.trim(),r=document.getElementById(`profile-new-password`).value;try{w.currentUser=a.updateUserProfile(w.currentUser.id,{name:t,email:n,newPassword:r?r.trim():void 0}),w.showProfileModal=!1,alert(`✅ Credenziali del profilo aggiornate con successo!`),T()}catch(e){alert(`Errore: ${e.message}`)}});let i=document.getElementById(`btn-logout`);i&&i.addEventListener(`click`,()=>{a.logout(),w.currentUser=null,w.activeTab=`dashboard`,T()});let o=document.getElementById(`btn-toggle-add-client`),s=document.getElementById(`add-client-form-container`),c=document.getElementById(`btn-cancel-add-client`),l=document.getElementById(`btn-save-new-client`);o&&s&&o.addEventListener(`click`,()=>{let e=s.style.display===`none`||!s.style.display;s.style.display=e?`block`:`none`}),c&&s&&c.addEventListener(`click`,()=>{s.style.display=`none`}),l&&l.addEventListener(`click`,()=>{let e=document.getElementById(`new-cli-name`).value.trim(),t=document.getElementById(`new-cli-type`).value,n=document.getElementById(`new-cli-ref`).value.trim(),r=document.getElementById(`new-cli-phone`).value.trim(),i=document.getElementById(`new-cli-email`).value.trim(),o=document.getElementById(`new-cli-city`).value.trim(),s=document.getElementById(`new-cli-machine`).value;if(!e){alert(`Inserisci la Ragione Sociale / Nome Cliente!`);return}try{let c=a.addClient({name:e,clientType:t,refPerson:n,phone:r,email:i,city:o,machineId:s});alert(`✅ Cliente "${c.name}" registrato con successo!`),T()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{w.editingId=e.getAttribute(`data-id`),T()})}),document.querySelectorAll(`.btn-del-client-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo cliente?`))try{a.deleteClient(t),alert(`✅ Cliente eliminato dall'anagrafica!`),T()}catch(e){alert(`Errore: ${e.message}`)}})});let u=document.getElementById(`btn-cancel-edit-client`);u&&u.addEventListener(`click`,()=>{w.editingId=null,T()});let d=document.getElementById(`form-edit-client`);d&&d.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-client-id`).value,n=document.getElementById(`edit-cli-name`).value.trim(),r=document.getElementById(`edit-cli-type`).value,i=document.getElementById(`edit-cli-ref`).value.trim(),o=document.getElementById(`edit-cli-phone`).value.trim(),s=document.getElementById(`edit-cli-city`).value.trim(),c=document.getElementById(`edit-cli-address`).value.trim(),l=document.getElementById(`edit-cli-machine`).value;try{a.updateClient(t,{name:n,clientType:r,refPerson:i,phone:o,city:s,address:c,assignedMachineId:l}),w.editingId=null,alert(`✅ Scheda Cliente salvata con successo!`),T()}catch(e){alert(`Errore: ${e.message}`)}});let f=document.getElementById(`btn-toggle-add-machine`),p=document.getElementById(`add-machine-form-container`),m=document.getElementById(`btn-cancel-add-machine`),h=document.getElementById(`btn-save-new-machine`);f&&p&&f.addEventListener(`click`,()=>{let e=p.style.display===`none`||!p.style.display;p.style.display=e?`block`:`none`}),m&&p&&m.addEventListener(`click`,()=>{p.style.display=`none`}),h&&h.addEventListener(`click`,()=>{let e=document.getElementById(`new-mc-serial`).value.trim(),t=document.getElementById(`new-mc-brand`).value.trim(),n=document.getElementById(`new-mc-model`).value.trim(),r=document.getElementById(`new-mc-year`).value.trim(),i=document.getElementById(`new-mc-board`).value,o=document.getElementById(`new-mc-client`).value;if(!e||!n){alert(`Inserisci Seriale Macchina e Modello!`);return}try{let s=a.addMachine({serialNumber:e,brand:t,model:n,productionYear:r,boardId:i,clientId:o});alert(`✅ Macchina da caffè SN "${s.serialNumber}" registrata con successo!`),T()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{w.editingId=e.getAttribute(`data-id`),T()})}),document.querySelectorAll(`.btn-del-machine-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa macchina da caffè?`))try{a.deleteMachine(t),alert(`✅ Macchina eliminata dal parco macchine!`),T()}catch(e){alert(`Errore: ${e.message}`)}})});let g=document.getElementById(`btn-cancel-edit-mc`);g&&g.addEventListener(`click`,()=>{w.editingId=null,T()});let _=document.getElementById(`form-edit-machine`);_&&_.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-mc-id`).value,n=document.getElementById(`edit-mc-serial`).value.trim(),r=document.getElementById(`edit-mc-brand`).value.trim(),i=document.getElementById(`edit-mc-model`).value.trim(),o=document.getElementById(`edit-mc-year`).value.trim(),s=document.getElementById(`edit-mc-board`).value,c=document.getElementById(`edit-mc-client`).value;try{a.updateMachine(t,{serialNumber:n,brand:r,model:i,productionYear:o,boardId:s,clientId:c}),w.editingId=null,alert(`✅ Scheda Macchina salvata con successo!`),T()}catch(e){alert(`Errore: ${e.message}`)}});let v=document.getElementById(`btn-toggle-add-board`),y=document.getElementById(`add-board-form-container`),b=document.getElementById(`btn-cancel-add-board`),x=document.getElementById(`btn-save-new-board`);v&&y&&v.addEventListener(`click`,()=>{let e=y.style.display===`none`||!y.style.display;y.style.display=e?`block`:`none`}),b&&y&&b.addEventListener(`click`,()=>{y.style.display=`none`}),x&&x.addEventListener(`click`,()=>{let e=document.getElementById(`new-board-code`).value.trim(),t=document.getElementById(`new-board-hwserial`).value.trim(),n=document.getElementById(`new-board-credits`).value,r=document.getElementById(`new-board-version`).value,i=document.getElementById(`new-board-machine`).value,o=document.getElementById(`new-board-groupcount`),s=o?parseInt(o.value,10):1;if(!e){alert(`Inserisci il Codice a 4 cifre del Deconto!`);return}try{let o=a.addBoard({shortCode:e,hwSerial:t,remainingCredits:n,version:r,groupCount:s,machineId:i});alert(`✅ Scheda Deconto #${o.shortCode} salvata PERMANENTEMENTE nel database!`),T()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{w.editingId=e.getAttribute(`data-id`),T()})}),document.querySelectorAll(`.btn-del-board-standalone`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questa scheda Deconto?`))try{a.deleteBoard(t),alert(`✅ Scheda Deconto eliminata!`),T()}catch(e){alert(`Errore: ${e.message}`)}})});let S=document.getElementById(`btn-cancel-edit-board`);S&&S.addEventListener(`click`,()=>{w.editingId=null,T()});let C=document.getElementById(`form-edit-board`);C&&C.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`edit-board-id`).value,n=document.getElementById(`edit-board-shortcode`).value.trim(),r=document.getElementById(`edit-board-hwserial`).value.trim(),i=document.getElementById(`edit-board-credits`).value,o=document.getElementById(`edit-board-version`).value,s=document.getElementById(`edit-board-machine`).value,c=document.getElementById(`edit-board-groupcount`),l=c?parseInt(c.value,10):1;try{a.updateBoard(t,{shortCode:n,hwSerial:r,remainingCredits:i,version:o,groupCount:l,machineId:s}),w.editingId=null,alert(`✅ Scheda Deconto salvata con successo!`),T()}catch(e){alert(`Errore: ${e.message}`)}});let E=document.getElementById(`btn-close-edit-modal`);E&&E.addEventListener(`click`,()=>{w.editingId=null,T()});let D=document.getElementById(`btn-toggle-add-user`),O=document.getElementById(`add-user-form-container`),k=document.getElementById(`btn-cancel-add-user`),A=document.getElementById(`btn-save-new-user`);D&&O&&D.addEventListener(`click`,()=>{let e=O.style.display===`none`||!O.style.display;O.style.display=e?`block`:`none`}),k&&O&&k.addEventListener(`click`,()=>{O.style.display=`none`}),A&&A.addEventListener(`click`,()=>{let e=document.getElementById(`new-user-username`).value.trim(),t=document.getElementById(`new-user-password`).value.trim(),n=document.getElementById(`new-user-name`).value.trim(),r=document.getElementById(`new-user-role`).value,i=document.getElementById(`new-user-email`).value.trim(),o=document.getElementById(`new-user-phone`).value.trim();if(!e||!t||!n){alert(`Compila i campi obbligatori: Codice Utente, Password e Nome!`);return}try{a.addUser({username:e,password:t,name:n,role:r,email:i,phone:o}),alert(`✅ Utente dipendente "${n}" (Codice ${e}) salvato PERMANENTEMENTE nel database!`),T()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-edit-staff-user`).forEach(e=>{e.addEventListener(`click`,()=>{w.editingStaffUserId=e.getAttribute(`data-id`),T()})});let j=document.getElementById(`btn-close-edit-staff-modal`),M=document.getElementById(`btn-cancel-edit-staff`);j&&j.addEventListener(`click`,()=>{w.editingStaffUserId=null,T()}),M&&M.addEventListener(`click`,()=>{w.editingStaffUserId=null,T()});let N=document.getElementById(`edit-staff-form`);N&&N.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`edit-staff-id`).value,n=document.getElementById(`edit-staff-username`)?document.getElementById(`edit-staff-username`).value:void 0,r=document.getElementById(`edit-staff-name`).value,i=document.getElementById(`edit-staff-role`)?document.getElementById(`edit-staff-role`).value:void 0,o=document.getElementById(`edit-staff-email`).value,s=document.getElementById(`edit-staff-phone`).value,c=document.getElementById(`edit-staff-password`).value;try{let e=a.updateUser(t,{username:n,name:r,role:i,email:o,phone:s,password:c?c.trim():void 0});w.editingStaffUserId=null,alert(`✅ Scheda Utente "${e.name}" salvata PERMANENTEMENTE!`),T()}catch(e){alert(`Errore: ${e.message}`)}}),document.querySelectorAll(`.btn-toggle-user-status`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-status`)===`ACTIVE`?`DISABLED`:`ACTIVE`;a.updateUser(t,{status:n}),T()})}),document.querySelectorAll(`.btn-delete-user`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);if(confirm(`Sei sicuro di voler eliminare questo utente dipendente?`))try{a.deleteUser(t),T()}catch(e){alert(`Errore: ${e.message}`)}})});let P=document.getElementById(`btn-open-email-logs`);P&&P.addEventListener(`click`,()=>{let e=a.getEmailLogs();e.length>0?(w.viewingEmailId=e[0].id,T()):alert(`Nessuna email spedita di recente nel registro.`)});let F=document.getElementById(`btn-close-email-preview`),I=document.getElementById(`btn-close-email-preview-footer`);F&&F.addEventListener(`click`,()=>{w.viewingEmailId=null,T()}),I&&I.addEventListener(`click`,()=>{w.viewingEmailId=null,T()});let L=document.getElementById(`rename-role-labels-form`);L&&L.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`role_label_UFFICIO`).value.trim(),n=document.getElementById(`role_label_ADR`).value.trim();a.updateRoleLabel(`UFFICIO`,t),a.updateRoleLabel(`ADR`,n),alert(`✅ Nomi delle Categorie Utente aggiornati con successo!`),T()});let R=document.getElementById(`permissions-matrix-form`);R&&R.addEventListener(`submit`,e=>{e.preventDefault();let t=[`UFFICIO`,`ADR`],n=[`canViewClients`,`canCreateClients`,`canEditClients`,`canDeleteClients`,`canGenerateQr`,`canGenerateOtp`,`canBleRefill`,`canUseSimulator`],r={UFFICIO:{},ADR:{}};t.forEach(e=>{n.forEach(t=>{let n=document.getElementById(`perm_${e}_${t}`);n&&(r[e][t]=n.checked)})}),a.updatePermissions(r),alert(`✅ Matrice dei Permessi aggiornata con successo per tutti gli utenti!`),T()});let z=document.getElementById(`setting-logo-file`);z&&z.addEventListener(`change`,e=>{let t=e.target.files[0];if(t){if(!t.type.startsWith(`image/`)){alert(`Seleziona un file immagine valido (PNG, JPG, SVG).`);return}let e=new FileReader;e.onload=function(e){let t=e.target.result;a.updateSettings({customLogoUrl:t}),alert(`✅ Nuovo Logo Aziendale caricato con successo!`),T()},e.readAsDataURL(t)}});let B=document.getElementById(`btn-reset-logo`);B&&B.addEventListener(`click`,()=>{confirm(`Ripristinare il logo predefinito con icona caffè ☕?`)&&(a.updateSettings({customLogoUrl:null}),alert(`✅ Logo predefinito ripristinato!`),T())});let V=document.getElementById(`settings-brand-form`);V&&V.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brand-title`).value.trim(),n=document.getElementById(`setting-brand-subtitle`).value.trim();a.updateSettings({brandTitle:t,brandSubtitle:n}),alert(`✅ Titolo e Sottotitolo Brand salvati con successo!`),T()});let H=document.getElementById(`settings-thresholds-form`);H&&H.addEventListener(`submit`,e=>{e.preventDefault();let t=parseInt(document.getElementById(`setting-threshold-yellow`).value,10),n=parseInt(document.getElementById(`setting-threshold-red`).value,10);if(isNaN(t)||isNaN(n)||n>=t){alert(`Attenzione: La Soglia Critica Rossa (X) deve essere inferiore alla Soglia Sottoscorta Gialla (Y)!`);return}a.updateSettings({thresholdYellow:t,thresholdRed:n}),alert(`✅ Soglie Automatiche Salvate con Successo!\n\n🟢 VERDE: > ${t} cialde\n🟡 GIALLO (Sottoscorta): da ${n+1} a ${t} cialde\n🔴 ROSSO (Critico): da 1 a ${n} cialde\n⚫ NERO (Bloccato): 0 cialde`),T()});let U=document.getElementById(`settings-brevo-form`);U&&U.addEventListener(`submit`,e=>{e.preventDefault();let t=document.getElementById(`setting-brevo-key`).value.trim(),n=document.getElementById(`setting-brevo-sender`).value.trim();a.updateSettings({brevoApiKey:t,brevoSenderEmail:n}),alert(`✅ API Key ed Email Mittente Brevo salvate con successo!`),T()}),document.querySelectorAll(`.kpi-card-clickable`).forEach(e=>{e.addEventListener(`click`,()=>{w.viewingKpiModal=e.getAttribute(`data-kpi`),T()})}),document.querySelectorAll(`.btn-close-kpi-modal`).forEach(e=>{e.addEventListener(`click`,()=>{w.viewingKpiModal=null,T()})}),document.querySelectorAll(`.btn-kpi-period`).forEach(e=>{e.addEventListener(`click`,()=>{w.kpiPeriod=e.getAttribute(`data-period`),T()})}),document.querySelectorAll(`.btn-kpi-charttype`).forEach(e=>{e.addEventListener(`click`,()=>{w.kpiChartType=e.getAttribute(`data-charttype`),T()})});let W=document.getElementById(`btn-apply-kpi-custom-date`);W&&W.addEventListener(`click`,()=>{let e=document.getElementById(`kpi-custom-start`).value,t=document.getElementById(`kpi-custom-end`).value;e&&t?(w.kpiCustomStart=e,w.kpiCustomEnd=t,alert(`✅ Filtro Date Personalizzato Applicato!\nDal: ${e}\nAl: ${t}`),T()):alert(`Seleziona sia la Data Inizio che la Data Fine dal calendario!`)});let G=document.getElementById(`btn-dash-search`),K=document.getElementById(`dash-search-input`);G&&K&&(G.addEventListener(`click`,()=>{w.dashSearchQuery=K.value,w.dashSearchCategory=document.getElementById(`dash-search-category`).value,T()}),K.addEventListener(`keypress`,e=>{e.key===`Enter`&&(w.dashSearchQuery=K.value,w.dashSearchCategory=document.getElementById(`dash-search-category`).value,T())}));let q=document.getElementById(`btn-dash-reset`);q&&q.addEventListener(`click`,()=>{w.dashSearchQuery=``,w.dashSearchCategory=`ALL`,T()}),document.querySelectorAll(`.th-sortable`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-col`);w.dashSortColumn===t?w.dashSortDirection=w.dashSortDirection===`ASC`?`DESC`:`ASC`:(w.dashSortColumn=t,w.dashSortDirection=`DESC`),T()})}),document.querySelectorAll(`.btn-deconto-detail`).forEach(e=>{e.addEventListener(`click`,()=>{w.viewingDecontoCode=e.getAttribute(`data-code`),T()})});let J=document.getElementById(`btn-close-deconto-modal`),Y=document.getElementById(`btn-close-deconto-modal-footer`);J&&J.addEventListener(`click`,()=>{w.viewingDecontoCode=null,T()}),Y&&Y.addEventListener(`click`,()=>{w.viewingDecontoCode=null,T()});let X=document.getElementById(`btn-export-csv`);X&&X.addEventListener(`click`,()=>{let e=a.exportCoffeeLogsCSV(),t=new Blob([e],{type:`text/csv;charset=utf-8;`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`deconto_erogazioni_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)});let Z=document.getElementById(`btn-trigger-backup`);Z&&Z.addEventListener(`click`,async()=>{try{Z.disabled=!0,Z.innerHTML=`⏳ Backup in Corso...`;let e=a.triggerGitHubBackup();alert(`✅ Backup Cloud completato!\nID: ${e.id}\nCommit: ${e.commitHash}\nRecords: ${e.recordCount}`)}catch(e){alert(`❌ Errore Backup GitHub: ${e.message}`)}finally{Z.disabled=!1,Z.innerHTML=`☁️ Esegui Backup GitHub`}})}T();